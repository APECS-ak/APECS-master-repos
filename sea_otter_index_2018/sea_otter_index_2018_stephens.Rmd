...
```{r}
# histograms
hist(df.index_2dens$dens_otter1) # transform
hist(df.index_2dens$dens_otter2) # transform
hist(df.index_2dens$n_pits) # transform
hist(df.index_2dens$prop_otter_shells) # transform

# log transformation
hist(log(df.index_2dens$dens_otter1 + 1))
hist(log(df.index_2dens$dens_otter2 + 1))
hist(log(df.index_2dens$n_pits + 1)) # log is good
hist(log(df.index_2dens$prop_otter_shells + 1)) # log is meh

# arcsine(sqrt) transformation
hist(asin(sqrt(df.index_2dens$prop_otter_shells))) # this is great
```
...
# no transformations
#dat.l <- cbind(dat.redu[,1:6])
#pairs(dat.l)

# all transformations
dat.l <- cbind(dat.redu[,1:2], log(dat.redu[,3:5] + 1))
dat.l <- cbind(dat.l, asin(sqrt(dat.redu[,6])))

colnames(dat.l)[6] <- "prop_so_shells" # change column names
...
```{r export}
# Data Export
write.csv(final.dat, "/Users/tiff/Desktop/R Studio/APECS-master-repos/sea_otter_index_2018/so_index_stephens.csv", row.names = FALSE)
```
...
#model specification
visual.model <- 'visual =~ so_duration + pop_dens_surv_km2 + dens_otter1 + dens_otter2 + n_pits + prop_so_shells'
...
summary(visual.fit, standardize = TRUE, fit.measures = TRUE)
```
