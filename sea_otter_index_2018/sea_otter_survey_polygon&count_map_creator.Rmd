...
```{r site}
site <- structure(list(id = "site", longitude = -132.994318, latitude = 55.264232), .Names = c("id", "longitude", "latitude"), class = "data.frame", row.names = c(NA, -1L))
xy <- site[,c(2,3)]
...
df.otts <- read.csv("/Users/tiff/Desktop/R Studio/APECS-master-repos/sea_otter_index_2018/sea_otter_counts_2018_stephens_2.csv", stringsAsFactors = FALSE, header = TRUE)
...
leaflet() %>% 
  ...
  addCircleMarkers(data = df.otts, ~longitude, ~latitude, color = "red", stroke = FALSE, radius = 2, fillOpacity = 1)
