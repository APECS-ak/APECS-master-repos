...
```{r model 1}
mod1 <- psem(
  lm(sqrt(eelgrass_total_mass) ~ log(epiphmass_shootmass) + log(clam_mass) + sqrt(sed_inside_prim) + Ntotal_site + light_avail, data = dat),
  ...
  lm(sqrt(crab_mass) ~ log(fish_preds) + sea_otter_index_tr_ns, data = dat),
  
  lm(log(clam_mass) ~ sqrt(crab_mass) + sea_otter_index_tr_ns, data = dat)
