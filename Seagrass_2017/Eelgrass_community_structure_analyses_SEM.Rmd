...
```{r mod list}
mods <- list(
    glm(log(abvgnd_mass) ~  sea_otter_index + log(epiphmass_shootmass), data = dat),
    glm(log(epiphmass_shootmass) ~  sea_otter_index  + log(abvgnd_mass) , data = dat),
    glm(sea_otter_index ~  log(abvgnd_mass) + log(epiphmass_shootmass) , data = dat))
```

```{r}
test <- psem(glm(log(abvgnd_mass) ~  sea_otter_index + log(epiphmass_shootmass), data = dat),
    glm(log(epiphmass_shootmass) ~  sea_otter_index + log(abvgnd_mass) , data = dat),
    glm(sea_otter_index ~ log(abvgnd_mass) + log(epiphmass_shootmass) , data = dat))
