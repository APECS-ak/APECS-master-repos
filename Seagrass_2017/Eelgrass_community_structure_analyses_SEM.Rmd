...
## Create fish predator group ##
dat$fish_pred_mass <- (dat$Cottidae + dat$Hexagrammidae + dat$Stichaeidae)
```
...
```{r mod list}
mods <- list(
    glm(abvgnd_mass ~  log(epiphmass_shootmass), data = dat),
    glm(log(epiphmass_shootmass) ~  log(grazermass_shootmass) + Ntotal_site, data = dat),
    glm(log(grazermass_shootmass) ~  sqrt(crab_mass), data = dat),
    glm(sqrt(crab_mass) ~ sea_otter_index_tr, data = dat))

```

```{r}
summary(psem(lm(abvgnd_mass ~  log(epiphmass_shootmass), data = dat),
    lm(log(epiphmass_shootmass) ~  log(grazermass_shootmass) + Ntotal_site, data = dat),
    lm(log(grazermass_shootmass) ~  sqrt(crab_mass), data = dat),
    lm(sqrt(crab_mass) ~ sea_otter_index_tr, data = dat)))
```

glm(log(grazermass_shootmass) ~ date_julian + sea_otter_index + log(epiphmass_shootmass) + sed_inside_prim + light_avail + Ntotal_site + log(crab_mass) + sqrt(fish_pred_mass), data = dat),
    glm(log(crab_mass) ~ date_julian + sea_otter_index + log(grazermass_shootmass) + sqrt(fish_pred_mass) + log(abvgnd_mass) + log(epiphmass_shootmass), data = dat),
    glm(sqrt(fish_pred_mass) ~ date_julian + sea_otter_index + log(grazermass_shootmass) + log(fish_mass) + log(abvgnd_mass) + log(epiphmass_shootmass) + log(crab_mass), data = dat)
