...
```{r seds by site, echo=FALSE, message=FALSE, warning=FALSE}
seds <- sed.site %>% 
...
Clam data preparation. We will take the mean biomass of all clams across all thress transects in each of the 4 bins in a transect. 
...
### The Meta Model
Starting from the theroetical prospective, we are studing the relationships, and by extension patterns of abundance, between predators, herbivors, plants, and the environment.
![](E:/wraymond2/My Documents/Graduate School/piecewiseSEM/Meta_model_1.png)

We can further refine this model to include apex predators.
![](E:/wraymond2/My Documents/Graduate School/piecewiseSEM/Meta_model_2.png)

### Conceptual model for sea otters in eelgrass ecosystems
Moving from the theroetical to the real world we can apply the above models to sea otters and eelgrass ecosystems. We can now build conceptual models for the relationships among species/functional groups. These models are developled from therory but also from obervational and experimental literature.

**Model 1**
![](E:/wraymond2/My Documents/Graduate School/piecewiseSEM/Model_1.png)

**Model 2**
![](E:/wraymond2/My Documents/Graduate School/piecewiseSEM/Model_2.png)

**Model 3**
![](E:/wraymond2/My Documents/Graduate School/piecewiseSEM/Model_3.png)

```{r model 1}
mod1 <- psem(
  lm(sqrt(eelgrass_total_mass) ~ log(epiphmass_shootmass) + sqrt(sed_inside_prim) + Ntotal_site + light_avail + log(grazermass_shootmass) + sea_otter_index_tr, data = dat),
  
  lm(log(epiphmass_shootmass) ~ log(grazermass_shootmass) + sqrt(sed_inside_prim) + Ntotal_site + light_avail, data = dat),
  
  lm(log(grazermass_shootmass) ~ sqrt(crab_mass) + log(fish_preds), data = dat),
  
  lm(sqrt(crab_mass) ~ log(fish_preds) + sea_otter_index_tr, data = dat)
)

mod2 <- psem(
    lm(sqrt(eelgrass_total_mass) ~ log(epiphmass_shootmass) + log(grazermass_shootmass) + sea_otter_index_tr, data = dat),
  
  lm(log(epiphmass_shootmass) ~ log(grazermass_shootmass) + sqrt(sed_inside_prim), data = dat),
