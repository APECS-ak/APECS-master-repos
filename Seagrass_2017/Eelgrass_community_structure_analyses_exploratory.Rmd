---
title: "Eelgrass_community_structure_analyses_exploratotry"
...
ggplot(dat) +
  ...
  geom_smooth(aes(x = date_julian, y = abvgnd_mass), method = lm) +
...
```{r model 2, echo = FALSE}
## Global Model ##
mod1t.step <- stepAIC(glm(sqrt(abvgnd_mass + blwgnd_mass) ~ date_julian + sea_otter_index_tr + log(epiphmass_shootmass) + log(grazermass_shootmass) + sqrt(crab_mass) + log(fish_preds) + sed_inside_prim + light_avail + Ntotal_site, data = dat), direction = "both")

mod1t.dr <- dredge(lm(sqrt(abvgnd_mass + blwgnd_mass) ~ date_julian + sea_otter_index_tr + log(epiphmass_shootmass) + log(grazermass_shootmass) + sqrt(crab_mass) + log(fish_preds) + sed_inside_prim + light_avail + Ntotal_site, data = dat), rank = "BIC", evaluate = TRUE)


mod1t.avg <- model.avg(mod1t.dr, rank = "BIC", subset = delta < 2, fit = TRUE)
summary(mod1t.avg)
predict(mod1t.avg, full = FALSE)
```
...
ggplot(dat, aes(reorder(place_name, sea_otter_index), sea_otter_index)) +
  geom_point(size = 8) +
  labs(x = "Site", y = "PC1", col = "Sea otter impact Index") +
  lims(y = c(-0.6, 1.5)) +
  theme(text = element_text(size = 25), axis.ticks = element_line(size = 2)) +
