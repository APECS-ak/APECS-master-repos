...
fish.um <- fish.um[c(1:26, 31, 32, 34, 35, 36, 38:53, 55:89, 91:98),]
...
```{r}
dat <- fish.m %>% 
  filter(site == "2017_H_03") %>% 
  filter(sp_code == "STICK3")

dens <- density(dat$fork_length, n = 1000)
plot(dens)
raw <- data.frame(x = round(dens$x, 0), y = dens$y)


test <- data.frame(bins = round(min(dens$x):max(dens$x), 0),
                   val = )

df <- data.frame(x = as.character(round(dens$x, 0)), y = dens$y)
df2 <- df %>% 
  group_by(x) %>% 
  summarise(pred = sum(y))
df2$x <- as.numeric(levels(df2$x))
df2$prop <- df2$pred/sum(df2$pred)
df2$count <- round(df2$prop * (1/min(df2$prop)), 0)
df3 <- rep(x = df2$x, df2$count)
test <- expandLenFreq(df3, 1, 3069)




d <- data.frame()
for(s in unique(fish.um$site)){
  dat.m <- fish.m %>% 
    filter(site == s)
  dat.um <- fish.um %>% 
      filter(site == s)
  for(i in unique(dat.um$sp_code)){
    samp <- dat.m %>%
      filter(sp_code == i)
    unmeas <- dat.um %>%
      filter(sp_code == i)
    unmeas <- as.numeric(unmeas$unmeasured)
    dens <- density(samp$fork_length, n = 1000)
    df <- data.frame(x = as.character(round(dens$x, 0)), y = dens$y)
    df2 <- df %>% 
      group_by(x) %>% 
      summarise(pred = sum(y))
    df2$x <- as.numeric(levels(df2$x))
    df2$prop <- df2$pred/sum(df2$pred)
    df2$count <- round(df2$prop * 1/min(df2$prop), 6)
    df3 <- rep(x = df2$x, df2$count)
    lfq <- expandLenFreq(df3, 1, numeas, startcat = abs(min(df3)), FALSE)
    lfq.df <- data.frame(site = s, sp_code = i, fl = lfq)
    d <- rbind(d, lfq.df)
  }
}
