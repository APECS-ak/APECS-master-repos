...
```{r}
hist(fish.m$fork_length[fish.m$sp_code == "SANDLNCP"], probability  = TRUE, breaks = 10)
sd(fish.m$fork_length[fish.m$sp_code == "SANDLNCP"])
lines(density(fish.m$fork_length[fish.m$sp_code == "SANDLNCP"], bw = 11.33))

density(fish.m$fork_length[fish.m$sp_code == "SANDLNCP"] )

dat <- fish.m %>% 
  filter(site == "2017_H_03") %>% 
  filter(sp_code == "STICK3")

hist(dat$fork_length, breaks = 20)
dens <- density(dat$fork_length, n = 1000)
hist(dens$x)
plot(dens)
range(dat$fork_length)
dens$x


test <- expandLenFreq(dens$x, 1, 3069)
```
