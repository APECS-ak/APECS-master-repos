---
title: "Eelgrass_community_structure_analyses_Bayesian"
...
# load other plotting features from Kruschke 2015
#source("../../DBDA2Eprograms/DBDA2Eprograms/DBDA2E-utilities.R")
```
...
plot(out4e, c('trace', 'histogram'), vars = c("sig", "beta"), layout = c(4, 2))
```
...
plot(out4e, "histogram", vars = c("sig", "beta"), layout = c(1, 6))
```
...
p1 <- ggplot(dfDay, aes(x = Julianday, y = Mean)) + 
  ...
  # geom_point(data = dfData, aes(x=x, y=y))+
  xlab('Julian Day') + 
  ylab('Predicted AG/BG mass +/- 95% CI ') +
  theme(text = element_text(size = 12), axis.ticks = element_line(size = 2), legend.position = "none")
...
p2 <- ggplot(dfOtt, aes(x = OtterIndex, y = Mean)) + 
  ...
  geom_line(size = 1) +
  xlab('Sea Otter Index') + 
  ylab('Mean Pred AG/BG mass +/- 95% CI ') +
  theme(text = element_text(size = 12), axis.ticks = element_line(size = 2), legend.position = "none")
...
CrabMass <- as.numeric(log(crab.string$mass + 0.0001)) [ii] ##### many 0s and/or log normalness to head with
...
FishCnt <- as.numeric(fish.site$count) ##### count obviously
FishMass <- as.numeric(fish.site$mass_g) 
FishDiv <- as.numeric(fish.site$SW.diverse)
Sitetxt <- tran.dat$site[ii]
Quad <- as.numeric(tran.dat$quadrat)[ii]
