...
dat.cor <- dat[, c(88, 92, 77, 69, 117, 19, 35, 37, 84, 116)]
colnames(dat.cor) <- c("Crabs", "Fish", "Grazers", "Epiphytes", "Eelgrass", "SOI*", "Macroalgae*", "Diatoms*", "Sediment*", "Nitrate*")

## Transform as necessary ##
dat.cor <- as.data.frame(cbind(sqrt(dat.cor$Crabs), log(dat.cor$Fish), log(dat.cor$Grazers), dat.cor$Epiphytes, dat.cor$Eelgrass, dat.cor$SOI, asin(sqrt(dat.cor$Macroalgae / 100)), asin(sqrt(dat.cor$Diatoms / 100)), dat.cor$Sediment, dat.cor$Nitrate))
colnames(dat.cor) <- c("Crabs", "Fish", "Grazers", "Epiphytes", "Eelgrass", "SOI*", "Macroalgae*", "Diatoms*", "Sediment*", "Nitrate*")

correl <- cor(dat.cor)

## Plotting correlations ##
corrplot(correl, method = "color", type = "lower", addCoef.col =  "black", number.cex = 0.5, tl.cex = 0.6, cl.cex = 0.5)
...
mod1 <- lm(abvgnd_mass_dt ~ sea_otter_index_tr + epiphmass_shootmass_dt + grazermass_shootmass + crab_mass + NOx_site, data = dat)
```
...
ggplot(data = dat) +
  ...
  geom_errorbar(aes(x = sea_otter_index_tr, ymin = crab_mass - crab_mass_se, ymax = crab_mass + crab_mass_se), width = 0) +
  labs(x = "SOI", y = "Crab mass (g)") +
...
## Fish and Crabs ##
ggplot(data = dat) +
  geom_point(aes(x = crab_mass, y = (log(fish_mass / 1000) + 1 )), size = 1.5) +
  labs(x = "Crab mass (g)", y = "Log fish mass (Kg)") +
...
## Grazers and Diatoms ##
ggplot(data = dat) +
  geom_point(aes(x = diatom_dens, y = grazermass_shootmass), size = 1.5) +
  geom_errorbar(aes(x = diatom_dens, ymin = grazermass_shootmass - grazermass_shootmass_se, ymax = grazermass_shootmass + grazermass_shootmass_se), width = 0) +
  labs(x = "Diatom cover (%)", y = "Grazer load (g/g)") +
...
ggplot(data = dat) +
  ...
  labs(x = "SOI", y = "Eelgrass mass (g/m)") +
  scale_x_continuous(limits = c(-0.80, 1.20), breaks = seq(-0.80, 1.20, by = 0.40)) +
...
ggplot(data = dat) +
  ...
  labs(x = bquote("Nitrate ("*mu~'mol)'), y = "Eelgrass mass (g/m)") +
  scale_x_continuous(limits = c(0.0, 3), breaks = seq(0, 3, by = 1)) +
  scale_y_continuous(limits = c(0.0, 200), breaks = seq(0, 200, by = 50)) +
  theme(text = element_text(size = 10))
```
