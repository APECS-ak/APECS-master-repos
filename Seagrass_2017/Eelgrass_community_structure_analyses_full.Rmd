...
mod4 <- lm(sqrt(crab_mass) ~ abvgnd_mass_dt + sea_otter_index_tr + log(grazermass_shootmass) + log(fish_mass) + sed_inside_prim, data = dat)
```
...
## Extract coefficients ##
coef.mod4 <- data.frame(confint(mod4.avg, full = FALSE))
t <- data.frame(mod4.avg$coefficients[2,])
coef.mod4$est <- t$mod4.avg.coefficients.2...
...
mod5 <- lm(log(fish_mass) ~  sea_otter_index_tr + abvgnd_mass_dt + log(grazermass_shootmass) + sqrt(crab_mass) + sed_inside_prim, data = dat)
```
