...
Nitrate is somewhat non-normal. Square-root tranformation improves.
```{r}
hist(dat$NOx_site)
hist(sqrt(dat$NOx_site))
```

Pit density is non-normal. Standard transformations do not help much. 
...
dat.cor <- dat[, c(19, 125, 94, 99, 81, 35, 37, 127, 126, 123, 90, 92)]

colnames(dat.cor) <- c("SOI", "Pits", "Crabs", "Fish", "Epifauna", "Macroalgae", "Diatoms", "Epiphytes", "Seagrass", "Nitrate", "Sediment", "Light")

## Transform as necessary ##
dat.cor$Macroalgae <- dat.cor$Macroalgae / 100
dat.cor$Diatoms <- dat.cor$Diatoms / 100
...
ggplot(dat) +
  ...
  labs(x = "Sea otter index", y = "Seagrass biomass") +
  theme(text = element_text(size = 10))

## SOI and sediment ##
ggplot(dat) +
  geom_point(aes(x = sea_otter_index_tr, y = sed_inside_prim)) +
  scale_y_continuous(limits = c(1, 6), breaks = seq(1, 6, by = 1)) +
  labs(x = "Sea otter index", y = "Sediment size score") +
  theme(text = element_text(size = 10))

## Sediment and diatoms ##
ggplot(dat) +
  geom_point(aes(x = sed_inside_prim, y = diatom_dens / 100)) +
  scale_y_continuous(limits = c(0, 1.00), breaks = seq(0, 1.00, by = 0.25)) +
  labs(x = "Sediment size score", y = "Proportion diatom cover") +
  theme(text = element_text(size = 10))

## Sediment and crabs ##
ggplot(dat) +
  geom_point(aes(x = sed_inside_prim, y = crab_mass)) +
  scale_y_continuous(limits = c(0, 300), breaks = seq(0, 300, by = 100)) +
  labs(x = "Sediment size score", y = "Crab biomass (g/site)") +
  theme(text = element_text(size = 10))
```
...
## Models
General approach
1. Pass a global model to the `dredge()` funciton. This global model will include only hypothesized direct (a -> b) interactions for a given response, no potential indirect effects. Models will be ranked using AICc, as is recommended with small sample sizes. In general interactions will not be evaluated unless there is a seriously good biological reason.
...
Model 1.1 (TC): epiphtyes, nitrate, light, pits, sediment


```{r mod1 global, echo=FALSE}
mod1.1 <- lm(abvgnd_mass_dt ~ epiphmass_shootmass_dt + NOx_site + pits_all + sed_inside_prim + light_avail, data = dat)
```
...
Model 2.1 (TC): nitrate, light, grazers

```{r mod2 global, echo=FALSE}
mod2.1 <- lm(epiphmass_shootmass_dt ~ grazermass_shootmass + NOx_site + light_avail, data = dat)
```
...
Model 3.1 (TC): epiphytes, crabs, fishes, macroalgae, diatom cover

```{r mod3 global, echo=FALSE}
mod3.1 <- lm(log(grazermass_shootmass) ~ epiphmass_shootmass_dt + crab_mass + fish_mass + macro_dens_as + diatom_dens_as, data = dat)
```
...
Model 4.1 (TC): sea otter index, epifauna

```{r mod4 global, echo=FALSE}
mod4.1 <- lm(sqrt(crab_mass) ~ sea_otter_index_tr + grazermass_shootmass, data = dat)
```
...
mod5.1 <- lm(log(fish_mass) ~  grazermass_shootmass, data = dat)
```

Forego dredge models because only one predictor
```{r mod5 dr, echo=FALSE}
## Summary ##
summary(mod5.1)

data.frame(std.coef(mod5.1, partial.sd = FALSE))
```
