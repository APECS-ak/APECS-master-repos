...
SOI, seagrass biomass, sediment grain size
```{r}
dat.cor.sed <- dat[, c(19, 126, 90)] 
colnames(dat.cor.sed) <- c("SOI", "Seagrass", "Sediment")

correl.sed <- cor(dat.cor.sed)
corrplot(correl.sed, method = "color", type = "lower", addCoef.col =  "black", number.cex = 0.75, tl.cex = 0.8, cl.cex = 0.75, tl.col = "black")

pairs(dat.cor.sed, cex.labels = 1, cex = 0.8, cex.axis = 0.6, pch = 16)
```
...
```{r export dredge}
write.csv(mod1.1.dr.top, "abv_raw_dredge_1.1.csv", row.names = FALSE)
write.csv(mod1.2.dr.top, "abv_raw_dredge_1.2.csv", row.names = FALSE)

write.csv(mod2.1.dr.top, "epi_raw_dredge.csv", row.names = FALSE)
write.csv(mod2.2.dr.top, "epi_raw_dredge_2.2.csv", row.names = FALSE)

write.csv(mod3.1.dr.top, "grz_raw_dredge.csv", row.names = FALSE)
write.csv(mod3.2.dr.top, "grz_raw_dredge_3.2.csv", row.names = FALSE)

write.csv(mod4.1.dr.top, "crb_raw_dredge.csv", row.names = FALSE)

write.csv(mod5.1.dr.top, "fsh_raw_dredge_1.1.csv", row.names = FALSE)
```
...
ggplot(nut.sum) +
  geom_col(aes(x = site_full, y = NH4_umol)) +
  theme(text = element_text(size = 18), axis.ticks = element_line(size = 2)) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
mean(dat$abvgnd_mass[dat$sea_otter_index_tr <=-0.4073554])
st.er(dat$abvgnd_mass[dat$sea_otter_index_tr <=-0.4073554])

mean(dat$abvgnd_mass[dat$sea_otter_index_tr > 0.1729845])
st.er(dat$abvgnd_mass[dat$sea_otter_index_tr > 0.1729845])

## Raw eelgrass and soi ##
ggplot(dat) +
  geom_point(aes(x = sea_otter_index_tr, y = abvgnd_mass), size = 2) +
  labs(x = "SOI", y = "aboveground biomass")
...
p1 <- ggplot(dat) +
  geom_point(aes(x = abvgnd_mass_dt, y = sqrt(crab_mass))) +
  geom_smooth(aes(x = abvgnd_mass_dt, y = sqrt(crab_mass)), method = "lm") +
  labs(x = "Seagrass mass", y = "Crabs") 

p2 <- ggplot(dat) +
  geom_point(aes(x = shoot_dens, y = sqrt(crab_mass))) +
  geom_smooth(aes(x = shoot_dens, y = sqrt(crab_mass)), method = "lm") +
  labs(x = "Seagrass density", y = "Crabs") 
