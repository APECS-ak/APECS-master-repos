...
mod1 <- lm(abvgnd_mass_dt ~ sea_otter_index_tr + epiphmass_shootmass_dt + grazermass_shootmass + crab_mass + fish_mass + asin(sqrt(macro_dens / 100)) + asin(sqrt(diatom_dens / 100)) + sed_inside_prim + Ntotal_site, data = dat)
```
...
mod2 <- lm(epiphmass_shootmass_dt ~ sea_otter_index_tr + grazermass_shootmass + crab_mass + fish_mass + sed_inside_prim + Ntotal_site + asin(sqrt(macro_dens / 100)) + asin(sqrt(diatom_dens / 100)), data = dat)
```
...
p.epi <- ggplot(coef.mod2) +
  ...
  scale_y_continuous(limits = c(-1, 1), breaks = seq(-1, 1, by = 0.50)) +
  labs(x = "Factor", y = "Estimate +/- 95% CI") +
  scale_x_discrete(labels = c("Benthic\ndiatoms", "Crabs", "Fish", "Grazers", "Nitrogen")) +
  theme(text = element_text(size = 10))
```
...
mod3 <- lm(log(grazermass_shootmass) ~ sea_otter_index_tr + epiphmass_shootmass_dt + crab_mass + fish_mass + sed_inside_prim + Ntotal_site + asin(sqrt(macro_dens / 100)) + asin(sqrt(diatom_dens / 100)), data = dat)
```
