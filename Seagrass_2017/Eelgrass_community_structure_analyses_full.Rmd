...
colnames(dat.cor) <- c("Sea otter index", "Pits", "Crabs", "Fish", "Epifauna", "Macroalgae", "Diatoms", "Epiphytes", "Seagrass", "Nitrate", "Sediment", "Light")
...
corr.all <- corrplot(correl, method = "color", type = "lower", addCoef.col =  "black", number.cex = 0.75, tl.cex = 0.8, cl.cex = 0.75, tl.col = "black")
...
crb.sed <- ggplot(dat) +
  ...
  labs(x = "Sediment size score", y = "Crab biomass (g)") +
  theme(text = element_text(size = 10))
```

Correlation one-to-one panel
```{r}
plot_grid(eg.soi, sed.soi,
          dia.sed, crb.sed,
