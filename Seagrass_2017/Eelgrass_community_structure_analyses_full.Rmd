...
Global model will include, sea otter index, epiphyte load, grazer load, crab biomass, and nitrate. Note transformations of variables where appropraite.
```{r mod1 global, echo=FALSE}
mod1 <- lm(abvgnd_mass_dt ~ sea_otter_index_tr + crab_mass + grazermass_shootmass + epiphmass_shootmass_dt + NOx_site, data = dat)
```
...
Global model will include sea otter index, grazer load, crab biomass, aboveground eelgrass biomass and nitrate. Note transformations of variables where appropraite.
```{r mod2 global, echo=FALSE}
mod2 <- lm(epiphmass_shootmass_dt ~ sea_otter_index_tr + grazermass_shootmass + crab_mass + NOx_site + abvgnd_mass_dt, data = dat)
```
...
Global model will include, sea otter index, epiphyte load, crab biomass, aboveground eelgrass biomass, and nitrate. Note transformations of variables where appropraite.

```{r mod3 global, echo=FALSE}
mod3 <- lm(log(grazermass_shootmass) ~ sea_otter_index_tr + epiphmass_shootmass_dt + crab_mass + NOx_site + abvgnd_mass_dt, data = dat)
```
...
mod4 <- lm(sqrt(crab_mass) ~ abvgnd_mass_dt + sea_otter_index_tr + grazermass_shootmass + epiphmass_shootmass_dt + NOx_site, data = dat)
```
