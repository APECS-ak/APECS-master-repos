...
2. epiphyte load (g epiphyte / g eelgrass)
3. total grazer load (g grazers/ g eelgrass)
4. crab biomass (g) - can also be counts
5. fish biomass (g) - can also be counts
...
```{r avb:blw grnd biomass, echo=FALSE}
hist(dat$abvgnd_mass/dat$blwgnd_mass)
shapiro.test(dat$abvgnd_mass/dat$blwgnd_mass)
```
...
Sediment is not normal and there is not a good transformation becasue there are so many low values.
...
Checking for multicolinearity among predictors. There is little evidence for multicolinearity among predictors. Note that the plots shows string correlating between crab mass and other crab variables, which is to be expected, but they would not be used together in the same model. 
...
In an effort to avoid spurious models, we can look at some of the environmental co-variates across sites to see if there are even patters.
...
Global model will include, sea otter index, epiphyte load, grazer load, crab biomass, fish biomass, sediment score, and total nitrogen. Note transformations of variables where appropraite.
```{r mod1 global, echo=FALSE}
mod1 <- lm(abvgnd_mass_dt ~ sea_otter_index_tr + epiphmass_shootmass_dt + log(grazermass_shootmass) + sqrt(crab_mass) + log(fish_mass) + sed_inside_prim + Ntotal_site, data = dat)
```
...
Global model will include sea otter index, grazer load, crab biomass, fish biomass, sediment score, total nitrogen. Note transformations of variables where appropraite.
```{r mod2 global}
mod2 <- lm(epiphmass_shootmass_dt ~ sea_otter_index_tr + log(grazermass_shootmass) + sqrt(crab_mass) + log(fish_mass) + sed_inside_prim + Ntotal_site, data = dat)
```
...
```{r mod3 global}
mod3 <- lm(log(grazermass_shootmass) ~ sea_otter_index_tr + epiphmass_shootmass_dt + sqrt(crab_mass) + log(fish_mass) + sed_inside_prim + Ntotal_site, data = dat)
```
...
```{r mod4 global}
mod4 <- lm(sqrt(crab_mass) ~ sea_otter_index_tr + +log(grazermass_shootmass) + log(epiphmass_shootmass) + log(fish_mass) + sqrt(sed_inside_prim) + Ntotal_site, data = dat)
```
...
```{r mod5 global}
mod5 <- lm(log(fish_mass) ~  sea_otter_index_tr + abvgnd_mass_dt + log(grazermass_shootmass) + log(epiphmass_shootmass) + sqrt(crab_mass)  + sqrt(sed_inside_prim) + Ntotal_site, data = dat)
```
...
ggplot(dat) +
  ...
  labs(x = "sea otter index", y = "fish preds") +
  theme(text = element_text(size = 18), axis.ticks = element_line(size = 2)) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
