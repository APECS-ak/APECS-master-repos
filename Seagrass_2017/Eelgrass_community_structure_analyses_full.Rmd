...
```{r mod1 global}
mod1 <- lm(abvgnd_mass_dt ~ sea_otter_index_tr + epiphmass_shootmass_dt + log(grazermass_shootmass) + sqrt(crab_mass) + log(fish_preds) + sed_inside_prim + Ntotal_site + light_avail, data = dat)
```
...
model.avg(mod1.dr, subset = delta < 2, fit = TRUE, beta = "sd")
```

Extract coefficents errors from top models.
```{r coefs top mod1}
mod1.list <- coefTable(mod1.dr)[1:3]

mod1.coefs <- data.frame()
for(i in 1:3){
  q <- data.frame(mod1.list[[i]])
  r <- cbind(row.names(q), q)
  r$rank <- i
  r$model <- 1
  mod1.coefs <- rbind(mod1.coefs, r)
}

colnames(mod1.coefs) <- c("factor", "estimate", "se", "df", "rank", "model")
mod1.coefs$lwr <- mod1.coefs$estimate - (1.96 * mod1.coefs$se)
mod1.coefs$upr <- mod1.coefs$estimate + (1.96 * mod1.coefs$se)
mod1.coefs <- filter(mod1.coefs, factor != "(Intercept)")
```

Plotting coefficients by model
```{r plots coefs}
ggplot(mod1.coefs, aes(color = factor)) +
  geom_hline(yintercept = 0, lty = 2) +
  geom_point(aes(rank, estimate), size = 3, position = position_dodge(width = 1/2)) +
  geom_errorbar(aes(rank, ymin = lwr, ymax = upr), width = 0, position = position_dodge(width = 1/2)) +
  coord_flip() + 
  scale_y_continuous(limits = c(-1, 1.07), breaks = seq(-1, 1.07, by = 0.50)) +
  labs(x = "Model Rank", y = " Coefficient +/- 95% CI")
```
...
```{r mod2 global}
mod2 <- lm(epiphmass_shootmass_dt ~ sea_otter_index_tr + log(grazermass_shootmass) + sqrt(crab_mass) + log(fish_preds) + sed_inside_prim + Ntotal_site + light_avail, data = dat)
```

Dredge and filter models
```{r mod2 dr}
## Dredge ##
mod2.dr <- dredge(mod2, rank = "AICc", evaluate = TRUE, extra = c("R^2","adjR^2"), beta = "sd")
mod2.dr
...
```{r fit top mod2}
## Model fits ##
mod2a <- lm(epiphmass_shootmass_dt ~ log(grazermass_shootmass) + sed_inside_prim, data = dat)
...
```{r mod3 global}
mod3 <- lm(log(grazermass_shootmass) ~ sea_otter_index_tr + epiphmass_shootmass_dt + sqrt(crab_mass) + log(fish_preds) + sed_inside_prim + Ntotal_site + light_avail, data = dat)
```

Dredge and filter models
```{r mod3 dr}
## Dredge ##
mod3.dr <- dredge(mod3, rank = "AICc", evaluate = TRUE, extra = "adjR^2", beta = "sd")
mod3.dr
...
```{r mod4 dr}
## Dredge ##
mod4.dr <- dredge(mod4, rank = "AICc", evaluate = TRUE, extra = "adjR^2", beta = "sd")
mod4.dr
...
```{r mod5 dr}
## Dredge ##
mod5.dr <- dredge(mod5, rank = "AICc", evaluate = TRUE, extra = "adjR^2", beta = "sd")
mod5.dr
