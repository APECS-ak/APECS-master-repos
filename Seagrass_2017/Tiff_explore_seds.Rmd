...
#### 1. To attack this, we need the sediment_pit, biometrics, and transect dataframes from the seagrass sites. Also throw in the coordinate info. Import all of these cleaned files from the GitHub repositry. 
```{r, tidy = TRUE}
...
#### 2. The next steps are to look at each imported DF, tidy them up, reshape them for the intended analyses, and join all reshaped DFs into one large DF (i.e. 'df.all'). All packages used are libraried at the top of this script. The sript echoed in this .html file but can be accessed in the .Rmd file. 
```{r, include = FALSE} 
...
#### 3. Now all of the prepared DFs will be joined together to form the 'df.all', the primary DF that will be used in the following analyses.
```{r, tidy = TRUE}
...
#### Regression of sediment versus sea otter index
```{r, include = FALSE}
