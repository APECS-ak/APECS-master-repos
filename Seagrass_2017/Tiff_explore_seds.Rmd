...
df.SOI: it would also be useful to have the SOI PCA1 values for plots, later...extract those.
```{r, include = FALSE}
# extract biomass and reshape to quadrat for working with count data
df.soi <- df.soi %>% 
...
OK. From the previous reshaping, we have 7 DFs (df.coords, df.soi, df.trans2, df.sedpit2, df.sedpit_trans, df.bio2, df.mass_area) that we want to join into one big DF.
...
##### There exists invisible steps here, where date is converted to Julian Day and all columns are reordered so variable names are easier to locate for later analysis (since there are 50 of them)
```{r, include = FALSE}
...
df.all <- df.all[, -c(38:39,42:51)] # delete rhi_length, rhi_mass_tot, and all nodes
colnames(df.all)




# old way to reorganize based on column number...this is sloppy in dynamic coding script
#df.all <- df.all[c(1,2,3,4,5,52,6,15,16,17,11,19,18,20,22,21,23,25,24,26,12,13,14,7,8,27,10,28,9,49,32,31,34,33,29,30,35,36,37,48,43,44,45,46,47,38,39,40,41,42,50,51)]
```
<br>
...
df.all[, 1:11][df.all[, 1:11] == -0.5] <- NA


my.formula <- y ~ x
p1 = ggplot(df.all, aes(leaf_length_max, epi_mass_leafarea_mg.cm2, color = so_index)) +
  geom_point(size=4) +
  geom_smooth(method=lm, formula=y~x, se=FALSE, fullrange=FALSE) +
  #xlab("Sea otter density\n")+ylab("\nAboveground biomass residuals") +
...
ln.m1 <- lm(rhi_mass_tot ~ sed2_site, data = df.all)
summary(ln.m1)
