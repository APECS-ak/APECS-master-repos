...
library(dplyr); library(tidyr); library(plyr); library(reshape2); library(ggplot2); library(DT)
...
#datatable(sed.table) %>% formatStyle(names(sed.table),
 # backgroundSize = '98% 88%',
 # backgroundRepeat = 'no-repeat',
 # backgroundPosition = 'center'
#)

#datatable(head(sed.table, width = 400), options = list(
...
#### To attack this, we need the sediment_pit, biometrics, and transect dataframes from the seagrass sites. First, import these cleaned files from the GitHub repositry. 
```{r}
...
OK. From the previous reshaping, we have 5 DFs (df.trans2, df.sedpit2, df.sedpit_trans, df.bio2, df.mass_area) that we want to join into one big DF. This results in 47 columns of unique variables...that's a lot but we aren't concerned about ALL of them...they are just there for exploration, afterall. 
