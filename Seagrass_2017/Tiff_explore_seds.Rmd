...
df.all <- left_join(df.sedpit2, df.trans2, by = c("site", "date"))
df.all <- left_join(df.all, df.bio2, by = c("site", "date"))
...
# sum all pits in each transect per site
pits_all <- df.sedpit %>% 
  select(site, date, trans_type, pit_bin)

m.pits_all <- melt(pits_all, id = 1:3, na.rm = TRUE)

trim_pits <- dcast(m.pits_all, site + date + trans_type ~ ., sum)
colnames(trim_pits)[which(names(trim_pits) == ".")] <- "tot_pits"
...
m.sed1_all <- melt(sed1_all, id = 1:3, na.rm = TRUE)

trim_sed1 <- dcast(m.sed1_all, site + date + trans_type ~ ., mean)
colnames(trim_sed1)[which(names(trim_sed1) == ".")] <- "sed1_mean"
...
m.sed2_all <- melt(sed2_all, id = 1:3, na.rm = TRUE)

trim_sed2 <- dcast(m.sed2_all, site + date + trans_type ~ ., mean)
colnames(trim_sed2)[which(names(trim_sed2) == ".")] <- "sed2_mean"
...
df.sedpit_transects <- left_join(trim_pits, trim_sed1, by = c("site", "date", "trans_type"))
df.sedpit_transects <- left_join(df.sedpit_transects, trim_sed2, by = c("site", "date", "trans_type"))

# find the mean of the primary and secondary sediment types
df.sedpit_transects$sed_all_mean <- rowMeans(subset(df.sedpit_transects, select = c(sed1_mean, sed2_mean)), na.rm = TRUE)
