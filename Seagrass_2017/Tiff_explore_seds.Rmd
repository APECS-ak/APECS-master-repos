...
library(dplyr); library(tidyr); library(plyr); library(reshape2); library(ggplot2); library(DT); library(leaflet); library(htmlwidgets); library(htmltools); library(shiny); library(mapview); library(sp); library(sf)
...
colnames(trim_pits)[which(names(trim_pits) == ".")] <- "tot_pits" # give data column appropriate name
...
map1 <- leaflet(df.all) %>% addTiles() %>% 
  addCircleMarkers(
    ...
    color = ~'Red',
    label = as.character(df.all$sed1_mean)
  )
...
map2 <- leaflet(df.all) %>% addTiles() %>% 
  addCircleMarkers(
    ...
    color = ~'Orange',
    label = as.character(df.all$sed2_mean)
  )
...
map3 <- leaflet(df.all) %>% addTiles() %>% 
  addCircleMarkers(
    lng = ~longitude, lat =  ~latitude,
    radius = ~(sed_all_mean) * 2,
    stroke = FALSE, fillOpacity = 0.7, 
    color = ~'Gold',
    label = as.character(df.all$sed_all_mean)
  )
