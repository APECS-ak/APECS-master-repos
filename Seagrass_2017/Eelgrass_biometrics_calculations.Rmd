...
so.index <- read.csv("ALL_DATA/sea_otter_impact_index_2017.csv", stringsAsFactors = FALSE, header = TRUE)
...
dat <- data.frame(cbind(eg.site[, c(1:3, 32, 4:9)], tran.dat[, 2:5], nuts.site.eg [,2:11], nuts.site.nday.1m [,2:11], nuts.site.nday.4m [,2:11], so.index[, 6:13], eg.site[, 14:26]))
```
