...
Alright so fishbase dumps a lot of data in there but we only care about part of it. There are two things to accopmplish.
1. Some species have multiple entries, thoes need to be reduced to what makes the most sense for us.
2. There data have a lot of extra columns that we don't care about, those can be removed.

```{r cleaning up fishbase data}
## Filter out ---
sp.diet <- sp.diet %>% 
  filter(SampleStage != "larvae")

sp.diet <- sp.diet[, c(1:6, 41:47)]
```
