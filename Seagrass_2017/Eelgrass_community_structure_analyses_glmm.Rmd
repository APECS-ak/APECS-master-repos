...
```{r model 7}
## Global Model ##
mod7 <- stepAIC(glm(log(dat$grazermass_shootmass) ~ 1), scope = list(upper = ~dat$date_julian + dat$sea_otter_index + log(dat$epiphmass_shootmass) + log(dat$crab_mass) + log(dat$fish_mass)), direction = "both")
...
```{r}
ggplot(dat) +
  geom_point(aes(x = log(fish_mass), y = log(grazermass_shootmass)), size = 3)
