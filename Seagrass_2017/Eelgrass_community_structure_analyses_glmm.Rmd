...
We generated a global model that includes linear and quadratic effects of julian day and sea otter index, and linear effects of sediment type and light attenuation. A combine forward and backwad election approach indicated that a model with quadratic effects of julian data and sea otter index was the best model.
```{r model 1}
## Global Model ##
mod1 <- stepAIC(glm(log(dat$abvgnd_mass) ~ 1), 
                scope = list(upper = ~dat$date_julian + dat$sea_otter_index + poly(dat$date_julian, 2) + poly(dat$sea_otter_index, 2) + dat$sed_inside_prim + dat$light_atten + dat$Ntotal_site, lower = ~ 1), direction = "both")
```

### Best Model 1
```{r model 1 best}
mod1.best <- glm(log(abvgnd_mass) ~  poly(date_julian, 2) + poly(sea_otter_index, 2), data = dat) 
summary(mod1.best)
par(mfrow = c(2, 2))
plot(mod1.best)
par(mfrow = c(1, 1))
...
mind <- min(dat$date_julian)
maxd <- max(dat$date_julian)
...
int <- mod1.best$coefficients[1]
time1 <- mod1.best$coefficients[2]
time2 <- mod1.best$coefficients[3]
soi1 <- mod1.best$coefficients[4]
soi2 <- mod1.best$coefficients[5]
sig <- mod1.best$

for (d in mind:maxd){
  Julday[d] <- d - 1 + mind
  Mu <- int + time1*d + time2*d^2
...
We generated a global model that includes linear and quadratic effects of julian day and sea otter index, and linear effects of sediment type and light attenuation. A combined forward and backwad election approach indicated that a model with quadratic effects of julian data and sea otter index was the best model.
```{r model 2}
## Global Model ##
mod2 <- stepAIC(glm(log(dat$blwgnd_mass) ~ 1), 
                scope = list(upper = ~dat$date_julian + dat$sea_otter_index + poly(dat$date_julian, 2) + poly(dat$sea_otter_index, 2) + dat$sed_inside_prim + dat$light_atten + dat$Ntotal_site, lower = ~ 1), direction = "both")
...
### Best Model 2
```{r model 2 best}
mod2.best <- glm(log(blwgnd_mass) ~  date_julian, data = dat)
summary(mod2.best)
par(mfrow = c(2, 2))
plot(mod2.best)
par(mfrow = c(1, 1))
...
## Global Model ##
mod3 <- stepAIC(glm.nb(dat$shoot_dens_rnd ~ 1, link = "log"), 
                scope = list(upper = ~dat$date_julian + dat$sea_otter_index + poly(dat$date_julian, 2) + poly(dat$sea_otter_index, 2) + dat$sed_inside_prim + dat$light_atten + dat$Ntotal_site, lower = ~ 1), direction = "both")
...
### Best Model 3
```{r model 3 best}
mod3.best <- glm.nb(shoot_dens_rnd ~  poly(date_julian, 2) + poly(sea_otter_index, 2), link = "log", data = dat)
summary(mod3.best)
par(mfrow = c(2, 2))
plot(mod3.best)
par(mfrow = c(1, 1))
...
```{r model 4}
## Global Model ##
mod4 <- stepAIC(glm(log(dat$abvgnd_mass / dat$blwgnd_mass) ~ 1), 
                scope = list(upper = ~dat$date_julian + dat$sea_otter_index + poly(dat$date_julian, 2) + poly(dat$sea_otter_index, 2) + dat$sed_inside_prim + dat$light_atten + dat$Ntotal_site, lower = ~ 1), direction = "both")
...
```{r model 5}
## Global Model ##
mod5 <- stepAIC(glm(log(dat$node2) ~ 1), 
                scope = list(upper = ~dat$date_julian + dat$sea_otter_index + poly(dat$date_julian, 2) + poly(dat$sea_otter_index, 2) + dat$sed_inside_prim + dat$light_atten + dat$Ntotal_site, lower = ~ 1), direction = "both")
...
### Best Model 5
```{r model 5 best}
mod5.best <- glm(log(node2) ~  poly(date_julian, 2) + light_atten + Ntotal_site, data = dat)
summary(mod5.best)
par(mfrow = c(2, 2))
plot(mod5.best)
par(mfrow = c(1, 1))
...
```{r model 6}
## Global Model ##
mod6 <- stepAIC(glm(log(dat$epiphmass_shootmass) ~ 1), 
                scope = list(upper = ~dat$date_julian + dat$sea_otter_index + dat$sed_inside_prim + dat$light_atten + log(dat$grazermass_shootmass) + dat$Ntotal_site), direction = "both")
...
### Best Model 6
```{r model 6 best}
mod6.best <- glm(log(dat$epiphmass_shootmass) ~ log(dat$grazermass_shootmass) + dat$sed_inside_prim + dat$light_atten)
summary(mod6.best)
par(mfrow = c(2, 2))
plot(mod6.best)
par(mfrow = c(1, 1))
...
```{r model 7}
## Global Model ##
mod7 <- stepAIC(glm(log(dat$grazermass_shootmass) ~ 1), 
                scope = list(upper = ~dat$date_julian + dat$sea_otter_index + log(dat$epiphmass_shootmass) + dat$crab_mass), direction = "both")
...
```{r model 8}
## Global Model ##
mod8 <- stepAIC(glm(log(dat$crab_mass) ~ 1), 
                scope = list(upper = ~dat$date_julian + dat$sea_otter_index + log(dat$grazermass_shootmass)), direction = "both")
...
## Model 8b - crab ~  SOI ##
mod8b <- summary(glm(log(crab_mass) ~ sea_otter_index, data = dat))

## Model 8c - crab  ~ Julian day + SOI ##
mod8c <- summary(glm(log(crab_mass) ~ date_julian + sea_otter_index, data = dat))

## Model 8d - crab  ~ Julian day + SOI + crab ##
mod8d <- summary(glm(log(crab_mass) ~ date_julian + sea_otter_index + log(grazermass_shootmass), data = dat))

## Model 8e - crab ~ grz  ##
mod8e <- summary(glm(log(crab_mass) ~ log(grazermass_shootmass), data = dat))

## Model 8f - crab ~ SOI + grz ##
mod8f <- summary(glm(log(crab_mass) ~ sea_otter_index + log(grazermass_shootmass), data = dat))

## Model 8 Comparison ##
Model.8.Comp <- data.frame(model = c("mod8a", "mod8b", "mod8c", "mod8d", "mod8e", "mod8f"), AIC = numeric(length = 6),
          dispersion = numeric(length = 6),
          deviance = numeric(length = 6),
          df.resid = numeric(length = 6))

Model.8.Comp[1, 2:5] <- c(mod8a$aic, mod8a$dispersion, mod8a$deviance, mod8a$df.residual)
Model.8.Comp[2, 2:5] <- c(mod8b$aic, mod8b$dispersion, mod8b$deviance, mod8b$df.residual)
Model.8.Comp[3, 2:5] <- c(mod8c$aic, mod8c$dispersion, mod8c$deviance, mod8c$df.residual)
Model.8.Comp[4, 2:5] <- c(mod8d$aic, mod8d$dispersion, mod8d$deviance, mod8d$df.residual)
Model.8.Comp[5, 2:5] <- c(mod8e$aic, mod8e$dispersion, mod8e$deviance, mod8e$df.residual)
Model.8.Comp[6, 2:5] <- c(mod8f$aic, mod8f$dispersion, mod8f$deviance, mod8f$df.residual)


## Order by lowest AIC and clean up ##
Model.8.Comp <- Model.8.Comp[order(Model.8.Comp$AIC, decreasing = FALSE),]

Model.8.Comp$del_AIC <- abs(Model.8.Comp[1,2] - Model.8.Comp$AIC)
```

### Model 8 Comparison
```{r model 8 comp}
Model.8.Comp <- data.frame(Model = Model.8.Comp$model, resid_df = Model.8.Comp$df.resid, round(Model.8.Comp[, c(2, 3, 4, 6)], 2))

datatable(Model.8.Comp)
```
...
### Modling results table
Table to look at result from best models
```{r full results best model, echo = FALSE, }
all.results.simp <- sjt.glm(mod1.best, mod2.best, mod3.best, mod4.best, mod5.best, mod6.best, mod7.best, mod8.best, group.pred = FALSE, depvar.labels = c("Aboveground", "Belowground", "Shoot density", "Aboveground/Belowground", "Second node distance", "Epiphyte Load", "Grazer Load", "Crab biomass"), exp.coef = FALSE, show.r2 = TRUE)

all.results.simp
```

## Environmental Data
Summary stats from environmental data from all sites.
```{r}
## Collate environmental data ##
site.envi <- cbind.data.frame(dat[,2:3], round(dat[,15:24],2), round(dat[,57:61], 2), round(dat[, 115], 1), round(dat[, 116], 2))
colnames(site.envi)[18] <- "sediment"
colnames(site.envi)[19] <- "light_attenuation"

## Export ##
write.csv(site.envi, "Eelgrass_structure_MS_results/envi_summary_stats.csv", row.names =  FALSE)
...
```{r}
ggplot(dat) +
  geom_point(aes(x = dat$sea_otter_index, y = dat$fish_SW_diverse), size = 3)
```
