...
## Global Model ##
mod1 <- dredge(lm(dat$abvgnd_mass ~ dat$date_julian + dat$sea_otter_index + log(dat$epiphmass_shootmass) + log(dat$grazermass_shootmass) + sqrt(dat$crab_mass) + log(dat$fish_preds) + dat$sed_inside_prim + dat$light_avail + dat$Ntotal_site), rank = "AICc", beta = "partial.sd", evaluate = TRUE)
...
pred.1.jday <- MuMIn:::predict.averaging(mod1.best, type = "response", backtransform = FALSE, se.fit = TRUE, full = FALSE)
