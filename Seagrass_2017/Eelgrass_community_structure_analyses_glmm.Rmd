...
## Predicted Values ##
newdata = data.frame(date_julian = 150, sea_otter_index = 0, epiphmass_shootmass = 0.04) # wrap the parameter
predict.glm(mod1.best, type = "response", newdata = newdata, se.fir = TRUE)

Minday <- min(dat$date_julian)
Julianday <- as.numeric(tran.dat$date_julian)
...
int <- mod1.1$coefficients[1]
time <- mod1.1$coefficients[2]
time2 <- mod1.1$coefficients[3]
soi <- mod1.1$coefficients[4]
soi2 <- mod1.1$coefficients[5]
epi <- mod1.1$coefficients[6]

for (d in mind:maxd){
  Julday[d] = d - 1 + Minday
  Mu = int + time*d + time2*d^2
  pred[d] = mean(exp(Mu))
  lwr[d] = as.numeric(quantile(exp(Mu), .025))
  upr[d] = as.numeric(quantile(exp(Mu), .975))
} 
...
### Best Model 6
```{r model 6 best}
mod6.best <- glm(log(dat$epiphmass_shootmass) ~ log(dat$grazermass_shootmass) + dat$sed_inside_prim + dat$light_avail)
summary(mod6.best)
par(mfrow = c(2, 2))
plot(mod6.best)
par(mfrow = c(1, 1))
...
## Predicted Values ##
predict(mod8.best, type = "response")

soi <- seq(min(dat$sea_otter_index), max(dat$sea_otter_index), length.out = 100)
...
int <- mod8.best$coefficients[1]
so <- mod8.best$coefficients[2]

for(s in unique(soi)){
  Mu = int + so*s
  Predict[s] = mean(exp(Mu))
  Lwr[s] = as.numeric(quantile(exp(Mu), .025))
  Upr[s] = as.numeric(quantile(exp(Mu), .975))
} 

dfSOI.8 <- data.frame(sea_otter_index = soi, Mean = Predict, Lwr = Lwr, Upr = Upr)
...
```{r}
ggplot(dat) +
  geom_point(aes(x =(dat$sea_otter_index), y = (dat$crab_mass)), size = 3) +
