...
mod6 <- stepAIC(glm(log(dat$epiphmass_shootmass) ~ 1), 
                scope = list(upper = ~dat$date_julian + dat$sea_otter_index + poly(dat$date_julian, 2) + poly(dat$sea_otter_index, 2) + dat$sed_inside_prim + dat$light_atten + log(dat$grazermass_shootmass), lower = ~ 1), direction = "both")
...
all.results.simp <- sjt.glm(mod1.best, mod2.best, mod3.best, mod4.best, mod5.best, mod6.best, mod7.best, mod8.best, group.pred = FALSE,
        depvar.labels = c("Aboveground", "Belowground", "Shoot density", "Aboveground/Belowground", "Second node distance", "Epiphyte Load", "Grazer Load", "Crab biomass"))
...
```{r}
ggplot(dat) +
  geom_point(aes(x = log(epiphmass_shootmass), y = log(grazermass_shootmass)), size = 3)
```
