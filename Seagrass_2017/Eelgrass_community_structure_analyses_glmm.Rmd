...
```{r model 6}
## Global Model ##
mod6 <- stepAIC(glm(log(dat$epiphmass_shootmass) ~ 1), scope = list(upper = ~dat$date_julian + dat$sea_otter_index + log(dat$grazermass_shootmass) + log(dat$abvgnd_mass) + dat$sed_inside_prim + dat$light_avail + dat$Ntotal_site  , lower = ~1), direction = "both", AICc = TRUE)
...
```{r model 6 best}
mod6.best <- glm(log(epiphmass_shootmass) ~ log(abvgnd_mass) + sea_otter_index, data = dat)
summary(mod6.best)
par(mfrow = c(2, 2))
plot(mod6.best)
par(mfrow = c(1, 1))
