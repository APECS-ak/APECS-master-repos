...
mod1 <- stepAIC(glm(log(dat$abvgnd_mass) ~ 1), 
                scope = list(upper = ~dat$date_julian + dat$sea_otter_index + poly(dat$date_julian, 2) + poly(dat$sea_otter_index, 2) + dat$sed_inside_prim + dat$light_atten, lower = ~ 1), direction = "both")
...
### Best Model 5
```{r model 5 best}
mod5.best <- glm(log(node2) ~  poly(date_julian, 2) + sea_otter_index + light_atten, data = dat)
summary(mod5.best)
par(mfrow = c(2, 2))
plot(mod5.best)
par(mfrow = c(1, 1))
