...
## Global Model ##
mod1 <- stepAIC(glm(log(dat$abvgnd_mass) ~ 1), scope = list(upper = ~dat$date_julian + poly(dat$date_julian, 2) + dat$sea_otter_index + + poly(dat$sea_otter_index, 2) + log(dat$epiphmass_shootmass) + log(dat$grazermass_shootmass) + dat$sed_inside_prim + dat$light_avail + dat$Ntotal_site, lower = ~ 1), direction = "both", AICc = TRUE)

mod1$anova
```

### Best Model 1
Minimum AIC value was greater than 4 AIC from next model. Only one model will be reported.
...
# Epiphyte Load #
newdata.1.epi <- data.frame(date_julian = rep(median(dat$date_julian), 100), sea_otter_index = rep(median(dat$sea_otter_index), 100), epiphmass_shootmass = seq(min(dat$epiphmass_shootmass), max(dat$epiphmass_shootmass), length.out = 100))
...
p1.epi <- ggplot(newdata.1.epi) +
  ...
  scale_y_continuous(lim = c(0,115), breaks = seq(0, 115, by = 20)) +
  theme(text = element_text(size = 10), legend.position = "none")
```
...
## Global Model ##
mod2 <- stepAIC(glm(log(dat$blwgnd_mass) ~ 1), scope = list(upper = ~dat$date_julian + dat$sea_otter_index + poly(dat$date_julian, 2) + poly(dat$sea_otter_index, 2) + log(dat$epiphmass_shootmass) + log(dat$grazermass_shootmass) + dat$sed_inside_prim + dat$light_avail + dat$Ntotal_site, lower = ~ 1), direction = "both", AICc = TRUE)
...
## Global Model ##
mod3 <- stepAIC(glm.nb(dat$shoot_dens_rnd ~ 1, link = "log"), scope = list(upper = ~dat$date_julian + dat$sea_otter_index + poly(dat$date_julian, 2) + poly(dat$sea_otter_index, 2) + log(dat$epiphmass_shootmass) + log(dat$grazermass_shootmass) + dat$sed_inside_prim + dat$light_avail + dat$Ntotal_site, lower = ~ 1), direction = "both", AICc = TRUE)
...
## Averging ##
mod3.1 <- glm.nb(shoot_dens_rnd ~  poly(date_julian, 2) + poly(sea_otter_index, 2) + log(epiphmass_shootmass), link = "log", data = dat)
mod3.2 <- glm.nb(shoot_dens_rnd ~  poly(date_julian, 2) + poly(sea_otter_index, 2), link = "log", data = dat)
mod3.3 <- glm.nb(shoot_dens_rnd ~  date_julian + poly(sea_otter_index, 2), link = "log", data = dat)
mod3.best <- model.avg(list(mod3.1, mod3.2, mod3.3), fit = TRUE)
...
# Epiphyte Load #
newdata.3.epi <- data.frame(date_julian = rep(median(dat$date_julian), 100), sea_otter_index = rep(median(dat$sea_otter_index), 100), epiphmass_shootmass = seq(min(dat$epiphmass_shootmass), max(dat$epiphmass_shootmass), length.out = 100))
pred.3.epi <- predict(mod3.best, se.fit = TRUE, type = "response", newdata = newdata.3.epi)
newdata.3.epi$Predict <- (pred.3.epi$fit)
newdata.3.epi$Lwr <- (pred.3.epi$fit - pred.3.epi$se.fit)
newdata.3.epi$Upr <- (pred.3.epi$fit + pred.3.epi$se.fit)
...
## Global Model ##
mod4 <- stepAIC(glm(log(dat$abvgnd_mass / dat$blwgnd_mass) ~ 1), scope = list(upper = ~dat$date_julian + dat$sea_otter_index + poly(dat$date_julian, 2) + poly(dat$sea_otter_index, 2) + log(dat$epiphmass_shootmass) + log(dat$grazermass_shootmass) + dat$sed_inside_prim + dat$light_avail + dat$Ntotal_site, lower = ~ 1), direction = "both", AICc = TRUE)
...
## Averaging ##
mod4.1 <- glm(log(abvgnd_mass / blwgnd_mass) ~  poly(date_julian, 2) + sea_otter_index + log(grazermass_shootmass), data = dat)
mod4.2 <- glm(log(abvgnd_mass / blwgnd_mass) ~  poly(date_julian, 2) + sea_otter_index, data = dat)
mod4.best <- model.avg(list(mod4.1, mod4.2), fit = TRUE)
...
## Predicted Values ##
# Julian Day #
newdata.4.jday <- data.frame(date_julian = seq(min(dat$date_julian), max(dat$date_julian)), sea_otter_index = rep(median(dat$sea_otter_index), 116), grazermass_shootmass = rep(median(dat$grazermass_shootmass), 116))
...
# Sea Otter Index #
newdata.4.soi <- data.frame(date_julian = rep(median(dat$date_julian), 100), sea_otter_index = seq(min(dat$sea_otter_index), max(dat$sea_otter_index), length.out = 100), grazermass_shootmass = rep(median(dat$grazermass_shootmass), 100)) 
...
# Grazer Load #
newdata.4.grz <- data.frame(date_julian = rep(median(dat$date_julian), 100), sea_otter_index = rep(median(dat$sea_otter_index), 100), grazermass_shootmass = seq(min(dat$epiphmass_shootmass), max(dat$epiphmass_shootmass), length.out = 100))
...
p4.grz <- ggplot(newdata.4.grz) +
  ...
  labs(x = "Grazer load", y =  "Aboveground : Belowground biomass +/- se") +
  scale_x_continuous(breaks = seq(0, 0.15, by = 0.05)) +
  scale_y_continuous(lim = c(0,6), breaks = seq(0, 6, by = 1)) +
...
## Global Model ##
mod5 <- stepAIC(glm(log(dat$node2) ~ 1), scope = list(upper = ~dat$date_julian + dat$sea_otter_index + poly(dat$date_julian, 2) + poly(dat$sea_otter_index, 2) + log(dat$epiphmass_shootmass) + log(dat$grazermass_shootmass) + dat$sed_inside_prim + dat$light_avail + dat$Ntotal_site, lower = ~ 1), direction = "both", AICc = TRUE)

mod5$anova
```

### Best Model 5
Top 3 models are within 4 AIC, therefore model average.
```{r model 5 best, echo = FALSE}
## Averaging ###
mod5.1 <- glm(log(node2) ~  poly(date_julian, 2) + light_avail + Ntotal_site, data = dat) 
mod5.2 <- glm(log(node2) ~  poly(date_julian, 2) + light_avail, data = dat)
mod5.3 <- glm(log(node2) ~  poly(date_julian, 2), data = dat)
mod5.best <- model.avg(list(mod5.1, mod5.2, mod5.3), fit = TRUE)
...
# Notal N #
newdata.5.nitro <- data.frame(date_julian = rep(median(dat$date_julian), 100), light_avail = rep(median(dat$light_avail), 100), Ntotal_site = seq(min(dat$Ntotal_site), max(dat$Ntotal_site), length.out = 100))
pred.5.nitro <- predict(mod5.best, se.fit = TRUE, type = "response", newdata = newdata.5.nitro)
newdata.5.nitro$Predict <- exp(pred.5.nitro$fit)
newdata.5.nitro$Lwr <- exp(pred.5.nitro$fit - pred.5.nitro$se.fit)
newdata.5.nitro$Upr <- exp(pred.5.nitro$fit + pred.5.nitro$se.fit)
...
p5.light <- ggplot(newdata.5.light) +
  ...
  labs(x = "Julian day", y = " Second rhizome internode length +/- se (cm)") +
  scale_y_continuous(lim = c(0,2), breaks = seq(0, 2, by = 0.5)) +
  theme(text = element_text(size = 10), axis.ticks = element_line(size = 1), legend.position = "none")

# Total N #
p5.nitro <- ggplot(newdata.5.nitro) +
  geom_line(aes(x = Ntotal_site, y = Predict), size = 1) +
  geom_ribbon(aes(x = Ntotal_site, ymin = Lwr, ymax = Upr), alpha = 0.2) +
  labs(x = "Julian day", y = " Second rhizome internode length +/- se (cm)") +
  scale_y_continuous(lim = c(0,2), breaks = seq(0, 2, by = 0.5)) +
  theme(text = element_text(size = 10), axis.ticks = element_line(size = 1), legend.position = "none")
```
