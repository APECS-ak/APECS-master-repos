...
## Models
Each model will get its own individual hypothesis and set up but will follow the same general framework. All models will be fit starting with a global model. That global model will be run through the `dredge` function. Models within 2 of lowes AICc will be reported and lowest AICc model woll be fit for formal analysis and predicted values plots. 
...
## Global Model ##
mod3 <- dredge(glm.nb(dat$shoot_dens_rnd ~ dat$date_julian + dat$date_julian_2 + dat$sea_otter_index + dat$sea_otter_index_2 + log(dat$epiphmass_shootmass) + log(dat$grazermass_shootmass) + dat$sed_inside_prim + dat$light_avail + dat$Ntotal_site + log(dat$crab_mass), link = "log"), rank = "AICc", evaluate = TRUE)
...
## Global Model ##
mod4 <- dredge(glm(log(dat$abvgnd_mass / dat$blwgnd_mass) ~ dat$date_julian + dat$date_julian_2 + dat$sea_otter_index  + dat$sea_otter_index_2 + log(dat$epiphmass_shootmass) + log(dat$grazermass_shootmass) + dat$sed_inside_prim + dat$light_avail + dat$Ntotal_site + log(dat$crab_mass)), rank = "AICc", evaluate = TRUE)
...
## Global Model ##
mod5 <- dredge(glm(log(dat$node2) ~ dat$date_julian + dat$date_julian_2 + dat$sea_otter_index  + dat$sea_otter_index_2 + log(dat$epiphmass_shootmass) + log(dat$grazermass_shootmass) + dat$sed_inside_prim + dat$light_avail + dat$Ntotal_site + log(dat$crab_mass)), rank = "AICc", evaluate = TRUE)
...
## Best Model ##
mod5.best <- glm(log(node2) ~  date_julian + date_julian_2, data = dat) 
...
Response  - log normally distributed. We hypothesise that time (as julian day), sea otter index, sediment type, light availability,  grazer load, and crab biomass may effect epiephyte load on eelgrass.

```{r model 6, echo = FALSE}
## Global Model ##
mod6 <- dredge(glm(log(dat$epiphmass_shootmass) ~ dat$date_julian + dat$sea_otter_index + log(dat$grazermass_shootmass) + dat$sed_inside_prim + dat$light_avail + dat$Ntotal_site + log(dat$crab_mass)), rank = "AICc", evaluate = TRUE)
...
## Best Model ##
mod6.best <- glm(log(epiphmass_shootmass) ~ sed_inside_prim + log(crab_mass) + log(grazermass_shootmass) , data = dat) 
...
newdata.6.grz <- data.frame(grazermass_shootmass = seq(min(dat$grazermass_shootmass), max(dat$grazermass_shootmass), length.out = 100), 
                            sed_inside_prim = rep(median(dat$sed_inside_prim), 100), 
                            crab_mass = rep(median(dat$crab_mass), 100)) 
...
# Crab Biomass #
newdata.6.crb <- data.frame(grazermass_shootmass = rep(median(dat$grazermass_shootmass), 100), 
                             sed_inside_prim = rep(median(dat$sed_inside_prim), 100), 
                             crab_mass = seq(min(dat$crab_mass), max(dat$crab_mass), length.out = 100))
pred.6.crb <- predict(mod6.best, se.fit = TRUE, type = "response", newdata = newdata.6.crb)
newdata.6.crb$Predict <- exp(pred.6.crb$fit)
newdata.6.crb$Lwr <- exp(pred.6.crb$fit - pred.6.crb$se.fit)
newdata.6.crb$Upr <- exp(pred.6.crb$fit + pred.6.crb$se.fit)
...
# Crab Biomass#
p6.crb <- ggplot(newdata.6.crb) +
  geom_line(aes(x = crab_mass, y = Predict)) +
  geom_ribbon(aes(x = crab_mass, ymin = Lwr, ymax = Upr), alpha = 0.2) +
  labs(x = "Crab Biomass (g)", y = "Epiphyte Load +/- se (g/g)") +
...
### Model 7 - Grazer load
Response - Log normally distributed. We hypothesise that time (as julian day), sea otter index, epiphyte load, log crab biomass, log fish biomass, light availability, total N, and sediment type may effect grazer biomass.

```{r model 7, echo = FALSE}
## Global Model ##
mod7 <- dredge(glm(log(dat$grazermass_shootmass) ~ dat$date_julian + dat$sea_otter_index + log(dat$epiphmass_shootmass) + dat$sed_inside_prim + dat$light_avail + dat$Ntotal_site + log(dat$crab_mass) + log(dat$fish_mass)) , rank = "AICc", evaluate = TRUE)
...
### Model 8 - Crabs
Response - Log normally distributed. We hypothesise that time (as julian day), sea otter index, grazer biomass, fish biomass, aboveground biomass, and shoot density, and epiphyte mass may affect crab biomass.

```{r model 8, echo = FALSE}
## Global Model ##
mod8 <- dredge(glm(log(dat$crab_mass) ~ dat$date_julian + dat$sea_otter_index + log(dat$grazermass_shootmass) + log(dat$fish_mass) + log(dat$abvgnd_mass) + dat$shoot_dens + log(dat$epiphmass_shootmass)), rank  ="AICc", evaluate = TRUE)
...
```{r}
all.results.exp <- sjt.glm(mod1.best, mod2.best, mod3.best, mod4.best, mod5.best, mod6.best, mod7.best, mod8.best, group.pred = FALSE, exp.coef = TRUE, show.r2 = TRUE, show.aic = TRUE, digits.est = 4, digits.p = 4, digits.ci = 4)
