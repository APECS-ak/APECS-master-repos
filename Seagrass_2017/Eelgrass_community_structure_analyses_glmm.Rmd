...
## Global Model ##
mod1 <- stepAIC(glm(log(dat$abvgnd_mass) ~ 1), scope = list(upper = ~dat$date_julian + dat$date_julian_2  + dat$sea_otter_index + dat$sea_otter_index_2 + log(dat$epiphmass_shootmass) + log(dat$grazermass_shootmass) + dat$sed_inside_prim + dat$light_avail + dat$Ntotal_site, lower = ~ 1), direction = "both", AICc = TRUE)

mod1$anova
```

### Best Model 1
Two models were within 4 AIC from next model. Model averaging over top two models
...
out.1 <- tidy(mod1.best)
sumout.1 <- data.frame(Responce = "Aboveground",
                       Term = out.1$term, 
                       Estimate = round(exp(out.1$estimate), 2), 
                       CIlow = round(exp(confint(mod1.best))[,1], 2), 
                       CIhigh = round(exp(confint(mod1.best))[,2], 2),
                       CI = do.call(paste, c(data.frame(round(exp(confint(mod1.best)),2))[,1:2], sep = "-")),
                       p = round(out.1$p.value, 4))
