...
## Models
Each model will get its own individual hypothesis and set up but will follow the same general framework. All models will be fit starting with a global model. That global model will be run through the `dredge` function. Interaction terms will only be included if deems appropriate for the given response / hypothesized mechanism. 

### Model 1 - Aboveground biomass
Response  - log normally distributed. We hypothesise that time (as julian day), sea otter index, epiphyte load, grazer load, sediment type, light availability, total N, and crab biomass may effect aboveground biomass of eelgrass.

```{r model 1, echo = FLASE}
## Global Model ##
mod1 <- dredge(glm(log(dat$abvgnd_mass) ~ dat$date_julian + dat$date_julian_2 + dat$sea_otter_index + dat$sea_otter_index_2 + log(dat$epiphmass_shootmass) + log(dat$grazermass_shootmass) + dat$sed_inside_prim + dat$light_avail + dat$Ntotal_site + log(dat$crab_mass)), rank = "AICc", evaluate = TRUE)

# AICc comp #
mod1$delAICprop <- mod1$delta / mod1$AICc
mod1.tbl <- filter(mod1, delAICprop <= 0.05)
...
## Best Model ##
mod1.best <- glm(log(abvgnd_mass) ~ date_julian + date_julian_2 + sea_otter_index + log(epiphmass_shootmass), data = dat)
...
## Global Model ##
mod2 <- dredge(glm(log(dat$blwgnd_mass) ~ dat$date_julian + dat$date_julian_2 + dat$sea_otter_index + dat$sea_otter_index_2 + log(dat$epiphmass_shootmass) + log(dat$grazermass_shootmass) + dat$sed_inside_prim + dat$light_avail + dat$Ntotal_site + log(dat$crab_mass)), rank = "AICc", evaluate = TRUE)

# AICc comp #
mod2$delAICprop <- mod2$delta / mod2$AICc
mod2.tbl <- filter(mod2, delAICprop <= 0.05)
