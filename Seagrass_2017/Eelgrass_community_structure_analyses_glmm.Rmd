...
## Models
Each model will get its own individual hypothesis and set up but will follow the same general framework. All models will be fit starting with a global model. That glocal model will be run through an forward/backward model selection procedure with AICc disrimination. If top candidate models are <= 4 AICc values they will be average using a model avergin procedure.

### Model 1 - Aboveground biomass
Response  - log normally distributed. We hypothesise that time (as julian day), sea otter index, epiphyte load, grazer load, sediment type, light availability, and total N may effect aboveground biomass of eelgrass.

```{r model 1, echo = FLASE}
## Global Model ##
mod1 <- stepAIC(glm(log(dat$abvgnd_mass) ~ 1), scope = list(upper = ~dat$date_julian + dat$date_julian_2  + dat$sea_otter_index + dat$sea_otter_index_2 + log(dat$epiphmass_shootmass) + log(dat$grazermass_shootmass) + dat$sed_inside_prim + dat$light_avail + dat$Ntotal_site, lower = ~ 1), direction = "both")

mod1t <- dredge(glm(log(dat$abvgnd_mass) ~ dat$date_julian + dat$date_julian_2  + dat$sea_otter_index + dat$sea_otter_index_2 + log(dat$epiphmass_shootmass) + log(dat$grazermass_shootmass) + dat$sed_inside_prim + dat$light_avail + dat$Ntotal_site), rank = "AICc", evaluate = TRUE)

# AICc comp #
mod1.tbl <- cbind.data.frame(mod1t$df, mod1t$logLik, mod1t$AICc, mod1t$delta, mod1t$weight)
colnames(mod1.tbl) <- c("df", "logLik", "AICc", "delta", "weight")
mod1.tbl$delAICprop <- mod1.tbl$delta / mod1.tbl$AICc

datatable(mod1.tbl)
```

### Best Model 1
Two models were within 5% AIC from next model. Model averaging over top two models
```{r model 1 best, echo = FALSE}
## Best Model ##
mod1.best <- glm(log(abvgnd_mass) ~ date_julian + date_julian_2 + sea_otter_index + sea_otter_index_2 + log(epiphmass_shootmass), data = dat)
...
newdata.1.jday <- data.frame(date_julian = seq(min(dat$date_julian), max(dat$date_julian)), 
                             ...
                             sea_otter_index_2 = rep(median(dat$sea_otter_index_2), 116), 
...
newdata.1.soi <- data.frame(date_julian = rep(median(dat$date_julian), 100),
                            ...
                            sea_otter_index_2 = seq(min(dat$sea_otter_index_2), max(dat$sea_otter_index_2), length.out = 100),
...
# Epiphyte Load #
newdata.1.epi <- data.frame(date_julian = rep(median(dat$date_julian), 100),
                            date_julian_2 = rep(median(dat$date_julian_2), 100), 
                            sea_otter_index = rep(median(dat$sea_otter_index), 100), 
                            sea_otter_index_2 = rep(median(dat$sea_otter_index_2), 100),
                            epiphmass_shootmass = seq(min(dat$epiphmass_shootmass), max(dat$epiphmass_shootmass), length.out = 100))
pred.1.epi <- predict(mod1.best, se.fit = TRUE, type = "response", newdata = newdata.1.epi)
newdata.1.epi$Predict <- exp(pred.1.epi$fit)
newdata.1.epi$Lwr <- exp(pred.1.epi$fit - pred.1.epi$se.fit)
newdata.1.epi$Upr <- exp(pred.1.epi$fit + pred.1.epi$se.fit)
...
## Global Model ##
mod2 <- stepAIC(glm(log(dat$blwgnd_mass) ~ 1), scope = list(upper = ~dat$date_julian + dat$date_julian_2 + dat$sea_otter_index  + dat$sea_otter_index_2 + log(dat$epiphmass_shootmass) + log(dat$grazermass_shootmass) + dat$sed_inside_prim + dat$light_avail + dat$Ntotal_site, lower = ~ 1), direction = "both", AICc = TRUE)

# AIC comp #
mod2.tbl <- data.frame(mod2$anova)
mod2.tbl <- mod2.tbl[order(mod2.tbl$AIC),]
mod2.tbl$delAIC <- NA
for(r in 2:nrow(mod2.tbl)){
  mod2.tbl$delAIC[r] <- round((mod2.tbl[r, 6] - mod2.tbl[1, 6]), 2) 
}

mod2.tbl$delAICprop <- round((mod2.tbl$delAIC / mod2.tbl$AIC), 2)

datatable(mod2.tbl)
```

### Best Model 2
Top 2 models are with in 4 AIC so models will be averaged.
...
# Epiphyte Load #
newdata.2.epi <- data.frame(date_julian = rep(median(dat$date_julian), 100), 
                            epiphmass_shootmass = seq(min(dat$epiphmass_shootmass), max(dat$epiphmass_shootmass), length.out = 100))
...
## Global Model ##
mod3 <- stepAIC(glm.nb(dat$shoot_dens_rnd ~ 1, link = "log"), scope = list(upper = ~dat$date_julian + dat$date_julian_2 + dat$sea_otter_index  + dat$sea_otter_index_2 + log(dat$epiphmass_shootmass) + log(dat$grazermass_shootmass) + dat$sed_inside_prim + dat$light_avail + dat$Ntotal_site, lower = ~ 1), direction = "both", AICc = TRUE)

# AIC comp #
mod3.tbl <- data.frame(mod3$anova)
mod3.tbl <- mod3.tbl[order(mod3.tbl$AIC),]
mod3.tbl$delAIC <- NA
for(r in 2:nrow(mod3.tbl)){
  mod3.tbl$delAIC[r] <- round((mod3.tbl[r, 6] - mod3.tbl[1, 6]), 2) 
}

mod3.tbl$delAICprop <- round((mod3.tbl$delAIC / mod3.tbl$AIC), 2)

datatable(mod3.tbl)
```

### Best Model 3
Top 3 models are within 4 AIC of minimun AIC so model everaging procedure will be employed

```{r model 3 best, echo = FALSE}
## Best Model ##
mod3.best <- glm.nb(shoot_dens_rnd ~ date_julian_2 + sea_otter_index_2, link = "log", data = dat)
...
# Sea Otter Index #
newdata.3.soi <- data.frame(date_julian_2 = rep(median(dat$date_julian), 100), 
                            sea_otter_index = seq(min(dat$sea_otter_index), max(dat$sea_otter_index), length.out = 100),
                            sea_otter_index_2 = seq(min(dat$sea_otter_index_2), max(dat$sea_otter_index_2), length.out = 100)) 
...
## Global Model ##
mod4 <- stepAIC(glm(log(dat$abvgnd_mass / dat$blwgnd_mass) ~ 1), scope = list(upper = ~dat$date_julian + dat$date_julian_2 + dat$sea_otter_index  + dat$sea_otter_index_2 + log(dat$epiphmass_shootmass) + log(dat$grazermass_shootmass) + dat$sed_inside_prim + dat$light_avail + dat$Ntotal_site, lower = ~ 1), direction = "both", AICc = TRUE)

# AIC comp #
mod4.tbl <- data.frame(mod4$anova)
mod4.tbl <- mod4.tbl[order(mod4.tbl$AIC),]
mod4.tbl$delAIC <- NA
for(r in 2:nrow(mod4.tbl)){
  mod4.tbl$delAIC[r] <- round((mod4.tbl[r, 6] - mod4.tbl[1, 6]), 2) 
}

mod4.tbl$delAICprop <- round((mod4.tbl$delAIC / mod4.tbl$AIC), 2)

datatable(mod4.tbl)
```

### Best Model 4
Top 4 models are within 4 AIC, therefore model average.
```{r model 4 best, echo = FALSE}
## Best Model ##
mod4.best <- glm(log(abvgnd_mass / blwgnd_mass) ~  date_julian + date_julian_2 + sea_otter_index + log(grazermass_shootmass), data = dat)
...
# Grazer Load #
newdata.4.grz <- data.frame(date_julian = rep(median(dat$date_julian), 100),
                            date_julian_2 = rep(median(dat$date_julian_2), 100),
                            sea_otter_index = rep(median(dat$sea_otter_index), 100), 
                            grazermass_shootmass = seq(min(dat$grazermass_shootmass), max(dat$grazermass_shootmass), length.out = 100),
                            sed_inside_prim = rep(median(dat$sed_inside_prim), 100)) 
pred.4.grz <- predict(mod4.best, se.fit = TRUE, type = "response", newdata = newdata.4.grz)
newdata.4.grz$Predict <- exp(pred.4.grz$fit)
newdata.4.grz$Lwr <- exp(pred.4.grz$fit - pred.4.grz$se.fit)
newdata.4.grz$Upr <- exp(pred.4.grz$fit + pred.4.grz$se.fit)
...
## Global Model ##
mod5 <- stepAIC(glm(log(dat$node2) ~ 1), scope = list(upper = ~dat$date_julian + dat$date_julian_2 + dat$sea_otter_index  + dat$sea_otter_index_2 + log(dat$epiphmass_shootmass) + log(dat$grazermass_shootmass) + dat$sed_inside_prim + dat$light_avail + dat$Ntotal_site, lower = ~ 1), direction = "both", AICc = TRUE)

# AIC comp #
mod5.tbl <- data.frame(mod5$anova)
mod5.tbl <- mod5.tbl[order(mod5.tbl$AIC),]
mod5.tbl$delAIC <- NA
for(r in 2:nrow(mod5.tbl)){
  mod5.tbl$delAIC[r] <- round((mod5.tbl[r, 6] - mod5.tbl[1, 6]), 2) 
}

mod5.tbl$delAICprop <- round((mod5.tbl$delAIC / mod5.tbl$AIC), 2)
...
## Best Model ##
mod5.best <- glm(log(node2) ~  date_julian + date_julian_2 + sea_otter_index + log(grazermass_shootmass), data = dat) 
...
# Sea otter index #
newdata.5.soi <- data.frame(date_julian = rep(median(dat$date_julian), 100),
                            date_julian_2 = rep(median(dat$date_julian_2), 100),
                            sea_otter_index = seq(min(dat$sea_otter_index), max(dat$sea_otter_index), length.out = 100), 
                            grazermass_shootmass = rep(median(dat$grazermass_shootmass), 100))
pred.5.soi <- predict(mod5.best, se.fit = TRUE, type = "response", newdata = newdata.5.soi)
newdata.5.soi$Predict <- exp(pred.5.soi$fit)
newdata.5.soi$Lwr <- exp(pred.5.soi$fit - pred.5.soi$se.fit)
newdata.5.soi$Upr <- exp(pred.5.soi$fit + pred.5.soi$se.fit)

# Grazer Load #
newdata.5.grz <- data.frame(date_julian = rep(median(dat$date_julian), 100),
                            date_julian_2 = rep(median(dat$date_julian_2), 100),
                            sea_otter_index = rep(median(dat$sea_otter_index), 100), 
                            grazermass_shootmass = seq(min(dat$grazermass_shootmass), max(dat$grazermass_shootmass), length.out = 100))
pred.5.grz <- predict(mod5.best, se.fit = TRUE, type = "response", newdata = newdata.5.grz)
newdata.5.grz$Predict <- exp(pred.5.grz$fit)
newdata.5.grz$Lwr <- exp(pred.5.grz$fit - pred.5.grz$se.fit)
newdata.5.grz$Upr <- exp(pred.5.grz$fit + pred.5.grz$se.fit)
...
# Sea otter index #
p5.soi <- ggplot(newdata.5.soi) +
  geom_line(aes(x = sea_otter_index, y = Predict)) +
  geom_ribbon(aes(x = sea_otter_index, ymin = Lwr, ymax = Upr), alpha = 0.2) +
  labs(x = "Sea otter index", y = " Second rhizome \n internode +/- se (cm)") +
  scale_x_continuous(breaks = seq(-0.8, 1.2, by = 0.4))  +
  scale_y_continuous(lim = c(0,2), breaks = seq(0, 2, by = 0.5)) +
  theme(text = element_text(size = 10), legend.position = "none")

# Grazer load #
p5.grz <- ggplot(newdata.5.grz) +
  geom_line(aes(x = grazermass_shootmass, y = Predict)) +
  geom_ribbon(aes(x = grazermass_shootmass, ymin = Lwr, ymax = Upr), alpha = 0.2) +
  labs(x = "Grazer Load", y = " Second rhizome \n internode +/- se (cm)") +
  scale_x_continuous(breaks = seq(0, 0.15, by = 0.05)) +
  scale_y_continuous(lim = c(0,2), breaks = seq(0, 2, by = 0.5)) +
  theme(text = element_text(size = 10), legend.position = "none")
```
...
Response  - log normally distributed. We hypothesise that time (as julian day), sea otter index, sediment type, light availability, and grazer load may effect epiephyte load on eelgrass.

```{r model 6, echo = FALSE}
## Global Model ##
mod6 <- stepAIC(glm(log(dat$epiphmass_shootmass) ~ 1), scope = list(upper = ~dat$date_julian + dat$sea_otter_index + log(dat$grazermass_shootmass) + dat$sed_inside_prim + dat$light_avail + dat$Ntotal_site  , lower = ~1), direction = "both", AICc = TRUE)

# AIC comp #
mod6.tbl <- data.frame(mod6$anova)
mod6.tbl <- mod6.tbl[order(mod6.tbl$AIC),]
mod6.tbl$delAIC <- NA
for(r in 2:nrow(mod6.tbl)){
  mod6.tbl$delAIC[r] <- round((mod6.tbl[r, 6] - mod6.tbl[1, 6]), 2) 
}

mod6.tbl$delAICprop <- round((mod6.tbl$delAIC / mod6.tbl$AIC), 2)
...
## Best Model ##
mod6.best <- glm(log(epiphmass_shootmass) ~  log(grazermass_shootmass) + light_avail + sed_inside_prim, data = dat) 
...
# Light Availability #
newdata.6.lite <- data.frame(grazermass_shootmass = rep(median(dat$grazermass_shootmass), 100), sed_inside_prim = rep(median(dat$sed_inside_prim), 100), light_avail = seq(min(dat$light_avail), max(dat$light_avail), length.out = 100))
pred.6.lite <- predict(mod6.best, se.fit = TRUE, type = "response", newdata = newdata.6.lite)
newdata.6.lite$Predict <- exp(pred.6.lite$fit)
newdata.6.lite$Lwr <- exp(pred.6.lite$fit - pred.6.lite$se.fit)
newdata.6.lite$Upr <- exp(pred.6.lite$fit + pred.6.lite$se.fit)
...
p6.grz <- ggplot(newdata.6.grz) +
  ...
  theme(text = element_text(size = 10), axis.ticks = element_line(size = 1), legend.position = "none")

# Light Availability #
p6.lite <- ggplot(newdata.6.lite) +
  geom_line(aes(x = light_avail, y = Predict), size = 1) +
  geom_ribbon(aes(x = light_avail, ymin = Lwr, ymax = Upr), alpha = 0.2) +
  labs(x = "Light Availability", y = "Epiphyte Load +/- se (g/g)") +
...
### Model 7 - Grazer load
Response - Log normally distributed. We hypothesise that time (as julian day), sea otter index, epiphyte load, log crab biomass, log fish biomass, log above ground biomass, and shoot density may affect grazer load on eelgrass.

```{r model 7, echo = FALSE}
## Global Model ##
mod7 <- stepAIC(glm(log(dat$grazermass_shootmass) ~ 1), scope = list(upper = ~dat$date_julian + dat$sea_otter_index + log(dat$epiphmass_shootmass) + log(dat$crab_mass) + log(dat$fish_mass) + log(dat$abvgnd_mass) + dat$shoot_dens, lower = ~ 1), direction = "both", AICc = TRUE)

# AIC comp #
mod7.tbl <- data.frame(mod7$anova)
mod7.tbl <- mod7.tbl[order(mod7.tbl$AIC),]
mod7.tbl$delAIC <- NA
for(r in 2:nrow(mod7.tbl)){
  mod7.tbl$delAIC[r] <- round((mod7.tbl[r, 6] - mod7.tbl[1, 6]), 2) 
}

mod7.tbl$delAICprop <- round((mod7.tbl$delAIC / mod7.tbl$AIC), 2)

datatable(mod7.tbl)
```

### Best Model 7
Model with epiphyte load is 3.87 AIC away from ~1 model. Will report model with epiphyte load but should note that it it poor fit, probably does not mean much. 
...
### Model 8 - Crabs
Response - Log normally distributed. We hypothesise that time (as julian day), sea otter index, grazer biomass, fish biomass, aboveground biomass, and shoot density, may affect grazer load on eelgrass.

```{r model 8, echo = FALSE}
## Global Model ##
mod8 <- stepAIC(glm(log(dat$crab_mass) ~ 1), scope = list(upper = ~dat$date_julian + dat$sea_otter_index + log(dat$grazermass_shootmass) + log(dat$fish_mass) + log(dat$abvgnd_mass) + dat$shoot_dens, lower = ~1), direction = "both", AICc = TRUE)

# AIC comp #
mod8.tbl <- data.frame(mod8$anova)
mod8.tbl <- mod8.tbl[order(mod8.tbl$AIC),]
mod8.tbl$delAIC <- NA
for(r in 2:nrow(mod8.tbl)){
  mod8.tbl$delAIC[r] <- round((mod8.tbl[r, 6] - mod8.tbl[1, 6]), 2) 
}

mod8.tbl$delAICprop <- round((mod8.tbl$delAIC / mod8.tbl$AIC), 2)

datatable(mod8.tbl)
```

### Best Model 8
No model within 4 AIC. 
...
```{r multivariate counts, echo = FALSE}
...
```{r multivariate mass, echo = FALSE}
## Dissimilarity matirx ##
...
ggplot(fishmass.fam) +
  geom_col(aes(x = family, y = log(mean_mass + 1))) +
  geom_errorbar(aes(x = family, ymin = log(mean_mass + 1) - log(se_mass + 1), ymax = log(mean_mass + 1) + log(se_mass + 1), width = 0), size = 1) +
...
More pretty table
```{r}
all.results <- sjt.glm(mod1.best, mod2.best, mod3.best, mod4.best, mod5.best, mod6.best, mod7.best, mod8.best, group.pred = FALSE, depvar.labels = c("Aboveground", "Belowground", "Shoot density", "Aboveground/Belowground", "Second internode distance", "Epiphyte Load", "Grazer Load", "Crab biomass"), pred.labels = c("Julian Day", "Julian Day^2", "Sea otter index", "Sea otter index ^2", "log Epiphyte load", "Julian Day", "Sea otter index", "log Grazer load", "Light availability", "Total surface nitrogen", "log Aboveground mass"), exp.coef = FALSE, show.r2 = TRUE, show.aicc = TRUE, digits.est = 4, digits.p = 4, digits.ci = 4)
...
```{r}
all.results.exp <- sjt.glm(mod1.best, mod2.best, mod3.best, mod4.best, mod5.best, mod6.best, mod7.best, mod8.best, group.pred = FALSE, depvar.labels = c("Aboveground", "Belowground", "Shoot density", "Aboveground/Belowground", "Second internode distance", "Epiphyte Load", "Grazer Load", "Crab biomass"), pred.labels = c("Julian Day", "Julian Day^2", "Sea otter index", "Sea otter index ^2", "log Epiphyte load", "Julian Day", "Sea otter index", "Light availability", "Total surface nitrogen", "log Grazer load", "Sediment type"), exp.coef = TRUE, show.r2 = TRUE, show.aic = TRUE)
...
```{r}
theme_set(theme_cowplot(font_size = 10))
plot_grid(p1.epi, p2.epi, p3.epi, p7.epi, nrow = 4, ncol = 1, align = "v", labels = c("auto"), label_size = 10, hjust = 0.05)
```
...
```{r}
pa <- ggplot(dat) +
  geom_point(aes(y = grazermass_shootmass, x = epiphmass_shootmass), size = 3)
...
```{r}
ggplot(dat, aes(reorder(site, sea_otter_index), sea_otter_index, col = sea_otter_index)) +
  geom_point(size = 4) +
  scale_color_gradient(low = "blue", high = "red") +
  labs(x = "Site", y = "Sea otter impact Index", col = "Sea otter impact Index") +
  theme(text = element_text(size = 10), axis.ticks = element_line(size = 2)) +
  theme(axis.text.x = element_text(angle = 90))
```

![Alt](E:/wraymond2/My Documents/Graduate School/Eelgrass/Eelgrass community stucture/AbvBlw_pred_plots.pdf)
