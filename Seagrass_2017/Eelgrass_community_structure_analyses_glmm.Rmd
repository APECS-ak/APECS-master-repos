...
```{r model 1}
## Global Model ##
mod1 <- stepAIC(glm(log(dat$abvgnd_mass) ~ 1), scope = list(upper = ~dat$date_julian + dat$sea_otter_index + poly(dat$date_julian, 2) + poly(dat$sea_otter_index, 2) + log(dat$epiphmass_shootmass) + log(dat$grazermass_shootmass) + dat$sed_inside_prim + dat$light_avail + dat$Ntotal_site, lower = ~ 1), direction = "both",AICc = TRUE)

mod1$anova
```

### Best Model 1
Minimum AIC value was greater tha 4 AIC from next model. Only one model will be reported.
```{r model 1 best}
...
pred.2.jday <- predict(mod2.best, se.fit = TRUE, type = "response", newdata = newdata.2.jday, full = FALSE)
...
p3.epi <- ggplot(newdata.3.epi) +
  ...
  scale_x_continuous(breaks = seq(0, 0.15, by = 0.05)) +
  scale_y_continuous(lim = c(0,350), breaks = seq(0, 350, by = 50)) +
  theme(text = element_text(size = 12), axis.ticks = element_line(size = 1), legend.position = "none")
```
...
## Predicted Values ##
# Aboveground biomass #
newdata.6.abv <- data.frame(abvgnd_mass = seq(min(dat$abvgnd_mass), max(dat$abvgnd_mass), length.out = 100), sea_otter_index = rep(median(dat$sea_otter_index), 100))
pred.6.abv <- predict(mod6.best, se.fit = TRUE, type = "response", newdata = newdata.6.abv)
newdata.6.abv$Predict <- exp(pred.6.abv$fit)
newdata.6.abv$Lwr <- exp(pred.6.abv$fit - pred.6.abv$se.fit)
newdata.6.abv$Upr <- exp(pred.6.abv$fit + pred.6.abv$se.fit)

# Sea Otter Index #
newdata.6.soi <- data.frame(abvgnd_mass = rep(median(dat$abvgnd_mass), 100), sea_otter_index = seq(min(dat$sea_otter_index), max(dat$sea_otter_index), length.out = 100))
pred.6.soi <- predict(mod6.best, se.fit = TRUE, type = "response", newdata = newdata.6.soi)
newdata.6.soi$Predict <- exp(pred.6.soi$fit)
newdata.6.soi$Lwr <- exp(pred.6.soi$fit - pred.6.soi$se.fit)
newdata.6.soi$Upr <- exp(pred.6.soi$fit + pred.6.soi$se.fit)

## Plots ##
# Grazer load #
p6.abv <- ggplot(newdata.6.abv) +
  geom_line(aes(x = abvgnd_mass, y = Predict), size = 1) +
  geom_ribbon(aes(x = abvgnd_mass, ymin = Lwr, ymax = Upr), alpha = 0.2) +
  labs(x = "Aboveground biomass", y = "Epiphyte Load +/- se (g/g)") +
  scale_x_continuous(breaks = seq(0, 160, by = 40)) +
  scale_y_continuous(lim = c(0,0.3), breaks = seq(0, 0.3, by = 0.1)) +
  theme(text = element_text(size = 12), axis.ticks = element_line(size = 1), legend.position = "none")

# Sea Otter Index #
p6.soi <- ggplot(newdata.6.soi) +
  geom_line(aes(x = sea_otter_index, y = Predict), size = 1) +
  geom_ribbon(aes(x = sea_otter_index, ymin = Lwr, ymax = Upr), alpha = 0.2) +
  labs(x = "Sea otter index", y = "Epiphyte Load +/- se (g/g)") +
  scale_x_continuous(breaks = seq(-0.8, 1.2, by = 0.4)) +
...
```{r}
ggplot(dat) +
  geom_point(aes(y = log(abvgnd_mass), x = log(epi_mass)), size = 3)
