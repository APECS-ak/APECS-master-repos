...
## Global Model ##
mod1 <- dredge(glm(dat$abvgnd_mass ~ dat$date_julian + dat$sea_otter_index + log(dat$epiphmass_shootmass) + log(dat$grazermass_shootmass) + sqrt(dat$crab_mass) + log(dat$fish_preds) + dat$sed_inside_prim + dat$light_avail + dat$Ntotal_site), rank = "AICc", evaluate = TRUE)
...
## Best Model ##
mod1.best <- model.avg(mod1, subset = delta < 2, fit = TRUE)
mod1.res <- summary(mod1.best)
mod1.out <- mod1.res$coefmat.subset
predict(mod1.best, full = F, type = "response")
```
...
## Predicted Values ##
# Julian Day #
newdata.1.jday <- data.frame(date_julian = seq(min(dat$date_julian), max(dat$date_julian), length.out = 21), 
                             sea_otter_index = rep(median(dat$sea_otter_index), 21),
                             sed_inside_prim = rep(median(dat$sed_inside_prim), 21))
newdata.1.jday$Predict <- mod1.out[1,1] + (mod1.out[2,1] * newdata.1.jday$date_julian) + (mod1.out[3,1] * newdata.1.jday$sea_otter_index) + (mod1.out[4,1] * newdata.1.jday$sed_inside_prim)
...
## Global Model ##
mod2 <- dredge(glm(log(dat$blwgnd_mass) ~ dat$date_julian + dat$sea_otter_index + log(dat$epiphmass_shootmass) + log(dat$grazermass_shootmass) + sqrt(dat$crab_mass) + log(dat$fish_preds) + dat$sed_inside_prim + dat$light_avail + dat$Ntotal_site), rank = "AICc", beta = "partial.sd", evaluate = TRUE)
...
## Best Model ##
mod2.best <- model.avg(mod2, subset = delta < 2, fit = TRUE)
mod2.res <- summary(mod2.best)
mod2.out <- mod2.res$coefmat.subset
```
