...
ggplot(newdata.8) +
  ...
  labs(x = "Sea otter index", y = "Predicted Crab Biomass (g)") +
  scale_y_continuous(breaks = seq(0, 150, by = 20)) +
  theme(text = element_text(size = 12), axis.ticks = element_line(size = 1), legend.position = "none")
```
...
```{r}
ggplot(dat) +
  geom_point(aes(x =(dat$date_julian), y = (dat$abvgnd_mass)), size = 3) +
