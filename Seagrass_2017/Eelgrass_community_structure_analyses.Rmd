...
params <- c("sigS","beta0","beta1","eps","loglik") 
...
# Calculate summary stats
reps <- dim(post)[1]
sumout <- add.summary(out3e, c("sig", "Dispers","beta"))
sumstats <- sumout$summaries
vn <- row.names(sumstats)

# Diagnostic plots
plot(out3e, c('trace', 'histogram'), vars=c("sig", "Dispers","beta"))
...
for (d in mind:maxd){
  Julday[d] = d - 1 + Minday
  ldayMu = bt0 + bt1*d + bt2*d +bt2b*d^2
  Predict[d] = mean(exp(Mu + sig^2/2))
  Lwr[d] = as.numeric(quantile(exp(Mu + sig^2/2), .025))
  Upr[d] = as.numeric(quantile(exp(Mu + sig^2/2), .975))
} 
