...
EFA.sum <- simp.sum %>% 
  filter(FA %in% EFA) %>%
  filter(taxa != "Particulate organic matter") %>% 
  group_by(taxa) %>%
...
LCEFA.sum <- simp.sum %>% 
  ...
  summarise(sp_group = unique(sp_group),
            sum_EFA_contrib = sum(average_scaled)) %>% 
  arrange(sp_group, sum_EFA_contrib) %>% 
  group_by(sp_group) %>% 
  mutate(rank = rank(-sum_EFA_contrib, ties.method = "first"))

LCEFA.sum$base <- ifelse(LCEFA.sum$sp_group == "PC", 6, NA)
LCEFA.sum$base <- ifelse(LCEFA.sum$sp_group == "SC", 10, LCEFA.sum$base)
LCEFA.sum$base <- ifelse(LCEFA.sum$sp_group == "PP", 0, LCEFA.sum$base)

LCEFA.sum$rank <- LCEFA.sum$rank + LCEFA.sum$base

LCEFA.sum[7, 1] <- "Epiphytes"
LCEFA.sum[4, 1] <- "Pentidotea"

p.LCEFA <- ggplot(LCEFA.sum) +
  geom_col(aes(x = reorder(taxa, rank), y = sum_EFA_contrib, fill = sp_group)) +
...
EPA.sum$base <- ifelse(EPA.sum$sp_group == "PC", 6, NA)
EPA.sum$base <- ifelse(EPA.sum$sp_group == "SC", 10, EPA.sum$base)
EPA.sum$base <- ifelse(EPA.sum$sp_group == "PP", 0, EPA.sum$base)

EPA.sum$rank <- EPA.sum$rank + EPA.sum$base

EPA.sum[9, 1] <- "Epiphytes"
EPA.sum[3, 1] <- "Pentidotea"
...
DHA.sum$base <- ifelse(DHA.sum$sp_group == "PC", 6, NA)
DHA.sum$base <- ifelse(DHA.sum$sp_group == "SC", 10, DHA.sum$base)
DHA.sum$base <- ifelse(DHA.sum$sp_group == "PP", 0, DHA.sum$base)

DHA.sum$rank <- DHA.sum$rank + DHA.sum$base

DHA.sum[9, 1] <- "Epiphytes"
DHA.sum[2, 1] <- "Pentidotea"
...
ALLEFA.sum$base <- ifelse(ALLEFA.sum$sp_group == "PC", 6, NA)
ALLEFA.sum$base <- ifelse(ALLEFA.sum$sp_group == "SC", 10, ALLEFA.sum$base)
ALLEFA.sum$base <- ifelse(ALLEFA.sum$sp_group == "PP", 0, ALLEFA.sum$base)

ALLEFA.sum$rank <- ALLEFA.sum$rank + ALLEFA.sum$base

ALLEFA.sum[5, 1] <- "Epiphytes"
ALLEFA.sum[4, 1] <- "Pentidotea"
...
fu.nmds <- ggplot(fa_fu_mds_dat) +
  ...
  geom_segment(data = vec.fu.dat.sig, aes(x = 0, y = 0, xend = MDS1_sc, yend = MDS2_sc), arrow = arrow(length = unit(0.07, "inches")), colour = "gray30", alpha = 1) +
...
pm.nmds <- ggplot(fa_pm_mds_dat) +
  geom_point(aes(x = NMDS1, y = NMDS2, color = Sea_otter_region), size = 2) +
  scale_color_manual(values = so.pal, "Sea otter treatment") +
  geom_segment(data = vec.pm.dat.sig, aes(x = 0, y = 0, xend = MDS1_sc, yend = MDS2_sc), arrow = arrow(length = unit(0.07, "inches")), colour = "gray30", alpha = 1) +
...
eg.nmds <- ggplot(fa_eg_mds_dat) +
  geom_point(aes(x = NMDS1, y = NMDS2, color = Sea_otter_region), size = 2) +
  scale_color_manual(values = so.pal, "Sea otter treatment") +
  geom_segment(data = vec.eg.dat.sig, aes(x = 0, y = 0, xend = MDS1_sc, yend = MDS2_sc), arrow = arrow(length = unit(0.07, "inches")), colour = "gray30", alpha = 1) +
...
ul.nmds <- ggplot(fa_ul_mds_dat) +
  geom_point(aes(x = NMDS1, y = NMDS2, color = Sea_otter_region), size = 2) +
  scale_color_manual(values = so.pal, "Sea otter treatment") +
  geom_segment(data = vec.ul.dat.sig, aes(x = 0, y = 0, xend = MDS1_sc, yend = MDS2_sc), arrow = arrow(length = unit(0.07, "inches")), colour = "gray30", alpha = 1) +
...
ido.nmsd <- ggplot(fa_ido_mds_dat) +
  geom_point(aes(x = NMDS1, y = NMDS2, color = Sea_otter_region), size = 2) +
  scale_color_manual(values = so.pal, "Sea otter treatment") +
  geom_segment(data = vec.ido.dat.sig, aes(x = 0, y = 0, xend = MDS1_sc, yend = MDS2_sc), arrow = arrow(length = unit(0.07, "inches")), colour = "gray30", alpha = 1) +
...
lmp.nmds <- ggplot(fa_lmp_mds_dat) +
  geom_point(aes(x = NMDS1, y = NMDS2, color = Sea_otter_region), size = 2) +
  scale_color_manual(values = so.pal, "Sea otter treatment") +
  geom_segment(data = vec.lmp.dat.sig, aes(x = 0, y = 0, xend = MDS1_sc, yend = MDS2_sc), arrow = arrow(length = unit(0.07, "inches")), colour = "gray30", alpha = 1) +
...
but.nmds <- ggplot(fa_but_mds_dat) +
  geom_point(aes(x = NMDS1, y = NMDS2, color = Sea_otter_region), size = 2) +
  scale_color_manual(values = so.pal, "Sea otter treatment") +
  geom_segment(data = vec.but.dat.sig, aes(x = 0, y = 0, xend = MDS1_sc, yend = MDS2_sc), arrow = arrow(length = unit(0.07, "inches")), colour = "gray30", alpha = 1) +
...
dsh.nmds <- ggplot(fa_dsh_mds_dat) +
  geom_point(aes(x = NMDS1, y = NMDS2, color = Sea_otter_region), size = 2) +
  scale_color_manual(values = so.pal, "Sea otter treatment") +
  geom_segment(data = vec.dsh.dat.sig, aes(x = 0, y = 0, xend = MDS1_sc, yend = MDS2_sc), arrow = arrow(length = unit(0.07, "inches")), colour = "gray30", alpha = 1) +
...
hel.nmds <- ggplot(fa_hel_mds_dat) +
  geom_point(aes(x = NMDS1, y = NMDS2, color = Sea_otter_region), size = 2) +
  scale_color_manual(values = so.pal, "Sea otter treatment") +
  geom_segment(data = vec.hel.dat.sig, aes(x = 0, y = 0, xend = MDS1_sc, yend = MDS2_sc), arrow = arrow(length = unit(0.07, "inches")), colour = "gray30", alpha = 1) +
...
grc.nmds <- ggplot(fa_grc_mds_dat) +
  geom_point(aes(x = NMDS1, y = NMDS2, color = Sea_otter_region), size = 2) +
  scale_color_manual(values = so.pal, "Sea otter treatment") +
  geom_segment(data = vec.grc.dat.sig, aes(x = 0, y = 0, xend = MDS1_sc, yend = MDS2_sc), arrow = arrow(length = unit(0.07, "inches")), colour = "gray30", alpha = 1) +
...
snk.nmds <- ggplot(fa_snk_mds_dat) +
  geom_point(aes(x = NMDS1, y = NMDS2, color = Sea_otter_region), size = 2) +
  scale_color_manual(values = so.pal, "Sea otter treatment") +
  geom_segment(data = vec.snk.dat.sig, aes(x = 0, y = 0, xend = MDS1_sc, yend = MDS2_sc), arrow = arrow(length = unit(0.07, "inches")), colour = "gray30", alpha = 1) +
...
stg.nmds <- ggplot(fa_stg_mds_dat) +
  geom_point(aes(x = NMDS1, y = NMDS2, color = Sea_otter_region), size = 2) +
  scale_color_manual(values = so.pal, "Sea otter treatment") +
  geom_segment(data = vec.stg.dat.sig, aes(x = 0, y = 0, xend = MDS1_sc, yend = MDS2_sc), arrow = arrow(length = unit(0.07, "inches")), colour = "gray30", alpha = 1) +
...
#### Panel - FA
```{r}
cowplot::plot_grid(eg.nmds, fu.nmds, ul.nmds, ido.nmsd, lmp.nmds, but.nmds, dsh.nmds, hel.nmds, grc.nmds, snk.nmds, stg.nmds, nrow = 4, ncol = 3, labels = c("a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k"), label_size = 10)
```
...
## Clean up ##
fa_grp_MWU_c <- rbind(PUFA.MWU_c, BAC.MWU_c, PAL.MWU_c, ALA.MWU_c, LIN.MWU_c, ARA.MWU_c, EPA.MWU_c, DHA.MWU_c)
