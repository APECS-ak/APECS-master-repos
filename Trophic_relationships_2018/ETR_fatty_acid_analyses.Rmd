...
## LCEFA ##
LCEFA.sum <- simp.sum %>% 
  filter(FA %in% c(LCEFA, "C20.4w6")) %>% 
  group_by(taxa) %>%
  summarise(sp_group = unique(sp_group),
            sum_EFA_contrib = sum(average_scaled))

p2.EFA <- ggplot(LCEFA.sum) +
  geom_col(aes(x = reorder(taxa, -sum_EFA_contrib), y = sum_EFA_contrib, fill = sp_group)) +
  theme(axis.text.x = element_text(angle = 90)) +
  xlab("Taxa") +
...
p3.EFA <- ggplot(EPA.sum) +
  geom_col(aes(x = reorder(taxa, -sum_EFA_contrib), y = sum_EFA_contrib, fill = sp_group)) +
  theme(axis.text.x = element_text(angle = 90)) +
  xlab("Taxa") +
  ylab(expression(atop("EPA contrib", paste("to dissimilarity"))))

p3.2.EFA <- ggplot(EPA.sum) +
  geom_boxplot(aes(x = sp_group, y = sum_EFA_contrib, fill = sp_group)) +
  xlab("Taxa") +
  scale_y_continuous(limits = c(0, 0.5), breaks = seq(0, 0.5, by = 0.1)) +
  ylab(expression(atop("EPA contrib", paste("to dissimilarity"))))

p3.3.EFA <- ggplot(subset(EPA.sum, taxa != "Particulate organic matter")) +
  geom_boxplot(aes(x = sp_group, y = sum_EFA_contrib, fill = sp_group)) +
  xlab("Taxa") +
  scale_y_continuous(limits = c(0, 0.5), breaks = seq(0, 0.5, by = 0.1)) +
  ylab(expression(atop("EPA contrib", paste("to dissimilarity"))))
...
p4.EFA <- ggplot(DHA.sum) +
  geom_col(aes(x = reorder(taxa, -sum_EFA_contrib), y = sum_EFA_contrib, fill = sp_group)) +
  theme(axis.text.x = element_text(angle = 90)) +
  xlab("Taxa") +
  ylab(expression(atop("DHA contrib", paste("to dissimilarity"))))

p4.2.EFA <- ggplot(DHA.sum) +
  geom_boxplot(aes(x = sp_group, y = sum_EFA_contrib, fill = sp_group)) +
  xlab("Taxa") +
  scale_y_continuous(limits = c(0, 0.5), breaks = seq(0, 0.5, by = 0.1)) +
  ylab(expression(atop("DHA contrib", paste("to dissimilarity"))))

p4.3.EFA <- ggplot(subset(DHA.sum, taxa != "Particulate organic matter")) +
  geom_boxplot(aes(x = sp_group, y = sum_EFA_contrib, fill = sp_group)) +
  xlab("Taxa") +
  scale_y_continuous(limits = c(0, 0.5), breaks = seq(0, 0.5, by = 0.1)) +
  ylab(expression(atop("DHA contrib", paste("to dissimilarity"))))
...
p5.EFA <- ggplot(ARA.sum) +
  geom_col(aes(x = reorder(taxa, -sum_EFA_contrib), y = sum_EFA_contrib, fill = sp_group)) +
  theme(axis.text.x = element_text(angle = 90)) +
  xlab("Taxa") +
  ylab(expression(atop("ARA contrib", paste("to dissimilarity"))))

p5.2.EFA <- ggplot(ARA.sum) +
  geom_boxplot(aes(x = sp_group, y = sum_EFA_contrib, fill = sp_group)) +
  xlab("Taxa") +
  scale_y_continuous(limits = c(0, 0.5), breaks = seq(0, 0.5, by = 0.1)) +
  ylab(expression(atop("ARA contrib", paste("to dissimilarity"))))

p5.3.EFA <- ggplot(subset(ARA.sum, taxa != "Particulate organic matter")) +
  geom_boxplot(aes(x = sp_group, y = sum_EFA_contrib, fill = sp_group)) +
  xlab("Taxa") +
  scale_y_continuous(limits = c(0, 0.5), breaks = seq(0, 0.5, by = 0.1)) +
  ylab(expression(atop("ARA contrib", paste("to dissimilarity"))))
...
pxxxx.EFA <- ggplot(ALLEFA.sum) +
  geom_col(aes(x = reorder(taxa, -sum_EFA_contrib), y = sum_EFA_contrib, fill = sp_group)) +
  theme(axis.text.x = element_text(angle = 90)) +
  xlab("Taxa") +
  ylab(expression(atop("All EFA contrib", paste("to dissimilarity"))))

## Panel ##
plot_grid(p2.EFA, p3.EFA, p4.EFA, p5.EFA, nrow = 2, ncol = 2)
plot_grid(p2.2.EFA, p3.2.EFA, p4.2.EFA, p5.2.EFA, nrow = 2, ncol = 2)
plot_grid(p2.3.EFA, p3.3.EFA, p4.3.EFA, p5.3.EFA, nrow = 2, ncol = 2, labels = "No POM")
