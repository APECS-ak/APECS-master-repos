...
POM
```{r}
mod.pm.1 <- lme(pc_om_POM ~ sea_otter_tr, random = ~1|site, data = pom)
summary(mod.pm.1)
anova(mod.pm.1)
```
...
# Helmet #
mod.hel.1 <- lme(mass_g^0.33 ~ sea_otter_tr, random = ~1|site, data = subset(crb.site.string.sp, species_common == "Helmet crab"))
summary(mod.hel.1)
...
ggplot(eg.sum) +
  geom_point(aes(x = sea_otter_tr, y = shoot_mass_m_mean, fill = sea_otter_tr))
