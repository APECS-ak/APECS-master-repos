...
eg.sum <- eg %>% 
  ...
  summarise(shoot_mass_m_mean = mean(shoot_mass_m),
            ...
            ido_load_mean = mean(Pentidotea_load),
            ido_load_sd = sd(Pentidotea_load ),
...
BAC <- c("a.C17.0", "a.C18.0",  "C15.0", "C17.0", "C17.1w9", "C18.1w7", "C18.1w9", "C20.1w7", "C20.1w9", "C22.1w9", "i.C15.0", "i.C16.0", "i.C17.0")
```
...
```{r}
## ALA.LIN ##
ALA.LIN.MWU <- fa_ALA.LIN_all %>% 
  group_by(Species_common) %>% 
  do(broom::tidy(wilcox.test(prop_sum ~ Sea_otter_region, exact = FALSE, data = .)))
ALA.LIN.MWU$FA_grp <- "ALA.LIN"

## C18PUFA ##
C18PUFA.MWU  <- fa_C18PUFA_all %>% 
  group_by(Species_common) %>% 
  do(broom::tidy(wilcox.test(prop_sum ~ Sea_otter_region, exact = FALSE, data = .)))
C18PUFA.MWU$FA_grp <- "C18PUFA"
...
## Clean up ##
fa_grp_MWU <- rbind(ALA.LIN.MWU, C18PUFA.MWU, PUFA.MWU, BAC.MWU, PAL.MWU, ARA.MWU, EPA.MWU, DHA.MWU)

fa_grp_MWU_p <- fa_grp_MWU[, c(1, 3, 6)]
fa_grp_MWU_p$p.value <- round(fa_grp_MWU_p$p.value, 3)
fa_grp_MWU_p$FA_grp <- factor(fa_grp_MWU_p$FA_grp, levels = c("BAC", "ALA.LIN", "C18PUFA", "PUFA", "PAL", "ARA", "EPA", "DHA"))
...
fa_grp_MWU_t$FA_grp <- factor(fa_grp_MWU_t$FA_grp, levels = c("BAC", "ALA.LIN", "C18PUFA", "PUFA", "PAL", "ARA", "EPA", "DHA"))
...
ggplot(subset(fa_grp, FA_grp == "ARA"), aes(x = Sea_otter_region, y = prop_sum_m, fill = Sea_otter_region)) +
  geom_bar(position = position_dodge(), stat = "identity") +
  geom_errorbar(aes(ymin = prop_sum_m - prop_sum_sd, ymax = prop_sum_m + prop_sum_sd), width = 0, position = position_dodge(.9)) +
  scale_fill_manual("Sea otter \n region", c("High", "Low"), values = so.pal) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
...
## All together now - for t.test ##
fa_grp_c_all <- rbind(fa_BAC_c_all, fa_ALA.LIN_c_all, fa_C18PUFA_c_all, fa_PUFA_c_all, fa_PAL_c_all, fa_ARA_c_all, fa_EPA_c_all, fa_DHA_c_all)

## All together now - for plots ##
fa_grp_c <- rbind(fa_BAC_c, fa_ALA.LIN_c, fa_C18PUFA_c, fa_PUFA_c, fa_PAL_c, fa_ARA_c, fa_EPA_c, fa_DHA_c)
fa_grp_c$FA_grp <- factor(fa_grp_c$FA_grp, levels = c("BAC", "ALA.LIN", "C18PUFA", "PUFA", "PAL", "ARA", "EPA", "DHA"))
...
```{r}
## ALA.LIN ##
ALA.LIN.MWU_c <- fa_ALA.LIN_c_all %>% 
  group_by(Species_common) %>% 
  do(broom::tidy(wilcox.test(conc_sum ~ Sea_otter_region, exact = FALSE, data = .)))
ALA.LIN.MWU_c$FA_grp <- "ALA.LIN"

## C18PUFA ##
C18PUFA.MWU_c  <- fa_C18PUFA_c_all %>% 
  group_by(Species_common) %>% 
  do(broom::tidy(wilcox.test(conc_sum ~ Sea_otter_region, exact = FALSE, data = .)))
C18PUFA.MWU_c$FA_grp <- "C18PUFA"
...
## Clean up ##
fa_grp_MWU_c <- rbind(ALA.LIN.MWU_c, C18PUFA.MWU_c, PUFA.MWU_c, BAC.MWU_c, PAL.MWU_c, ARA.MWU_c, EPA.MWU_c, DHA.MWU_c)

fa_grp_MWU_c_p <- fa_grp_MWU_c[, c(1, 3, 6)]
fa_grp_MWU_c_p$p.value <- round(fa_grp_MWU_c_p$p.value, 3)
fa_grp_MWU_c_p$FA_grp <- factor(fa_grp_MWU_c_p$FA_grp, levels = c("BAC", "ALA.LIN", "C18PUFA", "PUFA", "PAL", "ARA", "EPA", "DHA"))
...
fa_grp_MWU_c_t$FA_grp <- factor(fa_grp_MWU_c_t$FA_grp, levels = c("BAC", "ALA.LIN", "C18PUFA", "PUFA", "PAL", "ARA", "EPA", "DHA"))
...
#### FA Conc X Biomass
Individual FA concentration is interesting but what happens when we multiply that by the biomass of the taxa that we measured.

##### Combine FA conc and biomass data
```{r}
