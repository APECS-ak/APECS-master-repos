...
crb.site.string$string_mass_g <- as.numeric(crb.site.string$string_mass_g)

crb.site.string <- merge(crb.site.string, site[, c(1, 5)], by = "site")

# Site string and sp #
d <- expand.grid(species_common = c("Dungeness crab", "Helmet crab", "Red rock crab"), site = unique(crb$site), string = unique(crb$string))
...
# Dungeness #
mod.dun.1 <- lm(mass_g ~ sea_otter_tr + site %in% sea_otter_tr, data = subset(crb.site.string.sp, species_common == "Dungeness crab"))
anova(mod.dun.1)

# Helmet #
mod.hel.1 <- lm(mass_g ~ sea_otter_tr + site %in% sea_otter_tr, data = subset(crb.site.string.sp, species_common == "Helmet crab"))
anova(mod.hel.1)

# Red rock #
mod.roc.1 <- lm(mass_g ~ sea_otter_tr + site %in% sea_otter_tr, data = subset(crb.site.string.sp, species_common == "Red rock crab"))
anova(mod.roc.1)
```
...
# Butter clams #
mod.but.1 <- lm(mass_g ~ sea_otter_tr + site %in% sea_otter_tr, data = subset(clm.site.quad.sp, species_common == "Butter clam"))
anova(mod.but.1)

# Macoma sp. ##
mod.mac.1 <- lm(mass_g ~ sea_otter_tr + site %in% sea_otter_tr, data = subset(clm.site.quad.sp, species_common == "Macoma sp."))
anova(mod.mac.1)
```
...
## Stags ##
mod.stg.1 <- lm(SCULPSTG ~ sea_otter_tr, data = fsh)
anova(mod.stg.1)

## Shiners ##
mod.shn.1 <- lm(PERCHSH ~ sea_otter_tr, data = fsh)
anova(mod.shn.1)

## Snake Prickle bach ##
mod.snk.1 <- lm(PRICKSN ~ sea_otter_tr, data = fsh)
anova(mod.snk.1)
```
...
# Idotea #
fa_ido_1.0 <- merge(fa_ido_1.0, bimrk, by.x = "ID", by.y = "Biomarker_ID")
fa_ido_1.0 <- cbind(fa_ido_1.0[,c(69:76, 1, 3:67)])
colnames(fa_ido_1.0)[6] <- "Species_common"
colnames(fa_ido_1.0)[3] <- "Sea_otter_tr"
```
...
fa_ido_1.0_trs <- asin(sqrt(fa_ido_1.0[, 10:74]))
...
```{r dist, echo = FALSE}
## Producers ##
fa_pro_1.0_dist <- vegdist(fa_pro_1.0_trs, method = "euclidean")
...
## Producers ##
perm.pro <- adonis2(fa_pro_1.0_dist ~ Sea_otter_tr + Site %in% Sea_otter_tr, data = fa_pro_1.0, perm = 9999) 
perm.pro
