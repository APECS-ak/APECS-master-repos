...
## Nested Anova ##
mod.crb.1 <- lm(mass_g / 1000 ~ sea_otter_tr + sea_otter_tr %in% site, data = crb.site.string)
anova(mod.crb.1)
...
##### Panel - biomass
```{r}
cowplot::plot_grid(elg.m, epi.m, grz.all, ido.m, lmp.m, clm.all, but.m, mac.m, crb.all, grc.m, hel.m, rrc.m, fsh.all, stg.m, shn.m, snk.m, labels = c("a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "p"), nrow = 4, ncol = 4, label_size = 10, align = "v")
```
...
ggplot(si) +
  ...
  scale_color_manual(values = so.pal)
...
si.all <- ggplot() +
  ...
  geom_text_repel(aes(x = si.sum$del_C_mean, y = si.sum$del_N_mean, label = si.names), size = 3) +
  xlab(expression({delta}^13*C~'\u2030')) +
  ylab(expression({delta}^15*N~'\u2030'))

## N ranks ##
si.sum$N_rank <- NA
order.N <- order(si.sum$del_N_mean)
si.sum$N_rank[order.N] <- 1:nrow(si.sum)

si.n <- ggplot(si.sum) +
    geom_errorbar(aes(x = N_rank, ymin = del_N_mean - del_N_sd, ymax = del_N_mean + del_N_sd), width = 0) +
  geom_point(aes(x = N_rank, y = del_N_mean, shape = sp_group, color = Sea_otter_tr), size = 3) +
  scale_color_manual(values = so.pal, "Sea otter treatment") +
  scale_shape_manual(values = shape.pal, "Trophic role", labels = c("Primary consumer", "Primary producer", "Secondary consumer")) +
  geom_text_repel(aes(x = N_rank, y = del_N_mean, label = si.names), size = 3) +
  scale_y_continuous(limits = c(5, 15), breaks = seq(5, 15, by = 2)) +
...
##### Panel - SI
```{r}
legend <- get_legend(si.all)

si.pan1 <- cowplot::plot_grid(si.all + theme(legend.position="none"), si.n, labels = c("a", "b"), nrow = 1, ncol = 2, label_size = 10, rel_widths = c(1, 1.4))
...
fa_mrk <- merge(fa_mrk, fa[, c(3,6)], by = "Species_common")
...
ggplot(fa_mrk, aes(x = Species_common, y = prop_m, fill = Sea_otter_region)) +
  geom_bar(position = position_dodge(), stat = "identity") +
  geom_errorbar(aes(x = Species_common, ymin = prop_m - prop_sd, ymax = prop_m + prop_sd), width = 0, position = position_dodge(.9)) +
...
C18EFA <- c("C18.2w6", "C18.3w3", "C18.3w6", "C18.4w3")
...
## LCPUFA ##
fa_LCPUFA_all <- fa_tll %>% 
  filter(FA %in% LCPUFA) %>% 
  group_by(Biomarker_ID) %>% 
  summarize(prop_sum = sum(prop))

fa_LCPUFA_all <- merge(unique(fa_tll[, 1:10]), fa_LCPUFA_all, by = "Biomarker_ID")
fa_LCPUFA_all$FA_grp  <- "LCPUFA" 

fa_LCPUFA <- fa_LCPUFA_all %>% 
  group_by(Sea_otter_region, Species_common) %>% 
  summarize(prop_sum_m = mean(prop_sum),
            prop_sum_sd = sd(prop_sum))
fa_LCPUFA$FA_grp <- "LCPUFA"

## C18EFA ##
fa_C18EFA_all <- fa_tll %>% 
  filter(FA %in% C18EFA) %>% 
  group_by(Biomarker_ID) %>% 
  summarize(prop_sum = sum(prop))

fa_C18EFA_all <- merge(unique(fa_tll[, 1:10]), fa_C18EFA_all, by = "Biomarker_ID")
fa_C18EFA_all$FA_grp  <- "C18EFA" 

fa_C18EFA <- fa_C18EFA_all %>% 
  group_by(Sea_otter_region, Species_common) %>% 
  summarize(prop_sum_m = mean(prop_sum),
            prop_sum_sd = sd(prop_sum))
fa_C18EFA$FA_grp <- "C18EFA"
...
fa_OM3_all <- merge(unique(fa_tll[, 1:10]), fa_OM3_all, by = "Biomarker_ID")
fa_OM3_all$FA_grp  <- "OM3"
...
fa_OM6_all <- merge(unique(fa_tll[, 1:10]), fa_OM6_all, by = "Biomarker_ID")
fa_OM6_all$FA_grp  <- "OM6"
...
fa_OM3t <- fa_tll %>% 
  filter(FA %in% OM3) %>% 
  group_by(Biomarker_ID) %>% 
  summarize(prop_sum_OM3 = sum(prop))

fa_OM6t <- fa_tll %>% 
  filter(FA %in% OM6) %>% 
  group_by(Biomarker_ID) %>% 
  summarize(prop_sum_OM6 = sum(prop))

fa_OM3.6 <- data.frame(Biomarker_ID = fa_OM3t$Biomarker_ID, 
                       prop_OM3.6_ratio = fa_OM3t$prop_sum_OM3 / fa_OM6t$prop_sum_OM6)

fa_OM3.6 <- merge(unique(fa_tll[, 1:10]), fa_OM3.6, by = "Biomarker_ID")

fa_OM3.6 <- fa_OM3.6 %>% 
  group_by(Sea_otter_region, Species_common) %>% 
  summarize(prop_ratio_m = mean(prop_OM3.6_ratio),
            prop_ratio_sd = sd(prop_OM3.6_ratio))
...
fa_DHA_all <- fa_tll %>% 
  ...
  summarize(prop_sum_m = mean(prop),
            prop_sum_sd = sd(prop))
fa_DHA_all$FA_grp <- "DHA"

## All together now - for t.test ##
fa_grp_all <- rbind(fa_LCPUFA_all, fa_PUFA_all, fa_OM3_all, fa_OM6_all, fa_C18EFA_all, fa_BAC_all, fa_EPA_all, fa_DHA_all)

## All together now - for plots ##
fa_grp <- rbind(fa_LCPUFA, fa_PUFA, fa_OM3, fa_OM6, fa_C18EFA, fa_BAC, fa_EPA, fa_DHA)
fa_grp$FA_grp <- factor(fa_grp$FA_grp, levels = c("BAC", "C18EFA", "OM3", "OM6", "LCPUFA", "PUFA", "EPA", "DHA"))
...
```{r}
## LCPUFA ##
LCPUFA.ttest <- fa_LCPUFA_all %>% 
  group_by(Species_common) %>% 
  do(tidy(t.test(prop_sum ~ Sea_otter_region, data=.)))
LCPUFA.ttest$FA_grp <- "LCPUFA"

## C18EFA ##
C18EFA.ttest <- fa_C18EFA_all %>% 
  group_by(Species_common) %>% 
  do(tidy(t.test(prop_sum ~ Sea_otter_region, data=.)))
C18EFA.ttest$FA_grp <- "C18EFA"

## OM3 ##
OM3.ttest <- fa_OM3_all %>% 
  group_by(Species_common) %>% 
  do(tidy(t.test(prop_sum ~ Sea_otter_region, data=.)))
OM3.ttest$FA_grp <- "OM3"

## OM6 ##
OM6.ttest <- fa_OM6_all %>% 
  group_by(Species_common) %>% 
  do(tidy(t.test(prop_sum ~ Sea_otter_region, data=.)))
OM6.ttest$FA_grp <- "OM6"
...
## Clean up ##
fa_grp_ttest <- rbind(LCPUFA.ttest, C18EFA.ttest, OM3.ttest, OM6.ttest, PUFA.ttest, BAC.ttest, EPA.ttest, DHA.ttest)

fa_grp_ttest_p <- fa_grp_ttest[, c(1, 6, 12)]
fa_grp_ttest_p$p.value <- round(fa_grp_ttest_p$p.value, 3)
fa_grp_ttest_p$FA_grp <- factor(fa_grp_ttest_p$FA_grp, levels = c("BAC", "C18EFA", "OM3", "OM6", "LCPUFA", "PUFA", "EPA", "DHA"))
...
fa_grp_ttest_p <- spread(fa_grp_ttest_p, key = FA_grp, value = p.value)
```
...
ggplot(fa_grp, aes(x = FA_grp, y = prop_sum_m, fill = Sea_otter_region)) +
  ...
  xlab("Fatty acid") +
  ylab("Mean Proportion") +
  facet_wrap(~Species_common, labeller = )
```
