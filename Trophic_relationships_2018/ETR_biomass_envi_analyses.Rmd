...
oce18 <- read.csv("../ALL_DATA/Site_oceanography_2018_RAW.csv", header = TRUE, stringsAsFactors = FALSE)
...
```{r}
## 2018 ##
oce18 <- merge(oce18, site[, c(1, 5)], by.x = "site", by.y = "site")
oce18$Depth_m <- as.character(oce18$Depth_m)

oce18$light_percent <- oce18$X5m_PAR / oce18$X1m_PAR * 100

## 2017 ##
abiot17$sampling_type <- ifelse(abiot17$date_YYYYMMDD == 20170814, "nutday", "eelgrass")
abiot17$depth_m <- as.character(abiot17$depth_m)
```
...
pan.nox <- plot_grid(p.nox18, p.nox17, nrow = 1, ncol = 2)
...
pan.pho <- plot_grid(p.po418, p.po417, nrow = 1, ncol = 2)

## Temperature ##
p.tmp18 <- ggplot(oce18) +
...
p.tmp17 <- ggplot(subset(abiot17, depth_m != 0)) +
  geom_boxplot(aes(x = sampling_type, y = temperature_c, fill = sampling_type, alpha = depth_m)) +
  scale_fill_manual("Depth", labels = c("1m", "4m"), values = ext.pal) +
  scale_alpha_discrete(range = c(0.5, 1)) +
  scale_y_continuous(limits = c(8, 16)) +
  theme(legend.position = "none")

pan.tmp <- plot_grid(p.tmp18, p.tmp17, nrow = 1, ncol = 2)

## DO ##
p.do18 <- ggplot(oce18) +
  geom_boxplot(aes(x = sea_otter_tr, y = DO_mgl, fill = sea_otter_tr, alpha = Depth_m)) +
  scale_fill_manual("Depth", labels = c("1m", "5m"), values = so.pal) +
  scale_alpha_discrete(range = c(0.5, 1)) +
...
p.do17 <- ggplot(subset(abiot17, depth_m != 0)) +
  geom_boxplot(aes(x = sampling_type, y = DO_mg.L, fill = sampling_type, alpha = depth_m)) +
  scale_fill_manual("Depth", labels = c("1m", "4m"), values = ext.pal) +
  scale_alpha_discrete(range = c(0.5, 1)) +
  scale_y_continuous(limits = c(4, 12)) +
  theme(legend.position = "none")

pan.do <- plot_grid(p.do18, p.do17, nrow = 1, ncol = 2)

## Salinity ##
p.sal18 <- ggplot(oce18) +
  geom_boxplot(aes(x = sea_otter_tr, y = sal_ppt, fill = sea_otter_tr, alpha = Depth_m)) +
  scale_fill_manual("Depth", labels = c("1m", "5m"), values = so.pal) +
  scale_alpha_discrete(range = c(0.5, 1)) +
  scale_y_continuous(limits = c(30, 34), seq(30, 34, by = 2)) +
  scale_x_discrete(labels = c("High", "Low")) +
  ylab("Salinity (ppt)") +
  xlab("Sea otter region") +
  theme(legend.position = "none")

p.sal17 <- ggplot(subset(abiot17, depth_m != 0)) +
  geom_boxplot(aes(x = sampling_type, y = salinity_ppt, fill = sampling_type, alpha = depth_m)) +
  scale_fill_manual("Depth", labels = c("1m", "4m"), values = ext.pal) +
  scale_alpha_discrete(range = c(0.5, 1)) +
  scale_y_continuous(limits = c(14, 34), seq(14, 34, by = 2)) +
  ylab("Salinity (ppt)") +
  theme(legend.position = "none")

pan.sal <- plot_grid(p.sal18, p.sal17, nrow = 1, ncol = 2)

### Final Panel ###
plot_grid(pan.tmp, pan.sal, pan.nox, pan.do, pan.pho, nrow = 5, ncol = 1, align = "v")
```
...
elg.m <- ggplot() +
  ...
  scale_color_manual(values = so.pal, "Sea otter region") +
  geom_segment(data = subset(eg.sum.tr, sea_otter_tr == "high"), aes(x = 1, y = shoot_mass_m_mean, xend = 3, yend = shoot_mass_m_mean),size = 1.5, color = so.pal[1]) +
  geom_segment(data = subset(eg.sum.tr, sea_otter_tr == "high"), aes(x = 1, y = shoot_mass_m_mean + shoot_mass_m_sd, xend = 3, yend = shoot_mass_m_mean + shoot_mass_m_sd), size = 1.5, linetype = "dotted", color = so.pal[1]) +
  geom_segment(data = subset(eg.sum.tr, sea_otter_tr == "high"), aes(x = 1, y = shoot_mass_m_mean - shoot_mass_m_sd, xend = 3, yend = shoot_mass_m_mean - shoot_mass_m_sd), size = 1.5, linetype = "dotted", color = so.pal[1]) +
  geom_segment(data = subset(eg.sum.tr, sea_otter_tr == "low"), aes(x = 4, y = shoot_mass_m_mean, xend = 6, yend = shoot_mass_m_mean), size = 1.5 ,color = so.pal[2]) +
  geom_segment(data = subset(eg.sum.tr, sea_otter_tr == "low"), aes(x = 4, y = shoot_mass_m_mean + shoot_mass_m_sd, xend = 6, yend = shoot_mass_m_mean + shoot_mass_m_sd), size = 1.5 , linetype = "dotted", color = so.pal[2]) +
  geom_segment(data = subset(eg.sum.tr, sea_otter_tr == "low"), aes(x = 4, y = shoot_mass_m_mean - shoot_mass_m_sd, xend = 6, yend = shoot_mass_m_mean - shoot_mass_m_sd), size = 1.5 , linetype = "dotted", color = so.pal[2]) +
...
epi.m <- ggplot() +
  ...
  scale_color_manual(values = so.pal, "Sea otter region") +
  geom_segment(data = subset(eg.sum.tr, sea_otter_tr == "high"), aes(x = 1, y = epi_load_mean, xend = 3, yend = epi_load_mean),size = 1.5, color = so.pal[1]) +
  geom_segment(data = subset(eg.sum.tr, sea_otter_tr == "high"), aes(x = 1, y = epi_load_mean + epi_load_sd, xend = 3, yend = epi_load_mean + epi_load_sd), size = 1.5, linetype = "dotted", color = so.pal[1]) +
  geom_segment(data = subset(eg.sum.tr, sea_otter_tr == "high"), aes(x = 1, y = epi_load_mean - epi_load_sd, xend = 3, yend = epi_load_mean - epi_load_sd), size = 1.5, linetype = "dotted", color = so.pal[1]) +
  geom_segment(data = subset(eg.sum.tr, sea_otter_tr == "low"), aes(x = 4, y = epi_load_mean, xend = 6, yend = epi_load_mean), size = 1.5 ,color = so.pal[2]) +
  geom_segment(data = subset(eg.sum.tr, sea_otter_tr == "low"), aes(x = 4, y = epi_load_mean + epi_load_sd, xend = 6, yend = epi_load_mean + epi_load_sd), size = 1.5 , linetype = "dotted", color = so.pal[2]) +
  geom_segment(data = subset(eg.sum.tr, sea_otter_tr == "low"), aes(x = 4, y = epi_load_mean - epi_load_sd, xend = 6, yend = epi_load_mean - epi_load_sd), size = 1.5 , linetype = "dotted", color = so.pal[2]) +
  scale_x_discrete(labels = c("H1", "H2", "H3", "L1", "L2", "L3")) +
...
grz.all <- ggplot() +
  ...
  scale_color_manual(values = so.pal, "Sea otter region") +
  geom_segment(data = subset(eg.sum.tr, sea_otter_tr == "high"), aes(x = 1, y = grz_load_mean, xend = 3, yend = grz_load_mean),size = 1.5, color = so.pal[1]) +
  geom_segment(data = subset(eg.sum.tr, sea_otter_tr == "high"), aes(x = 1, y = grz_load_mean + grz_load_sd, xend = 3, yend = grz_load_mean + grz_load_sd), size = 1.5, linetype = "dotted", color = so.pal[1]) +
  geom_segment(data = subset(eg.sum.tr, sea_otter_tr == "high"), aes(x = 1, y = grz_load_mean - grz_load_sd, xend = 3, yend = grz_load_mean - grz_load_sd), size = 1.5, linetype = "dotted", color = so.pal[1]) +
  geom_segment(data = subset(eg.sum.tr, sea_otter_tr == "low"), aes(x = 4, y = grz_load_mean, xend = 6, yend = grz_load_mean), size = 1.5 ,color = so.pal[2]) +
  geom_segment(data = subset(eg.sum.tr, sea_otter_tr == "low"), aes(x = 4, y = grz_load_mean + grz_load_sd, xend = 6, yend = grz_load_mean + grz_load_sd), size = 1.5 , linetype = "dotted", color = so.pal[2]) +
  geom_segment(data = subset(eg.sum.tr, sea_otter_tr == "low"), aes(x = 4, y = grz_load_mean - grz_load_sd, xend = 6, yend = grz_load_mean - grz_load_sd), size = 1.5 , linetype = "dotted", color = so.pal[2]) +
  scale_x_discrete(labels = c("H1", "H2", "H3", "L1", "L2", "L3")) +
...
ido.m <- ggplot() +
  ...
  scale_color_manual(values = so.pal, "Sea otter region") +
  geom_segment(data = subset(eg.sum.tr, sea_otter_tr == "high"), aes(x = 1, y = ido_load_mean, xend = 3, yend = ido_load_mean),size = 1.5, color = so.pal[1]) +
  geom_segment(data = subset(eg.sum.tr, sea_otter_tr == "high"), aes(x = 1, y = ido_load_mean + ido_load_sd, xend = 3, yend = ido_load_mean + ido_load_sd), size = 1.5, linetype = "dotted", color = so.pal[1]) +
  geom_segment(data = subset(eg.sum.tr, sea_otter_tr == "high"), aes(x = 1, y = ido_load_mean - ido_load_sd, xend = 3, yend = ido_load_mean - ido_load_sd), size = 1.5, linetype = "dotted", color = so.pal[1]) +
  geom_segment(data = subset(eg.sum.tr, sea_otter_tr == "low"), aes(x = 4, y = ido_load_mean, xend = 6, yend = ido_load_mean), size = 1.5 ,color = so.pal[2]) +
  geom_segment(data = subset(eg.sum.tr, sea_otter_tr == "low"), aes(x = 4, y = ido_load_mean + ido_load_sd, xend = 6, yend = ido_load_mean + ido_load_sd), size = 1.5 , linetype = "dotted", color = so.pal[2]) +
  geom_segment(data = subset(eg.sum.tr, sea_otter_tr == "low"), aes(x = 4, y = ido_load_mean - ido_load_sd, xend = 6, yend = ido_load_mean - ido_load_sd), size = 1.5 , linetype = "dotted", color = so.pal[2]) +
  scale_x_discrete(labels = c("H1", "H2", "H3", "L1", "L2", "L3")) +
...
lmp.m <- ggplot() +
  ...
  scale_color_manual(values = so.pal, "Sea otter region") +
  geom_segment(data = subset(eg.sum.tr, sea_otter_tr == "high"), aes(x = 1, y = lmp_load_mean, xend = 3, yend = lmp_load_mean),size = 1.5, color = so.pal[1]) +
  geom_segment(data = subset(eg.sum.tr, sea_otter_tr == "high"), aes(x = 1, y = lmp_load_mean + lmp_load_sd, xend = 3, yend = lmp_load_mean + lmp_load_sd), size = 1.5, linetype = "dotted", color = so.pal[1]) +
  geom_segment(data = subset(eg.sum.tr, sea_otter_tr == "high"), aes(x = 1, y = lmp_load_mean - lmp_load_sd, xend = 3, yend = lmp_load_mean - lmp_load_sd), size = 1.5, linetype = "dotted", color = so.pal[1]) +
  geom_segment(data = subset(eg.sum.tr, sea_otter_tr == "low"), aes(x = 4, y = lmp_load_mean, xend = 6, yend = lmp_load_mean), size = 1.5 ,color = so.pal[2]) +
  geom_segment(data = subset(eg.sum.tr, sea_otter_tr == "low"), aes(x = 4, y = lmp_load_mean + lmp_load_sd, xend = 6, yend = lmp_load_mean + lmp_load_sd), size = 1.5 , linetype = "dotted", color = so.pal[2]) +
  geom_segment(data = subset(eg.sum.tr, sea_otter_tr == "low"), aes(x = 4, y = lmp_load_mean - lmp_load_sd, xend = 6, yend = lmp_load_mean - lmp_load_sd), size = 1.5 , linetype = "dotted", color = so.pal[2]) +
  scale_x_discrete(labels = c("H1", "H2", "H3", "L1", "L2", "L3")) +
...
mac.m <- ggplot() +
  ...
  scale_color_manual(values = so.pal, "Sea otter regmion") +
  geom_segment(data = subset(clm.sum.tr, sea_otter_tr == "high"), aes(x = 1, y = mac_mass_mean, xend = 3, yend = mac_mass_mean),size = 1.5, color = so.pal[1]) +
  geom_segment(data = subset(clm.sum.tr, sea_otter_tr == "high"), aes(x = 1, y = mac_mass_mean + mac_mass_sd, xend = 3, yend = mac_mass_mean + mac_mass_sd), size = 1.5, linetype = "dotted", color = so.pal[1]) +
  geom_segment(data = subset(clm.sum.tr, sea_otter_tr == "high"), aes(x = 1, y = mac_mass_mean - mac_mass_sd, xend = 3, yend = mac_mass_mean - mac_mass_sd), size = 1.5, linetype = "dotted", color = so.pal[1]) +
  geom_segment(data = subset(clm.sum.tr, sea_otter_tr == "low"), aes(x = 4, y = mac_mass_mean, xend = 6, yend = mac_mass_mean), size = 1.5 ,color = so.pal[2]) +
  geom_segment(data = subset(clm.sum.tr, sea_otter_tr == "low"), aes(x = 4, y = mac_mass_mean + mac_mass_sd, xend = 6, yend = mac_mass_mean + mac_mass_sd), size = 1.5 , linetype = "dotted", color = so.pal[2]) +
  geom_segment(data = subset(clm.sum.tr, sea_otter_tr == "low"), aes(x = 4, y = mac_mass_mean - mac_mass_sd, xend = 6, yend = mac_mass_mean - mac_mass_sd), size = 1.5 , linetype = "dotted", color = so.pal[2]) +
  scale_x_discrete(labels = c("H1", "H2", "H3", "L1", "L2", "L3")) +
...
grc.m <- ggplot() +
  ...
  scale_color_manual(values = so.pal, "Sea otter regmion") +
  geom_segment(data = subset(crb.sum.tr, sea_otter_tr == "high"), aes(x = 1, y = grc_mass_mean, xend = 3, yend = grc_mass_mean),size = 1.5, color = so.pal[1]) +
  geom_segment(data = subset(crb.sum.tr, sea_otter_tr == "high"), aes(x = 1, y = grc_mass_mean + grc_mass_sd, xend = 3, yend = grc_mass_mean + grc_mass_sd), size = 1.5, linetype = "dotted", color = so.pal[1]) +
  geom_segment(data = subset(crb.sum.tr, sea_otter_tr == "high"), aes(x = 1, y = grc_mass_mean - grc_mass_sd, xend = 3, yend = grc_mass_mean - grc_mass_sd), size = 1.5, linetype = "dotted", color = so.pal[1]) +
  geom_segment(data = subset(crb.sum.tr, sea_otter_tr == "low"), aes(x = 4, y = grc_mass_mean, xend = 6, yend = grc_mass_mean), size = 1.5 ,color = so.pal[2]) +
  geom_segment(data = subset(crb.sum.tr, sea_otter_tr == "low"), aes(x = 4, y = grc_mass_mean + grc_mass_sd, xend = 6, yend = grc_mass_mean + grc_mass_sd), size = 1.5 , linetype = "dotted", color = so.pal[2]) +
  geom_segment(data = subset(crb.sum.tr, sea_otter_tr == "low"), aes(x = 4, y = grc_mass_mean - grc_mass_sd, xend = 6, yend = grc_mass_mean - grc_mass_sd), size = 1.5 , linetype = "dotted", color = so.pal[2]) +
  scale_x_discrete(labels = c("H1", "H2", "H3", "L1", "L2", "L3")) +
...
hel.m <- ggplot() +
  ...
  scale_color_manual(values = so.pal, "Sea otter region") +
  geom_segment(data = subset(crb.sum.tr, sea_otter_tr == "high"), aes(x = 1, y = hel_mass_mean, xend = 3, yend = hel_mass_mean),size = 1.5, color = so.pal[1]) +
  geom_segment(data = subset(crb.sum.tr, sea_otter_tr == "high"), aes(x = 1, y = hel_mass_mean + hel_mass_sd, xend = 3, yend = hel_mass_mean + hel_mass_sd), size = 1.5, linetype = "dotted", color = so.pal[1]) +
  geom_segment(data = subset(crb.sum.tr, sea_otter_tr == "high"), aes(x = 1, y = hel_mass_mean - hel_mass_sd, xend = 3, yend = hel_mass_mean - hel_mass_sd), size = 1.5, linetype = "dotted", color = so.pal[1]) +
  geom_segment(data = subset(crb.sum.tr, sea_otter_tr == "low"), aes(x = 4, y = hel_mass_mean, xend = 6, yend = hel_mass_mean), size = 1.5 ,color = so.pal[2]) +
  geom_segment(data = subset(crb.sum.tr, sea_otter_tr == "low"), aes(x = 4, y = hel_mass_mean + hel_mass_sd, xend = 6, yend = hel_mass_mean + hel_mass_sd), size = 1.5 , linetype = "dotted", color = so.pal[2]) +
  geom_segment(data = subset(crb.sum.tr, sea_otter_tr == "low"), aes(x = 4, y = hel_mass_mean - hel_mass_sd, xend = 6, yend = hel_mass_mean - hel_mass_sd), size = 1.5 , linetype = "dotted", color = so.pal[2]) +
  scale_x_discrete(labels = c("H1", "H2", "H3", "L1", "L2", "L3")) +
...
fsh.all <- ggplot() +
  scale_color_manual(values = so.pal, "Sea otter region") +
  geom_segment(data = subset(fsh.sum.1, sea_otter_tr == "high"), aes(x = 1, y = mass_kg_mean, xend = 3, yend = mass_kg_mean),size = 1.5, color = so.pal[1]) +
  geom_segment(data = subset(fsh.sum.1, sea_otter_tr == "high"), aes(x = 1, y = mass_kg_mean + mass_kg_sd, xend = 3, yend = mass_kg_mean + mass_kg_sd), size = 1.5, linetype = "dotted", color = so.pal[1]) +
  geom_segment(data = subset(fsh.sum.1, sea_otter_tr == "high"), aes(x = 1, y = mass_kg_mean - mass_kg_sd, xend = 3, yend = mass_kg_mean - mass_kg_sd), size = 1.5, linetype = "dotted", color = so.pal[1]) +
  geom_segment(data = subset(fsh.sum.1, sea_otter_tr == "low"), aes(x = 4, y = mass_kg_mean, xend = 6, yend = mass_kg_mean), size = 1.5 ,color = so.pal[2]) +
  geom_segment(data = subset(fsh.sum.1, sea_otter_tr == "low"), aes(x = 4, y = mass_kg_mean + mass_kg_sd, xend = 6, yend = mass_kg_mean + mass_kg_sd), size = 1.5 , linetype = "dotted", color = so.pal[2]) +
  geom_segment(data = subset(fsh.sum.1, sea_otter_tr == "low"), aes(x = 4, y = mass_kg_mean - mass_kg_sd, xend = 6, yend = mass_kg_mean - mass_kg_sd), size = 1.5 , linetype = "dotted", color = so.pal[2]) +
  scale_x_discrete(labels = c("High", "Low")) +
...
stg.m <- ggplot() +
  scale_color_manual(values = so.pal, "Sea otter region") +
  geom_segment(data = subset(stg.sum.1, sea_otter_tr == "high"), aes(x = 1, y = mass_kg_mean, xend = 3, yend = mass_kg_mean),size = 1.5, color = so.pal[1]) +
  geom_segment(data = subset(stg.sum.1, sea_otter_tr == "high"), aes(x = 1, y = mass_kg_mean + mass_kg_sd, xend = 3, yend = mass_kg_mean + mass_kg_sd), size = 1.5, linetype = "dotted", color = so.pal[1]) +
  geom_segment(data = subset(stg.sum.1, sea_otter_tr == "high"), aes(x = 1, y = mass_kg_mean - mass_kg_sd, xend = 3, yend = mass_kg_mean - mass_kg_sd), size = 1.5, linetype = "dotted", color = so.pal[1]) +
  geom_segment(data = subset(stg.sum.1, sea_otter_tr == "low"), aes(x = 4, y = mass_kg_mean, xend = 6, yend = mass_kg_mean), size = 1.5 ,color = so.pal[2]) +
  geom_segment(data = subset(stg.sum.1, sea_otter_tr == "low"), aes(x = 4, y = mass_kg_mean + mass_kg_sd, xend = 6, yend = mass_kg_mean + mass_kg_sd), size = 1.5 , linetype = "dotted", color = so.pal[2]) +
  geom_segment(data = subset(stg.sum.1, sea_otter_tr == "low"), aes(x = 4, y = mass_kg_mean - mass_kg_sd, xend = 6, yend = mass_kg_mean - mass_kg_sd), size = 1.5 , linetype = "dotted", color = so.pal[2]) +
  scale_x_discrete(labels = c("High", "Low")) +
...
shn.m <- ggplot() +
  scale_color_manual(values = so.pal, "Sea otter region") +
  geom_segment(data = subset(shn.sum.1, sea_otter_tr == "high"), aes(x = 1, y = mass_kg_mean, xend = 3, yend = mass_kg_mean),size = 1.5, color = so.pal[1]) +
  geom_segment(data = subset(shn.sum.1, sea_otter_tr == "high"), aes(x = 1, y = mass_kg_mean + mass_kg_sd, xend = 3, yend = mass_kg_mean + mass_kg_sd), size = 1.5, linetype = "dotted", color = so.pal[1]) +
  geom_segment(data = subset(shn.sum.1, sea_otter_tr == "high"), aes(x = 1, y = mass_kg_mean - mass_kg_sd, xend = 3, yend = mass_kg_mean - mass_kg_sd), size = 1.5, linetype = "dotted", color = so.pal[1]) +
  geom_segment(data = subset(shn.sum.1, sea_otter_tr == "low"), aes(x = 4, y = mass_kg_mean, xend = 6, yend = mass_kg_mean), size = 1.5 ,color = so.pal[2]) +
  geom_segment(data = subset(shn.sum.1, sea_otter_tr == "low"), aes(x = 4, y = mass_kg_mean + mass_kg_sd, xend = 6, yend = mass_kg_mean + mass_kg_sd), size = 1.5 , linetype = "dotted", color = so.pal[2]) +
  geom_segment(data = subset(shn.sum.1, sea_otter_tr == "low"), aes(x = 4, y = mass_kg_mean - mass_kg_sd, xend = 6, yend = mass_kg_mean - mass_kg_sd), size = 1.5 , linetype = "dotted", color = so.pal[2]) +
  scale_x_discrete(labels = c("High", "Low")) +
...
snk.m <- ggplot() +
  scale_color_manual(values = so.pal, "Sea otter region") +
  geom_segment(data = subset(snk.sum.1, sea_otter_tr == "high"), aes(x = 1, y = mass_kg_mean, xend = 3, yend = mass_kg_mean),size = 1.5, color = so.pal[1]) +
  geom_segment(data = subset(snk.sum.1, sea_otter_tr == "high"), aes(x = 1, y = mass_kg_mean + mass_kg_sd, xend = 3, yend = mass_kg_mean + mass_kg_sd), size = 1.5, linetype = "dotted", color = so.pal[1]) +
  geom_segment(data = subset(snk.sum.1, sea_otter_tr == "high"), aes(x = 1, y = mass_kg_mean - mass_kg_sd, xend = 3, yend = mass_kg_mean - mass_kg_sd), size = 1.5, linetype = "dotted", color = so.pal[1]) +
  geom_segment(data = subset(snk.sum.1, sea_otter_tr == "low"), aes(x = 4, y = mass_kg_mean, xend = 6, yend = mass_kg_mean), size = 1.5 ,color = so.pal[2]) +
  geom_segment(data = subset(snk.sum.1, sea_otter_tr == "low"), aes(x = 4, y = mass_kg_mean + mass_kg_sd, xend = 6, yend = mass_kg_mean + mass_kg_sd), size = 1.5 , linetype = "dotted", color = so.pal[2]) +
  geom_segment(data = subset(snk.sum.1, sea_otter_tr == "low"), aes(x = 4, y = mass_kg_mean - mass_kg_sd, xend = 6, yend = mass_kg_mean - mass_kg_sd), size = 1.5 , linetype = "dotted", color = so.pal[2]) +
  scale_x_discrete(labels = c("High", "Low")) +
...
##### Panel - biomass
```{r}
cowplot::plot_grid(elg.m, epi.m, grz.all, ido.m, lmp.m, clm.all, but.m, mac.m, crb.all, grc.m, hel.m, rrc.m, fsh.all, stg.m, shn.m, snk.m, labels = c("a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "p", "q"), nrow = 4, ncol = 4, label_size = 10, align = "v", label_x = 0.26)
```
