---
title: "Eelgrass_crab_calculations_2018"
...
```{r data}
crab <- read.csv("../ALL_DATA/Crab_pots_2018_RAW.csv", stringsAsFactors = FALSE, header = TRUE)
...
crab <- merge(crab, crabLW, by = "sp_code") # this also removes the sea star at the same time which is nice
...
Reduce for by string export
```{r crab redu, echo = FALSE}
crab.redu <- cbind(crab[, c(1:17, 33)])
colnames(crab.redu)[14] <- "species_scientific"
colnames(crab.redu)[15] <- "taxon"
```
...
crab.site.sp <- crab %>% 
  filter(taxon.x == "Decapoda") %>% # make sure we only have crabs
...
colnames(crab.site)[1:7] <- c("site", "place_name", "date_set", "date_pulled", "date_set_julian", "date_pulled_julian", "time_soak_hr")
...
crab.site.sp.wide$shan_diverse <- diversity(crab.site.sp.wide[2:4], index = "shannon")
crab.site.sp.wide$simp_diverse <- diversity(crab.site.sp.wide[2:4], index = "simpson")
```

Now we can append it to the rest of the data
```{r append diversity to site}
crab.site <- merge(crab.site, crab.site.sp.wide[, c(1, 5:6)], by = "site")
```
...
write.csv(crab.site, "../../ALL_DATA/crab_simple_site_2018_derived.csv", row.names = FALSE)
write.csv(crab.site.sp, "../../ALL_DATA/crab_site_sp_2018_derived.csv", row.names = FALSE)

write.csv(crab.redu, "../ALL_DATA/crab_all_2018_dertived.csv", row.names = FALSE)
```