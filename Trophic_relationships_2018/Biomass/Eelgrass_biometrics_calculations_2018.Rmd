...
## Total Grazer mass ##
eg.bio$grazer_mass <- (eg.bio$idotea_mass + eg.bio$limpet_mass + eg.bio$caprellid_mass + eg.bio$amphipod_mass + eg.bio$gastropod_mass)

## Epiphyte load ##
eg.bio$epi_shoot_mass <- eg.bio$epi_mass / eg.bio$shoot_mass

## Grazer load ##
eg.bio$grz_shoot_mass <- eg.bio$grazer_mass / eg.bio$shoot_mass
...
site.bio <- eg.bio %>% 
  ...
  summarise(eg_shoot_mass = mean(shoot_mass),
            ...
            epiph_mass_shoot_mass = mean(epi_shoot_mass, na.rm =  TRUE),
            epiph_mass_shoot_mass_se = st.er(epi_shoot_mass, na.rm =  TRUE),
            grazer_mass_shoot_mass = mean(grz_shoot_mass, na.rm = TRUE),
            grazer_mass_shoot_mass_se = st.er(grz_shoot_mass, na.rm = TRUE))
...
site.tran <- eg.tran %>% 
  ...
  summarise(macro_cover = mean(macroalgae_cover_0.25msq, na.rm = TRUE),
            ...
            eelgrass_shoot_dens = mean(eelgrass_shoots_msq, na.rm = TRUE),
            eelgrass_shoot_dens_se = st.er(eelgrass_shoots_msq, na.rm = TRUE),
            flowering_shoot_dens = mean(flowering_shoots_msq, na.rm = TRUE),
            flowering_shoot_dens_se = st.er(flowering_shoots_msq, na.rm = TRUE),
...
## Export
```{r export}
write.csv(dat, "../../ALL_DATA/Eelgrass_biometrics_derived.csv", row.names = FALSE)
```
