...
fa_1.0_trs <- asin(sqrt(fa_1.0[, 3: 67]))

## Standardization ##
fa_0.5_trs_std <- scale(fa_0.5_trs, center = FALSE, scale = apply(fa_0.5_trs, 2, max))
fa_1.0_trs_std <- scale(fa_1.0_trs, center = FALSE, scale = apply(fa_1.0_trs, 2, max))
```

#### NMDS
```{r fa nmds, echo = FALSE}
## Calculate distances ##
fa_1.0_dist <- vegdist(fa_1.0_trs_std, method = "bray")
fa_0.5_dist <- vegdist(fa_0.5_trs_std, method = "bray")

## nmds ##
fa_1.0_mds <- metaMDS(fa_1.0_dist)
...
## Prep dataframe ##
fa_1.0_mds_dat <- as.data.frame(scores(fa_1.0_mds))
fa_1.0_mds_dat$ID <- fa_1.0$ID 
fa_1.0_mds_dat$Species_common <- fa_1.0$Species_common
fa_1.0_mds_dat <- merge(fa_1.0_mds_dat, bimrk[, c(1:4, 7, 9, 10)], by.x = "ID", by.y = "Biomarker_ID")
