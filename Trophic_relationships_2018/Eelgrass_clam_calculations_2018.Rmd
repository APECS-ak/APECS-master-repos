...
Reduce for by quadrat export
```{r clam redu, echp = FALSE}
clam.redu <- cbind(clam[, c(1:9, 12)])
```
...
clam.site.quad <- clam %>% 
  ...
  summarise(unique(place_name),
            unique(date),
            total_clam = n(),
            total_clam_m = total_clam * 16,
            clam_width = mean(width_mm), 
            clam_width_se = st.er(width_mm),
            clam_mass_m = mean(mass_g * 16),
            clam_mass_m_se = st.er(mass_g * 16))

# clean up column names
colnames(clam.site.quad)[3:4] <- c("place_name", "date")

clam.site <- clam.site.quad %>% 
  group_by(site) %>% 
  summarise(unique(place_name),
            unique(date),
            n_clam_m = mean(total_clam_m),
            n_clam_m_se = st.er(total_clam_m, na.rm = TRUE),
            clm_width = mean(clam_width), 
            clm_width_se = st.er(clam_width, na.rm = TRUE),
            clm_mass_m = mean(clam_mass_m),
            clm_mass_m_se = st.er(clam_mass_m, na.rm = TRUE))
```
...
## Export
By site summaries and by quadrat
```{r export simple summaries}
write.csv(dat, "../ALL_DATA/eelgrass_clam_2018_derived.csv", row.names = FALSE)
write.csv(clam.site.sp, "../ALL_DATA/eelgrass_clam_site_sp_2018.csv", row.names = FALSE)

write.csv(clam.redu, "../ALL_DATA/eelgrass_clam_all_2018_derived.csv", row.names = FALSE)
```
