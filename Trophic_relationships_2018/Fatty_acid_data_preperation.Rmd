...
## Batch 5 ##
b5_ac <- gather(b5_area, key = "FA", value = "Area", 2:61)
b5_ac$Conc <- gather(b5_conc, key = "FA", value = "Conc", 2:61)[,3]
b5_ac$Batch <- as.character(5)

## Batch 6 ##
b6_ac <- gather(b6_area, key = "FA", value = "Area", 2:72)
b6_ac$Conc <- gather(b6_conc, key = "FA", value = "Conc", 2:72)[,3]
...
For a sample, calculate the proportion of each unique FA as the area of that FA divided my the total area of all FAs of that sample. NOTE that C19 is removed for this calculation
...
## Summary by species form greater than 0.5% data ##
FA_ac_0.5 <- merge(FA_ac_0.5, samp_mass_redu[, c(1, 2)], by.x = "ID", by.y = "Biomarker_ID")
...
## Summary by species form greater than 1.0% data ##
FA_ac_1.0 <- merge(FA_ac_1.0, samp_mass_redu[, c(1, 2)], by.x = "ID", by.y = "Biomarker_ID")
...
## Export ##
sp_prop_summary_0.5 <- cbind(sp_prop_summary_0.5[, c(1, 2, 8, 3:7)])
write.csv(sp_prop_summary_0.5, "Species_FA_summaries.csv", row.names = FALSE)
```
...
## 0.5% filter data ##
FA_prop_0.5 <- spread(FA_ac_0.5[, c(1, 4, 10, 11)], key = FA, value = Prop_area_0.5)

FA_prop_0.5[is.na(FA_prop_0.5)] <- 0

## 1.0 % filter data ##
FA_prop_1.0 <- spread(FA_ac_1.0[, c(1, 4, 10, 11)], key = FA, value = Prop_area_1.0)
