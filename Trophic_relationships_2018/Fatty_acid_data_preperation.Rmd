...
Filter out B_226 from the coccolithophore bloom
```{r}
b6_area <- b6_area %>% 
  filter(!str_detect(Sample, "^B_226"))
...
## Batch 5 ##
b5_ac <- gather(b5_area, key = "FA", value = "Area", 2:59)
b5_ac$Conc <- gather(b5_conc, key = "FA", value = "Conc", 2:59)[,3]
b5_ac$Batch <- as.character(5)

## Batch 6 ##
b6_ac <- gather(b6_area, key = "FA", value = "Area", 2:65)
b6_ac$Conc <- gather(b6_conc, key = "FA", value = "Conc", 2:65)[,3]
b6_ac$Batch <- as.character(6)

## Batch 7 ##
b7_ac <- gather(b7_area, key = "FA", value = "Area", 2:57)
b7_ac$Conc <- gather(b7_conc, key = "FA", value = "Conc", 2:57)[,3]
...
write
```{r}
write.csv(FA_ac, "FA_all_batch_RAW.csv", row.names = FALSE)
```
...
# Unique FAs #
FAs_1.0 <- unique(FA_a_1.0$FA)
FAs_1.0

FAs_0.5 <- unique(FA_a_0.5$FA)
FAs_0.5

setdiff(FAs_0.5, FAs_1.0)

# Filter Master FA data by unique FA >= 1% #
FA_ac_filt_1.0 <- FA_ac %>%
  filter(FA %in% FAs_1.0) %>% 
  group_by(ID) %>%
  mutate(Total_filt_area = sum(Area))

FA_ac_filt_1.0$Prop_filt_area <- FA_ac_filt_1.0$Area / FA_ac_filt_1.0$Total_filt_area

# Filter Master FA data by unique FA >=0.5% #
FA_ac_filt_0.5 <- FA_ac %>%
  filter(FA %in% FAs_0.5) %>% 
  group_by(ID) %>%
  mutate(Total_filt_area = sum(Area))

FA_ac_filt_0.5$Prop_filt_area <- FA_ac_filt_0.5$Area / FA_ac_filt_0.5$Total_filt_area
```
...
## 1.0% Filter ##
FA_prop_1.0 <- spread(FA_ac_filt_1.0[, c(1, 3, 4, 7, 8, 9, 10, 14)], key = FA, value = Prop_filt_area)
FA_prop_1.0[is.na(FA_prop_1.0)] <- 0

FA_conc_1.0 <- spread(FA_ac_filt_1.0[, c(1, 3, 4, 6, 7, 8, 9, 10)], key = FA, value = Conc)
FA_conc_1.0[is.na(FA_conc_1.0)] <- 0

## 0.5 Filter ##
FA_prop_0.5 <- spread(FA_ac_filt_0.5[, c(1, 3, 4, 7, 8, 9, 10, 14)], key = FA, value = Prop_filt_area)
FA_prop_0.5[is.na(FA_prop_0.5)] <- 0

FA_conc_0.5 <- spread(FA_ac_filt_0.5[, c(1, 3, 4, 6, 7, 8, 9, 10)], key = FA, value = Conc)
FA_conc_0.5[is.na(FA_conc_0.5)] <- 0
```
...
write.csv(FA_conc_1.0, "../ALL_DATA/FA_concentrations_1.0filter_2018_derived.csv", row.names = FALSE)

## 0.5% filter ##
write.csv(FA_prop_0.5, "../ALL_DATA/FA_proportions_0.5filter_2018_derived.csv", row.names = FALSE)
write.csv(FA_conc_0.5, "../ALL_DATA/FA_concentrations_0.5filter_2018_derived.csv", row.names = FALSE)
```
