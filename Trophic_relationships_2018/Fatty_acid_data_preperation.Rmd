...
1) Starting with data in proportions, melt wide-format FA so each sample's measure for each FA has its own row.
2) Aggregate data by sample type, Lz presence, and FA (in your case probs tissue and FA).
   Here, you should have a data frame with the means of each FA by each tissue type
3) Subset the data frame by each tissue type and where the mean is >1% (e.g. df_crab=subset(df, dftissue=="crab" & dfmean >1) ).  I'm sure there's a better way to do this part...
   Here, you should have J number of data frames, where J is the number of tissue types you have
4) Create a list of FA with >1% by combining the FA listed in each new df.
   e.g. FA_list=as.list(as.character(df_crabFA), as.character(df_slugFA), as.character(df_CornishGameHen$FA)) and whatnot
5) Pare that list down to unique FA (e.g. FA_list=unique(FA_list) )
6) Use the list to subset your melted data frame, only including FA on the list (e.g. df_final=df[df$FA %in% FA_list,] )
7) Cast the final data frame into wide format to use for multivariate analyses.
...
## 1.0 % filter data ##
FA_prop_1.0 <- spread(FA_ac_1.0[, c(1, 3, 4, 9, 13)], key = FA, value = Prop_area_1.0)

FA_prop_1.0[is.na(FA_prop_1.0)] <- 0

```

New summary with zeros
```{r wide to tall zero, echo=FALSE}
sp_prop_summary_1.0.2 <- FA_prop_1.0 %>% 
  gather(key = "FA", value = "Prop", 4:56) %>% 
  group_by(Species_common, FA) %>% 
  summarise(n = n(),
