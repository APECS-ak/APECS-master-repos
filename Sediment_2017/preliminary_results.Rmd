...
hlinedat_om <- ddply(df.core_noL06, "otter_region", summarize, regionmean_om = mean(om.im_ratio, na.rm = TRUE))

# plot organic matter with sites
plot_omima = ggplot(df.core_noL06, aes(layer_depth, om.im_ratio, color = site_n)) +
  stat_summary(fun.y=mean, geom="point") +
  stat_summary(fun.data=mean_se, geom="errorbar", width=0.5) +
  coord_flip() +
  scale_x_reverse() +
...
# plot organic matter averaged across sites
plot_omimb = ggplot(df.core_noL06, aes(layer_depth, om.im_ratio)) +
  stat_summary(fun.y=mean, geom="point")+
  stat_summary(fun.data=mean_se, geom="errorbar", width=0.5) +
  coord_flip() +
  scale_x_reverse() +
...
# organize om and im side-by-side
plot_grid(plot_omima, plot_omimb, align = "hv", ncol = 2)
```
...
title.cp01 <- ggdraw() + draw_label(".", fontface='bold')

cp01 <- plot_grid(p01, p02, labels=c('A', 'B'), align = 'h', ncol=2, nrow=1, rel_widths = c(1,1))
...
>>>>>>> 5190decfe74e29772a0e476713fa6e85c6b65e25
```{r, echo = FALSE, include = FALSE, warning = FALSE}
...
```{r, echo = FALSE, include = FALSE, warning = FALSE}
df.core.top <-subset(df.core, layer_depth <= 14) # select only top 20cm of core
>>>>>>> e0c293cf278fd3d2038dcf8b128e9919846397af
...
=======
=======
df.core.trimtop <- df.core.trimtop[-c(2,3,4,7,8,9,10,11,12,13,16,17,18,19,21),]
...
```{r, include = FALSE}
test <- aggregate(om_density ~ site, df.core, function(x) c(Var = var(x)))
test2 <- aggregate(layer_thickness ~ site, df.core, function(x) c(Meandepth = mean(x)))

df.var <- left_join(test, test2, by = c("site"))

df.var <- df.var[-6,]


varplot = ggplot(df.var, aes(layer_thickness, om_density)) +
>>>>>>> 5190decfe74e29772a0e476713fa6e85c6b65e25
>>>>>>> e0c293cf278fd3d2038dcf8b128e9919846397af
  geom_point(size=3) +
  #geom_smooth(method=lm, formula=y~x, se=FALSE, fullrange=FALSE) +
