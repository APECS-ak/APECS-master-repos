...
cr.mat <- read.csv("https://raw.githubusercontent.com/APECS-ak/APECS-master-repos/master/ALL_DATA/core_matter_2017_CLEAN.csv", stringsAsFactors = FALSE, header = TRUE)
...
```{r}
cr.mat = cr.mat %>% 
  select(-sed_ww_pre_60C, -sed_dw_post_60C, -sed_dw_pre_450C, -sed_dw_post_450C, -sed_dw_pre_950C, -sed_dw_post_950C)
```
...
# plot organic matter
plot_om_all = ggplot(cr.mat, aes(core_depth, om_content)) +
...
plot_im_all = ggplot(cr.mat, aes(core_depth, im_content)) +
  stat_summary(fun.y=mean, geom="point")+
  stat_summary(fun.data=mean_se, geom="errorbar", width=0.5) +
  coord_flip() +
  scale_x_reverse() +
...
# plot organic matter
plot_om_sites_exL06 = ggplot(cr.mat_noL06, aes(core_depth, om_content)) +
  stat_summary(fun.y=mean, geom="point")+
  stat_summary(fun.data=mean_se, geom="errorbar", width=0.5) +
  coord_flip() +
  scale_x_reverse() +
...
# plot inorganic matter
plot_im_sites_exL06 = ggplot(cr.mat_noL06, aes(core_depth, im_content)) +
  stat_summary(fun.y=mean, geom="point")+
  stat_summary(fun.data=mean_se, geom="errorbar", width=0.5) +
  coord_flip() +
  scale_x_reverse() +
...
# plot organic matter
plot_om = ggplot(cr.mat_noL06, aes(core_depth, om_content)) +
  stat_summary(fun.y=mean, geom="point")+
  stat_summary(fun.data=mean_se, geom="errorbar", width=0.5) +
  coord_flip() +
  scale_x_reverse() +
...
plot_im = ggplot(cr.mat_noL06, aes(core_depth, im_content)) +
  stat_summary(fun.y=mean, geom="point")+
  stat_summary(fun.data=mean_se, geom="errorbar", width=0.5) +
  coord_flip() +
  scale_x_reverse() +
