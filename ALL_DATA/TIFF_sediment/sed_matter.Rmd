...
t.test(leaf_dw_g_m2 ~ so_region2, paired=FALSE, data = df.all)
t.test(tot_biomass_dw_g_m2 ~ sed2, paired=FALSE, data = df.all)

df.all$site <- factor(df.all$site, levels = df.all$site[order(df.all$sea_otter_index)])

pz4 = ggplot(df.all, aes(x = sea_otter_index, y = organic_dw_percent, color = sed2)) +
  ...
  #geom_text(aes(label=transect), size=2) +
  facet_wrap(~transect) +
...
pz4 = ggplot(df.all, aes(x = reorder(site, sea_otter_index), y = organic_dw_percent, color = transect)) +
  geom_point(size = 3) + # allows for trendline on general trend
  #geom_smooth(method=lm, formula=y ~ poly(x,2), se=FALSE, fullrange=FALSE) +
  geom_smooth(method=lm, formula= y~x, se=FALSE, fullrange=FALSE) +
  #geom_smooth(method=lm, formula = -(log(y) ~ x), se=FALSE, fullrange=FALSE) +
...
```{r}
```{r}
write.csv(df.all, "/Users/tiff/Desktop/R Studio/APECS-master-repos/ALL_DATA/TIFF_sediment/summary_2018_matter.csv", row.names = TRUE)
```
