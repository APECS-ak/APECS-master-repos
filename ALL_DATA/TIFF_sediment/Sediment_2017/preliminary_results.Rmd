...
df.core <- read.csv("~/Desktop/R Studio/APECS-master-repos/ALL_DATA/DATA_ARCHIVING/2017/sediment_matter_2017/sediment_matter_2017_CLEANDATA.csv", stringsAsFactors = FALSE, header = TRUE)
# check 
str(df.core)
```
...
locations_all = df.core %>%
  distinct(df.core, latitude_N, longitude_E, site_name) %>%
  drop_na()

# map "locations_all"
leaflet(locations_all) %>%
  addTiles() %>%
  addMarkers(~longitude_E, ~latitude_N, popup = ~site_name, label = ~site_name)
```
<br><br><br><br><br>
...
```{r}
df.core = df.core[-c(195, 276, 365, 403, 470, 482, 484, 513, 515, 516),] # rows that seem really high or low
...
# plot organic matter
plot_om_all = ggplot(df.core_noL06, aes(layer_depth_cm, organic_dw_percent)) +
  stat_summary(fun.y=mean, geom="point")+
  stat_summary(fun.data=mean_se, geom="errorbar", width=0.5) +
  coord_flip() +
  scale_x_reverse() +
  xlab("Core Depth (cm)") +
  ylab("Organic Matter (% dry weight)") +
  #scale_x_continuous(position = "top") +
  #scale_y_continuous(position = "left") +
  facet_wrap(~site_name, ncol = 1)

# plot inorganic matter
plot_im_all = ggplot(df.core_noL06, aes(layer_depth_cm, inorganic_dw_percent)) +
  stat_summary(fun.y=mean, geom="point")+
  stat_summary(fun.data=mean_se, geom="errorbar", width=0.5) +
  coord_flip() +
  scale_x_reverse() +
  xlab("Core Depth (cm)") +
  ylab("Inorganic Matter (% dry weight)") +
  #scale_x_continuous(position = "top") +
  #scale_y_continuous(position = "left") +
  facet_wrap(~site_name, ncol = 1)
