...
h2o.utm <- readOGR("~/Desktop/R Studio/APECS-master-repos/ALL_DATA/spatial_data", layer = "POW_water_UTM")
...
```{r site}
site <- structure(list(id = "site", longitude = -133.044890, latitude = 55.415907), .Names = c("id", "longitude", "latitude"), class = "data.frame", row.names = c(NA, -1L))
xy <- site[,c(2,3)]
...
#df.otts <- read.csv("/Users/tiff/Desktop/R Studio/APECS-master-repos/sea_otter_index_2018/sea_otter_counts_2018_stephens_1.csv", stringsAsFactors = FALSE, header = TRUE)
...
```{r}
buff <- gBuffer(site, byid = TRUE, width = 92600, quadsegs = 10) # create buffer
...
wt <- transition(wr, transitionFunction = function(x){50}, directions = 8) # convert raster to transition layer (required for accCost() below)
...
survey.poly <- rasterToPolygons(sa, dissolve = TRUE) # convert raster to polygon
...
leaflet() %>% 
  ...
  addPolygons(data = survey.poly.latlong, stroke = NA, fillColor = "lightseagreen", fillOpacity = 0.75, group = "POW") %>%
  addCircleMarkers(data = site, ~longitude, ~latitude, color = "deeppink", fillOpacity = 1, stroke = FALSE, radius = 7) #%>% 
  #addCircleMarkers(data = df.otts, ~longitude, ~latitude, color = "yellow", stroke = FALSE, radius = 3, fillOpacity = 1)


a <- (2500 * length(survey.poly.utm) / 1000000) # caculate area of the polygon in sq km. 2500 becasue 50*50=2500
