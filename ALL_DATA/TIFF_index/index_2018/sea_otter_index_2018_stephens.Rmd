...
df.otts <- df.otts[ -c(5) ] # remove unneccessary columns
```
...
colnames(df.shells)[2] <- "shell_n_otter" # change column names
colnames(df.shells)[3] <- "shell_prop_otter" # change column names
...
```{r}
# rename things
colnames(df.index)[7] <- "site_water_area" # change column names
colnames(df.index)[10] <- "so_pop_1" # change column names
colnames(df.index)[11] <- "so_pop_2" # change column names
colnames(df.index)[15] <- "so_dens_1" # change column names
colnames(df.index)[16] <- "so_dens_2" # change column names
colnames(df.index)[17] <- "so_dens_avg" # change column names
```
...
```{r data exploration}
# Correlations
df.index <- df.index[ -c(9,11,12,13,14,15,16,18) ] # remove columns that are no longer necessary

corrgram(df.index[, 8:13])
pairs(df.index[,8:13])
```
...
hist(df.index$shell_n_otter)
hist(df.index$shell_prop_otter)

# log transformation
hist(log(df.index$so_dens_avg + 1))
hist(log(df.index$pits_all + 1))
hist(log(df.index$shell_n_otter + 1))
hist(log(df.index$shell_prop_otter + 1))
```
...
colnames(final.dat)[14] <- "so_index_pc1"
colnames(final.dat)[15] <- "so_index_pc2"
...
```{r export}
# Data Export
write.csv(final.dat, "/Users/tiff/Desktop/R Studio/APECS-master-repos/ALL_DATA/TIFF_index/index_2018/so_index_stephens.csv", row.names = FALSE)
```
