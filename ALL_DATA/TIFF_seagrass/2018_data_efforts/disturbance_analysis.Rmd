...
\OtterMetrics
```{r}
index <- read.csv("/Users/tiff/Desktop/R Studio/APECS-master-repos/ALL_DATA/TIFF_index/index_2018/so_index_stephens.csv", stringsAsFactors = FALSE, header = TRUE)
```
...
colnames(df.site)[6] <- "sed1_category" # change column names
colnames(df.site)[7] <- "sed2_category" # change column names
...
df.site <- df.site[, -c(23:25, 50, 51)] # remove these columns

colnames(df.site)[2] <- "longitude" # change column names
colnames(df.site)[3] <- "latitude" # change column names
colnames(df.site)[9] <- "date_so1_MM.DD.YY" # change column names
colnames(df.site)[10] <- "date_so2_MM.DD.YY" # change column names
...
write.csv(df.sgdens.means, "/Users/tiff/Desktop/density_stephens.csv", row.names = FALSE)
```
...
pf1 <- ggplot(df.alldat, aes(so_region2, fucus_mean_elev_mllw_cm, color = so_region2)) +
...
pf2 = ggplot(df.alldat, aes(fucus_mean_elev_mllw_cm, edge_diffuse_elev_mllw_cm)) +
...
pz1 <- ggplot(df.alldat, aes(so_region2, edge_diffuse_elev_corrct)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(alpha = 0.3, width = 0.15) +
  theme(legend.position="right") +
  #theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 0)) +
...
pz2 <- ggplot(df.alldat, aes(so_region2, edge_continuous_elev_corrct)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(alpha = 0.3, width = 0.15) +
  theme(legend.position="right") +
  #theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 0)) +
...
#t.test(edge_continuous_elev_mllw_cm ~ so_region2, paired=FALSE, data = df.alldat)
```
...
pz3 = ggplot(df.alldat, aes(edge_continuous_elev_corrct, edge_diffuse_elev_corrct, color = so_region2)) +
...
#df.alldat <- df.alldat[-c(6),] # port refugio removed



pz4 = ggplot(df.alldat, aes(soi1_pc2, edge_diffuse_elev_mllw_cm, color = sed1_category)) +
  geom_point(size=3) + # allows for trendline on general trend
...
pz5 = ggplot(df.alldat, aes(pits_all, edge_continuous_elev_mllw_cm)) +
  geom_point(size=3) + # allows for trendline on general trend
  geom_smooth(method=lm, formula=y ~ poly(x,2), se=FALSE, fullrange=FALSE) +
  #geom_smooth(method=lm, formula=y~x, se=FALSE, fullrange=FALSE) +
  #geom_smooth(method=lm, formula = -(log(y) ~ x), se=FALSE, fullrange=FALSE) +
...
```{r, out.width="50%"}
df.alldat = df.alldat %>% 
  mutate(edge_mean_elev_mllw_cm = (edge_diffuse_elev_mllw_cm + edge_continuous_elev_mllw_cm) /2)

pz6 = ggplot(df.alldat, aes(so_index, edge_mean_elev_mllw_cm, color = sed2_qual)) +
  geom_point(size=3) + # allows for trendline on general trend
  geom_smooth(method=lm, formula=y ~ poly(x,2), se=FALSE, fullrange=FALSE) +
  #geom_smooth(method=lm, formula=(log(y+60) ~ x), se=FALSE, fullrange=FALSE) +
  #geom_smooth(method=lm, formula = -(log(y) ~ x), se=FALSE, fullrange=FALSE) +
...
pz6 <- ggplot(df.alldat, aes(so_region2, diffuse_band_height_m2)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(alpha = 0.3, width = 0.15) +
  theme(legend.position="right") +
  #theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 0)) +
...
pz7 = ggplot(df.alldat, aes(so_index, diffuse_band_distance_cm, color = sed2_qual)) +
  geom_point(size=3) + # allows for trendline on general trend
  #geom_smooth(method=lm, formula=y ~ x, se=FALSE, fullrange=FALSE) +
  #xlab("Sea Otter Index")+ylab("Continuous edge elevation (cm, MLLW)") +
  #theme(legend.title=element_text(size=8), legend.text=element_text(size=8)) +
...
pz8 = ggplot(df.alldat, aes(dens_otter1, edge_diffuse_elev_mllw_cm, color = so_index)) +
  geom_point(size=3) + # allows for trendline on general trend
  #geom_smooth(method=lm, formula=(y ~ x), se=FALSE, fullrange=FALSE) +
  #geom_smooth(method=lm, formula=(log(y+60) ~ x), se=FALSE, fullrange=FALSE) +
  #geom_smooth(method=lm, formula = -(log(y) ~ x), se=FALSE, fullrange=FALSE) +
...
pz9 = ggplot(df.alldat, aes(dens_otter2, edge_diffuse_elev_mllw_cm, color = so_index)) +
  geom_point(size=3) + # allows for trendline on general trend
  #geom_smooth(method=lm, formula=(y ~ x), se=FALSE, fullrange=FALSE) +
  #geom_smooth(method=lm, formula=(log(y+60) ~ x), se=FALSE, fullrange=FALSE) +
  #geom_smooth(method=lm, formula = -(log(y) ~ x), se=FALSE, fullrange=FALSE) +
...
pz10 = ggplot(df.alldat, aes(dens_otter1, edge_continuous_elev_mllw_cm, color = so_index)) +
  geom_point(size=3) + # allows for trendline on general trend
  #geom_smooth(method=lm, formula=(y ~ x), se=FALSE, fullrange=FALSE) +
  #geom_smooth(method=lm, formula=(log(y+60) ~ x), se=FALSE, fullrange=FALSE) +
  #geom_smooth(method=lm, formula = -(log(y) ~ x), se=FALSE, fullrange=FALSE) +
...
pz11 = ggplot(df.alldat, aes(dens_otter2, edge_continuous_elev_mllw_cm, color = so_index)) +
  geom_point(size=3) + # allows for trendline on general trend
  #geom_smooth(method=lm, formula=(y ~ x), se=FALSE, fullrange=FALSE) +
  #geom_smooth(method=lm, formula=(log(y+60) ~ x), se=FALSE, fullrange=FALSE) +
  #geom_smooth(method=lm, formula = -(log(y) ~ x), se=FALSE, fullrange=FALSE) +
...
pz12 = ggplot(df.alldat, aes(pop_dens_surv_km2, edge_mean_elev_mllw_cm, color = so_index)) +
  geom_point(size=3) + # allows for trendline on general trend
  geom_smooth(method=lm, formula=y ~ poly(x,2), se=FALSE, fullrange=FALSE) +
  #geom_smooth(method=lm, formula=(log(y+60) ~ x), se=FALSE, fullrange=FALSE) +
  #geom_smooth(method=lm, formula = -(log(y) ~ x), se=FALSE, fullrange=FALSE) +
...
pz14 = ggplot(df.alldat, aes(pits_site_sum, edge_mean_elev_mllw_cm, color = so_index)) +
  geom_point(size=3) + # allows for trendline on general trend
  geom_smooth(method=lm, formula=y ~ poly(x,2), se=FALSE, fullrange=FALSE) +
  #geom_smooth(method=lm, formula=(log(y+60) ~ x), se=FALSE, fullrange=FALSE) +
  #geom_smooth(method=lm, formula = -(log(y) ~ x), se=FALSE, fullrange=FALSE) +
...
pz15 = ggplot(df.alldat, aes(pits_site_sum, log(edge_mean_elev_mllw_cm + 62), color = so_index)) +
  geom_point(size=3) + # allows for trendline on general trend
  geom_smooth(method=lm, formula=y ~ x, se=FALSE, fullrange=FALSE) +
  #geom_smooth(method=lm, formula=(log(y+60) ~ x), se=FALSE, fullrange=FALSE) +
  #geom_smooth(method=lm, formula = -(log(y) ~ x), se=FALSE, fullrange=FALSE) +
...
pz16 = ggplot(df.alldat, aes(prop_otter_shells, edge_mean_elev_mllw_cm, color = so_index)) +
  geom_point(size=3) + # allows for trendline on general trend
  geom_smooth(method=lm, formula=y ~ x, se=FALSE, fullrange=FALSE) +
  #geom_smooth(method=lm, formula=(log(y+60) ~ x), se=FALSE, fullrange=FALSE) +
  #geom_smooth(method=lm, formula = -(log(y) ~ x), se=FALSE, fullrange=FALSE) +
...
pz17 = ggplot(df.alldat, aes(prop_otter_shells, log(edge_mean_elev_mllw_cm + 62), color = so_index)) +
  geom_point(size=3) + # allows for trendline on general trend
  geom_smooth(method=lm, formula=y ~ x, se=FALSE, fullrange=FALSE) +
  #geom_smooth(method=lm, formula=(log(y+60) ~ x), se=FALSE, fullrange=FALSE) +
  #geom_smooth(method=lm, formula = -(log(y) ~ x), se=FALSE, fullrange=FALSE) +
...
```{r, out.width="25%"}
df.alldat$sed1_qual=factor(df.alldat$sed1_qual, levels=c("mud","sugar","sand")) # order
df.alldat$sed1_qua2=factor(df.alldat$sed2_qual, levels=c("mud","sugar","sand")) # order

linear.model <- (y ~ x)
poly.model <- y ~ poly(x,2)

pz18 = ggplot(df.alldat, aes(sed1_qual, edge_mean_elev_mllw_cm, color = so_index)) +
  geom_point(size=3) + # allows for trendline on general trend
  geom_smooth(method=lm, formula=y ~ poly(x,2), se=FALSE, fullrange=FALSE) +
  #geom_smooth(method=lm, formula=(log(y+60) ~ x), se=FALSE, fullrange=FALSE) +
  #geom_smooth(method=lm, formula = -(log(y) ~ x), se=FALSE, fullrange=FALSE) +
...
pz19 = ggplot(df.alldat, aes(sed2_site_avg, log(edge_mean_elev_mllw_cm + 62), color = so_index)) +
  geom_point(size=3) + # allows for trendline on general trend
  #geom_smooth(method=lm, formula=y ~ poly(x,2), se=FALSE, fullrange=FALSE) +
  #geom_smooth(method=lm, formula=(log(y+60) ~ x), se=FALSE, fullrange=FALSE) +
  #geom_smooth(method=lm, formula = -(log(y) ~ x), se=FALSE, fullrange=FALSE) +
...
df.alldat2 <- df.alldat[c(17,9,24,2,21,25,22,13,5,19),] # only mid 8


mean(df.alldat2$edge_mean_elev_mllw_cm)

aggregate(edge_mean_elev_mllw_cm ~ so_region2, data = df.alldat2, mean, na.rm = TRUE)

((3.98825) - (-19.03478)) # all sites
(-7.632094) - (-26.761308) # sugar sites gone
(-4.154955) - (-27.782894) # phyllo sites removed
(-12.88596) - (-32.17956) # low and high 8
((26.487192) + (7.254782)) /2 # only mid 8

(-12.88596) - (16.87099) # bottom and mid
(-32.17956) - (16.87099) # high and mid
...
area.potential.loss.m2 <- (sqrt((--49.05055^2) + ((--49.05055 / 0.0555911)^2))) / 100
```
...
pz20 = ggplot(df.alldat, aes(beach_slope, edge_mean_elev_mllw_cm)) +
...
pz21 = ggplot(df.alldat, aes(edge_continuous_elev_mllw_cm, shoot_density_continuous_m2)) +
  geom_point(aes(color = so_region2), size=3) + # allows for trendline on general trend
  geom_smooth(method=lm, formula=y ~ x, color = "black", se=FALSE, fullrange=FALSE) +
  #geom_smooth(method=lm, formula=y ~ x, color = "black", se=FALSE, fullrange=FALSE) +
  #geom_smooth(method=lm, formula=(log(y+60) ~ x), se=FALSE, fullrange=FALSE) +
