...
First, here is a schematic of what elevation data we collected in the field: 
...
pf2 = ggplot(df.alldat, aes(fucus_upper_elev_mllw_cm, fucus_lower_elev_mllw_cm)) +
  geom_point(aes(color = so_region2), size=3) +
...
pz1 <- ggplot(df.alldat, aes(so_region2, edge_diffuse_elev_mllw_cm)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(alpha = 0.3, width = 0.15) +
  theme(legend.position="right") +
  #theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 0)) +
...
#t.test(edge_diffuse_elev_mllw_cm ~ so_region2, paired=FALSE, data = df.alldat)



pz2 <- ggplot(df.alldat, aes(so_region2, edge_diffuse_elev_mllw_cm)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(alpha = 0.3, width = 0.15) +
  theme(legend.position="right") +
  #theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 0)) +
...
pz3 = ggplot(df.alldat, aes(edge_continuous_elev_corrct, edge_diffuse_elev_corrct, color = so_region2)) +
  ...
  #geom_smooth(method=lm, formula=(log(y+60) ~ x), se=FALSE, fullrange=FALSE) +
  #geom_smooth(method=lm, formula = -(log(y) ~ x), se=FALSE, fullrange=FALSE) +
  xlab("Continuous edge (cm, MLLW)")+ylab("Diffuse edg elevatione (cm, MLLW)") +
...
pz4 = ggplot(df.alldat, aes(so_index, edge_diffuse_elev_mllw_cm)) +
  geom_point(size=3) + # allows for trendline on general trend
...
pz5 = ggplot(df.alldat, aes(so_index, edge_continuous_elev_mllw_cm)) +
  geom_point(size=3) + # allows for trendline on general trend
  geom_smooth(method=lm, formula=y ~ poly(x,2), se=FALSE, fullrange=FALSE) +
  #geom_smooth(method=lm, formula=(log(y+60) ~ x), se=FALSE, fullrange=FALSE) +
  #geom_smooth(method=lm, formula = -(log(y) ~ x), se=FALSE, fullrange=FALSE) +
...
pz6 = ggplot(df.alldat, aes(so_index, edge_mean_elev_mllw_cm)) +
  geom_point(size=3) + # allows for trendline on general trend
  geom_smooth(method=lm, formula=y ~ poly(x,2), se=FALSE, fullrange=FALSE) +
  #geom_smooth(method=lm, formula=(log(y+60) ~ x), se=FALSE, fullrange=FALSE) +
  #geom_smooth(method=lm, formula = -(log(y) ~ x), se=FALSE, fullrange=FALSE) +
...
pz7 = ggplot(df.alldat, aes(so_index, diffuse_band_area_m2)) +
  geom_point(size=3) + # allows for trendline on general trend
  #geom_smooth(method=lm, formula=y ~ poly(x,2), se=FALSE, fullrange=FALSE) +
  #geom_smooth(method=lm, formula=(log(y+60) ~ x), se=FALSE, fullrange=FALSE) +
  #geom_smooth(method=lm, formula = -(log(y) ~ x), se=FALSE, fullrange=FALSE) +
