...
df.all <- left_join(df.all, density, by = c("site"))
head(df.all)
```
...
df.all = df.all %>% 
  mutate(pit_shelln1 = pits_site_sum * shells_n_otter) %>%
  mutate(pit_shelln2 = (pits_site_sum / shells_n_otter) * shells_n_otter) %>%
  mutate(hmm = pit_shellprop * shells_n_otter)
...
pz4 = ggplot(df.all, aes((pits_site_sum/2), shells_n_otter, color = so_region1)) +
  geom_point(size=3) + # allows for trendline on general trend
