---
title: "New progress on Tiff's SOI and Disturbance"
...
pz4 = ggplot(df.all, aes(so_index_pc1, edge_diffuse_elev_corrct_neg, color = sed2_qual)) +
...
#df.all <- df.all[-c(20),] # remove port refugio

edge_continuous_elev_corrct_neg

pz5 = ggplot(df.all, aes(so_index_pc2, shoot_density_continuous_m2, color = sed2_qual)) +
  geom_point(size=3) + # allows for trendline on general trend
  #geom_smooth(method=lm, formula=y ~ poly(x,2), se=FALSE, fullrange=FALSE) +
