...
star.mean <- star %>%
  summarise(Cmean=mean(MeanC), Nmean=mean(MeanN), Csd=sd(MeanC), Nsd=sd(MeanN),
            Cse=sd(MeanC)/sqrt(length(MeanC)), Nse=sd(MeanN)/sqrt(length(MeanN)))
star.mean$PreyCat <- "Star"

si.mean <- full_join(si.mean, star.mean)
