...
#load whisker data
whisker <- read.csv("SI/whisker_final.csv")

#OtterID loads as number - want it to be a factor
whisker$OtterID<-as.factor(whisker$OtterID)
whisker$Season <- factor(whisker$Season , levels=c("Spring", "Summer", "Fall", "Winter"))


#################################################################################
#####                   Initial Prey Visualizations                         #####
#################################################################################

# plot of all species Nitrogen
ggplot(data=si.prey, aes(x=N)) + 
  geom_histogram(binwidth = .5) +
  facet_wrap(vars(Species))

#plot of all species Carbon
ggplot(data=si.prey, aes(x=C)) + 
  geom_histogram(binwidth = .5) +
  facet_wrap(vars(Species))

#bi-plot by prey cat
ggplot(data=si.prey, aes(x=C, y=N, color=PreyCat)) + 
  geom_point() 
  
#now all normalized carbon
ggplot(data=si.prey, aes(x=Cnorm)) + 
  geom_histogram(binwidth = .5) +
  facet_wrap(vars(Species))
...
ggplot(data= filter(si.prey, PreyCat== "Clam"), aes(x=Cnorm, y=N)) +
...
#site
ggplot(data= filter(si.prey, PreyCat== "Clam"), aes(x=Size, y=Cnorm)) +
  geom_point(aes(color=Site, shape= Species)) +
...
ggplot(data= filter(si.prey, PreyCat== "Crab"), aes(x=Cnorm, y=N)) +
  geom_point(aes(color=Site, shape=Species)) +
  labs(x=expression(paste(delta^13, "C (\u2030)")), 
       y=expression(paste(delta^15, "N (\u2030)" ))) +
  stat_ellipse(mapping=aes(color=Site)) +
...
ggplot(data= filter(si.prey, PreyCat== "Urchin"), aes(x=Cnorm, y=N)) +
...
#SNAIL and CUCUMBER
ggplot(data= filter(si.prey, PreyCat== "Cucumber.Snail"), aes(x=Cnorm, y=N)) +
...
ggplot(data= filter(si.prey, Species == "pio" | Species == "evt"), aes(x=Cnorm, y=N)) +
...
ggplot(data= filter(si.prey, Site=="Soda Bay" & PreyCat == "Clam"), aes(x=Cnorm, y=N)) +
  geom_point(aes(color=Season, shape= Species)) +
  labs(x=expression(paste(delta^13, "C (\u2030)")), 
       y=expression(paste(delta^15, "N (\u2030)" ))) +
  stat_ellipse(mapping=aes(color=Season)) +
...
ggplot(data= filter(si.prey, Site=="Craig" & PreyCat == "Clam"), aes(x=Cnorm, y=N)) +
  geom_point(aes(color=Season, shape= Species)) +
  labs(x=expression(paste(delta^13, "C (\u2030)")), 
       y=expression(paste(delta^15, "N (\u2030)" ))) +
  stat_ellipse(mapping=aes(color=Season)) +
...
ggplot(data = filter(si.prey, PreyCat == "Mussel"), aes(x=Cnorm, y=N)) +
...
ggplot(data= filter(si.prey, Site=="Soda Bay" & PreyCat == "Crab"), aes(x=Cnorm, y=N)) +
  geom_point(aes(color=Season, shape= Species)) +
  labs(x=expression(paste(delta^13, "C (\u2030)")), 
       y=expression(paste(delta^15, "N (\u2030)" ))) +
  stat_ellipse(mapping=aes(color=Season)) +
...
ggplot(data= filter(si.prey, Site=="Craig" & PreyCat == "Crab"), aes(x=Cnorm, y=N)) +
  geom_point(aes(color=Season, shape= Species)) +
  labs(x=expression(paste(delta^13, "C (\u2030)")), 
       y=expression(paste(delta^15, "N (\u2030)" ))) +
  stat_ellipse(mapping=aes(color=Season)) +
...
si.summer<-filter(si.prey, Season=="Summer")
si.mean.su <-si.summer %>%
  group_by(PreyCat) %>% 
  drop_na(PreyCat) %>%
  summarise(Cmean=mean(Cnorm), Nmean=mean(N), Cmin=min(Cnorm), Cmax= max(Cnorm), 
            Csd=sd(Cnorm), Nsd=sd(N), Cse=sd(Cnorm)/sqrt(length(Cnorm)), 
            Nse=sd(N)/sqrt(length(N)))

#Winter values
si.winter<-filter(si.prey, Season=="Winter")
...
#Spring values
si.spring<-filter(si.prey, Season=="Spring")
...
si.craig<-filter(si.prey, Site=="Craig")
si.mean3 <-si.craig %>%
  group_by(PreyCat) %>% 
  drop_na(PreyCat) %>%
  summarise(Cmean=mean(Cnorm), Nmean=mean(N), Cmin=min(Cnorm), Cmax= max(Cnorm), 
...
#this isn't working for urchins. I don't know why
si.mean <-si.prey %>%
  group_by(PreyCat) %>% 
  drop_na(PreyCat) %>%
...
si.mean<-si.prey %>%
  group_by(PreyCat) %>%
  summarise(Cmean=mean(Cnorm), Csd=sd(Cnorm), Nmean=mean(N), Nsd=sd(N), n()) %>%
  drop_na(PreyCat)
write.csv(si.mean.site, "si_site.csv")
...
#Whisker Graph for ind otter Nitrogen
...
#Whisker Graph for ind otter Carbon
...
ggplot(data=whisker) +
  ...
  scale_y_continuous(sec.axis = sec_axis(~.-28, 
                        name = expression(paste(delta^13, "C (\u2030)" )))) +
  geom_smooth(aes(x=as.numeric(Season), y=N, color="N"), method= "loess", se= FALSE, show.legend = FALSE) +
  geom_smooth(aes(x=as.numeric(Season), y=C+28, color="C"), method= "loess", se= FALSE, show.legend = FALSE) +
...
ggplot(data=whisker) + theme_few()+
  ...
  stat_smooth(aes(x=as.numeric(Season), y=N, colour = "Nitrogen")) +
  stat_smooth(aes(x=as.numeric(Season), y=C+25, colour = "Carbon")) +
  labs(x= "Season", 
       y=expression(paste(delta^15, "N (\u2030)" )), 
       colour = "Isotope")  +
...
ggplot() +   
  theme_few() +
  geom_density(data=filter(si.prey, Season == "Summer"), 
               aes(x=N), alpha=.9, fill= "#E69F00") +
  geom_density(data=filter(si.prey, Season == "Spring"), 
               aes(x=N), alpha=.5, fill= "#009E73") +
  geom_density(data=filter(si.prey, Season == "Winter"), 
               aes(x=N), alpha=.4, fill = "#56B4E9") +
  labs(x=expression(paste(delta^15, "N" ))) 
...
ggplot() +   
  ...
  geom_density(data=filter(si.prey, Season == "Summer"| Season == "Spring"), 
               aes(x=N, colour= "Spring/Summer"), alpha=.5, fill = "darkgoldenrod1") +
