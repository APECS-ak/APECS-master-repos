...
Sea otters are hard to keep track of. Until the 2017 field season the most reliable measure of sea otters in the Prince of Wales Island region were the USFWS aerial surveys. These data provided densities and time since occupation of sea otters. However the most recent survey was done in 2011. In an effort to add more resolution to how we think about sea otters we collected a variety of sea otter impact measures over the 2017 field season. This script summarises and then combined these measures to creat a sea otter impact index. 
...
3. **Sea otter clams**. At each eelgrass site we collected dead clam shells along the same three transects described in the pit surveys. We only collected shells that had at least part of both valves. Clams were collected in 25 m bins along the transect but are pooled together for this analysis. All clams were measured, identified to species, and assigned a predation catagory. SOMEONE ELSE PLEASE WRITE HOW THE CRITERIA WORKS.
...
5. **Sea otter population model density**. Tim Tinker has recently developed a spatially expplicit density dependent growth model for sea otter in Southeast Alaska. His derived populaiton density estimates were assigned to each site. This data was an extraction from his paper.
...
## Summaries
Pit and clam shell data need to be summarised from its raw form to data we want to use for index calculation. As mentioned above pit and shell data will be pooled across transect at the site level.
...
pits.site <- pits.site[2:22,] # removes weird row
...
Now that we have all the data we can merge the summaries calcualted here and the data the was imported directly.
...
## Sea otter impact index
We will be using a principle compnents analysis to create the sea otter impact index 
```{r data exploration}
# Correlations
corrgram(dat[,6:11])
pairs(dat[,6:11])
...
```
Histograms inticate that the data are right skewed. Log transformation hellps this somewhat.
...
```{r export}
# Data Export
write.csv(dat, "sea_otter_impact_index_2017.csv", row.names = FALSE)
```
