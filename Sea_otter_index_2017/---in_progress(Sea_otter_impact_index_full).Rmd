...
h2o <- spTransform(h2o, CRS("+init=epsg:4326"))
```
...
leaflet() %>% 
  ...
  addPolygons(data = h2o, stroke = NA, fillColor = "red", fillOpacity = 0.8, group = "POW") %>% 
...
buff.water <- gIntersection(h2o, buff, byid = TRUE)
...
# Covert back to UTMs for eahc measurment
buff.water.utm <- spTransform(buff.water, CRS("+proj=utm +zone=8 +ellps=GRS80 +towgs84=0,0,0"))

# Save extent
water.ext <- extent(buff.water.utm)

# create raster with appropriate dimentions. Not that dividing by 50 means that each cell will be 50 by 50 meters
...
leaflet() %>% 
  addTiles() %>% 
  addRasterImage(water.rast.latlong, colors = "red", opacity = 0.8)
```

Create the as the water swims survey area, i.e. the actual survey area.
```{r survey area}
# Transition layer with 8 directions START HERE NEXT TIME
