...
for(i in unique(1:21)){
  ...
  w <- gBuffer(h2o.latlong, width = 0)
  bw <- gIntersection(w, b, byid = TRUE) # clip the buffer area by water so we only have water area left
  ...
  wt <- transition(wr, transitionFunction = function(x){50}, directions = 16) # conver raster to transition layer (required for accCost() below)
...
# Clean up NA row and funky site names. These are sites not in the study or double counts
...
## Data
For this you will need the sea otter pit data
```{r}
...
## Data
You will need sea otter clam data
```{r}
...
#### read/write in if you don't want to re-run all of the above
```{r read already caculated}
write.csv(site.dat, "../ALL_DATA/sea_otter_impact_index_new.csv", row.names = FALSE)
...
```{r}
site.dat.tr <- cbind(site.dat[, 1:9], log(site.dat[, 12] + 1), site.dat[,13], log(site.dat[, 14] + 1), asin(sqrt(site.dat[, 15])))
```
...
colnames(dat.redu.tr)[1:4] <- c("avg_surv", "pop_dens", "n_pits", "prop_so")
...
colnames(final.dat.tr)[14] <- "sea_otter_index"
final.dat.tr$site_num <- as.character(c(1:21))
...
# no color
soi.site <- ggplot(final.dat.tr, aes(reorder(site_num, sea_otter_index), sea_otter_index)) +
  geom_point(size = 2) +
...
final.dat.redu <- final.dat.all[, c(2, 12:16)]
final.dat.redu <- cbind.data.frame(final.dat.redu$place_name, round(final.dat.redu[, 2:6], 2))
