...
## Sites ##
# for 2019
sites <- readOGR(dsn = "../APECS Master repo/ALL_DATA/spatial_data", layer = "Otter_survey_sites_2019_UTM")
# for 2017
...
# change syntax for 2019 data
sites.eg <- subset(sites, habitat %in% "eelgrass")
# for 2019 remove site code column
# sites.eg <- sites.eg[,-2]

## Sea otters that where observed ##
# for 2019
otts <- readOGR(dsn = "../APECS Master repo/ALL_DATA/spatial_data", 
                layer = "so_survey_2019_UTM")

# for 2017: 
#otts <- readOGR(dsn = "../APECS Master repo/ALL_DATA/spatial_data", layer = "Otter_Survey_All_10Jan18_UTM")

# change 2019 dataframe to match exactly the 2017 data
otts@data$longitude <- otts@data$X 
otts@data$latitude <- otts@data$Y
otts@data <- otts@data %>%
  select(site, latitude, longitude, n_otter, date_YYYYM) %>%
  mutate(survey_dat = ymd(date_YYYYM)) %>%
  rename(n_otters = n_otter) %>%
  select(-date_YYYYM)

otts@data$n_otters <- as.numeric(as.character(otts@data$n_otters)) # data is imported as a factor
otts@data$survey_dat <- as.factor(otts@data$survey_dat)
...
require(rgdal)
df.otts <- readOGR(dsn="../APECS Master repo/ALL_DATA/spatial_data", 
                   layer="so_survey_2019_UTM")
df.otts <- as(df.otts, "data.frame")


write.csv(df.otts, "/Users/liadomke/liadomke1/Desktop/Grad School/UAF/Data/APECS Master repository/2019_Data_LD/Otter_Survey_2019/otter_gps_2019.csv", row.names = FALSE)
```
