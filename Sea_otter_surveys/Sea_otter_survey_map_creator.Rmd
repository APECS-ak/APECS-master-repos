...
```{r site}
site <- structure(list(id = "site", longitude = -133.166397, latitude = 55.594827), .Names = c("id", "longitude", "latitude"), class = "data.frame", row.names = c(NA, -1L))
xy <- site[,c(2,3)]
...
#leaflet() %>% 
  #addTiles(https://tileservice.charts.noaa.gov/tileset.html) %>% 
  #addCircleMarkers(data = site, ~longitude, ~latitude, color = "black", fillOpacity = 1, stroke = FALSE, radius = 5) %>% 
  #addPolygons(data = survey.poly.latlong, stroke = NA, fillColor = "yellow", fillOpacity = 0.5, group = "POW")
```
